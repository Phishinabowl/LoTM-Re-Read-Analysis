...
`Project_Config/reconciliation.yaml` schema version 1 owns auditable redirects, merges, splits, retirements, and tombstones across stable-record providers. `Tools/reconciliation_config.py` and `Tools/Reconciliation-Config.ps1` compose exact pack-controlled target providers from taxonomy, resources, sources, and entities; validate source existence state, operation cardinality, type preservation, historical supersession, terminal targets, and acyclic active resolution; and return canonical, redirected, ambiguous, or retired outcomes with traversal provenance. Reconciliation records are themselves provenance-addressable.

Resolution is read-only. It does not rename folders or files, rewrite page/YAML references, mutate aliases, or update generated graph IDs. The later migration service consumes reconciliation decisions through previewed, validated, reversible repository operations. A split never silently chooses a replacement, and a tombstone reserves a historical ID against reuse.
