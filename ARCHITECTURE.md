...
`Project_Config/reconciliation.yaml` schema version 2 owns auditable redirects, merges, splits, retirements, reclassifications, and tombstones across stable-record providers. `Tools/reconciliation_config.py` and `Tools/Reconciliation-Config.ps1` compose exact pack-controlled providers from taxonomy, resources, sources, and entities; validate provider alias ownership, source existence, operation/reason compatibility, operation cardinality, same-type behavior except explicit reclassification, privacy-aware source-label retention, repository-derived or explicit audit metadata, historical supersession, terminal targets, and acyclic active resolution. Cached iterative resolution returns canonical, redirected, ambiguous, or retired outcomes with both convenience summaries and branch-specific traversal provenance. Reconciliation records are themselves provenance-addressable.
