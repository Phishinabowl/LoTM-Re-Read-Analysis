...
`Project_Config/sources.yaml` schema version 14 owns:
...
- reusable provenance-addressable applicability scopes that target a work, segment, content group, work relationship, adaptation mapping, manifestation, release component, release package, or stable provenance claim and independently qualify it by territory, effective time, and explicit precedence;
- scope-backed work relationships, continuity assertions, claim supersessions, and work-production contexts that keep narrow applicability, canon state, production origin, authorization, rights basis, and commerciality independent from creative lineage;
...
Applicability is a reusable axis. A whole-work scope may coexist with a higher-precedence segment, relationship, mapping, manifestation, component, package, or claim scope. Exact target/territory/precedence scopes cannot overlap in effective time. Consumers must gather semantically applicable scopes and honor the highest explicit precedence; equal winning precedence is an ambiguity to report, not an invitation to guess. Work relationships may use a scope only when it resolves entirely inside their source work. Production contexts retain their owning `work_id` and use a scope to identify the exact material, relationship, territory, and time to which the context applies.
