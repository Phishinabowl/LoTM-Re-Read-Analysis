...
function Resolve-OperationName {
    ...
    switch -Regex ($Name) {
        ...
        '^(?i:extract|extractepubimages|extract-epub-images|extract-images|listepubimages|list-epub-images|list-images)$' {
            return "ExtractEpubImages"
        }
        default {
...
function Resolve-PresetName {
    ...
    switch -Regex ($Name) {
        '^(?i:pathwaytarotcard|pathway-tarot-card|pathway-tarot|tarot-card)$' {
            return "PathwayTarotCard"
        }
        '^(?i:pathwaysymbol|pathway-symbol|pathway-symbol-crop|symbol)$' {
            return "PathwaySymbol"
        }
        default {
    ...
    foreach ($match in $matches) {
        ...
        [pscustomobject]@{
            ...
            alt = if ($altMatch.Success) {
                [System.Net.WebUtility]::HtmlDecode($altMatch.Groups[1].Value)
            }
            else {
                $null
            }
...
function Get-ImageType {
    ...
    if ($leaf -eq 'cover.xhtml') {
        return "Cover"
    }
    if ($leaf -eq 'back_cover.xhtml') {
        return "BackCover"
    }
    if ($leaf -eq 'world_map.xhtml') {
        return "Map"
    }
    if ($leaf -in @('copyright.xhtml', 'foreword.xhtml', 'synopsis.xhtml', 'table_of_contents.xhtml')) {
        return "FrontMatter"
    }
    if ($Title -match '^Volume\s+\d+:') {
        return "VolumeCover"
    }
    if ($leaf -match 'end_of') {
        return "EndOfVolume"
    }
    if ($Title -eq 'Pathways Guide' -or $leaf -match 'pathways') {
        return "Pathways"
    }
    if ($Title -eq 'Characters' -or $leaf -match 'character|tarot') {
        return "Characters"
    }
    if ($Title -eq 'Locations' -or $leaf -match 'location') {
        return "Locations"
    }
    if ($Title -eq 'Image Gallery' -or $Title -eq 'Artwork' -or $leaf -match 'image_gallery|artwork') {
        return "Artwork"
    }
    ...
    try {
        ...
        foreach ($image in $selectedImages) {
            if ($Extract) {
                ...
                $safeAlt = if ([string]::IsNullOrWhiteSpace($image.alt)) {
                    "image"
                }
                else {
                    $image.alt -replace '[^A-Za-z0-9_-]+', '-'
                }
            ...
            if ($Json) {
                ...
                $volumeText = if ($null -ne $image.volume) {
                    "Vol $($image.volume)"
                }
                else {
                    "Vol -"
                }
                $extractText = if ($image.output_path) {
                    " | Extracted: $($image.output_path)"
                }
                else {
                    ""
                }
                $descriptionParts = @(
                    ...
                    "Spine $($image.spine_index)"
                    $image.image_type
                    $volumeText
                    $image.xhtml_file
                    $image.image_file
                    $image.title
                    "Alt: $($image.alt)$extractText"
                )
