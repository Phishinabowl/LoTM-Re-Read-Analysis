...
$valid=ConvertFrom-KnowledgeYamlFile (Join-Path $fixtures "valid-windows.yaml") 1 "Temporal fixture"
$windows=[ordered]@{}
...
$malformed=ConvertFrom-KnowledgeYamlFile (Join-Path $fixtures "invalid-windows.yaml") 1 "Temporal fixture"
...
foreach($case in @($expected.matches)){$instant=(ConvertTo-KnowledgeTemporalInstant $case[1]).instant;$outcome=Get-KnowledgeTemporalMatch $windows[$case[0]] $instant;if($null -eq $outcome){$outcome="not-effective"};if($outcome -ne $case[2]){throw "Temporal match vector failed for $($case[0]) at $($case[1]): $outcome"}}
