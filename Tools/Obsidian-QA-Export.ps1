...
. $projectConfigHelper
$taxonomyConfigHelper = Join-Path $PSScriptRoot "Taxonomy-Config.ps1"
if (-not (Test-Path -LiteralPath $taxonomyConfigHelper -PathType Leaf)) {
    throw "Taxonomy configuration helper not found: $taxonomyConfigHelper"
}
. $taxonomyConfigHelper
...
function New-Relationship {
    ...
    return [pscustomobject]@{
        source = if ($Data.ContainsKey("source")) {
            $Data["source"]
        }
        else {
            ""
        }
        target = if ($Data.ContainsKey("target")) {
            $Data["target"]
        }
        else {
            ""
        }
        relationship_type = if ($Data.ContainsKey("relationship_type")) {
            $Data["relationship_type"]
        }
        else {
            ""
        }
        status = if ($Data.ContainsKey("status")) {
            $Data["status"]
        }
        else {
            ""
        }
        confidence = if ($Data.ContainsKey("confidence")) {
            $Data["confidence"]
        }
        else {
            ""
        }
        notes = if ($Data.ContainsKey("notes")) {
            $Data["notes"]
        }
        else {
            ""
        ...
        start_medium = if ($Data.ContainsKey("start_medium")) {
            $Data["start_medium"]
        }
        else {
            ""
        }
        start_volume = if ($Data.ContainsKey("start_volume")) {
            $Data["start_volume"]
        }
        else {
            ""
        }
        start_chapter = if ($Data.ContainsKey("start_chapter")) {
            $Data["start_chapter"]
        }
        else {
            ""
        }
        projection_source = if ($Data.ContainsKey("projection_source")) {
...
function New-AvailabilityEntry {
    ...
    return [pscustomobject]@{
        medium = if ($Data.ContainsKey("medium")) {
            $Data["medium"]
        }
        else {
            ""
        }
        volume = if ($Data.ContainsKey("from_volume")) {
            $Data["from_volume"]
        }
        elseif ($Data.ContainsKey("volume")) {
            $Data["volume"]
        }
        else {
            ""
        }
        chapter = if ($Data.ContainsKey("from_chapter")) {
            $Data["from_chapter"]
        }
        elseif ($Data.ContainsKey("chapter")) {
            $Data["chapter"]
        }
        else {
            ""
        }
        season = if ($Data.ContainsKey("from_season")) {
            $Data["from_season"]
        }
        elseif ($Data.ContainsKey("season")) {
            $Data["season"]
        }
        else {
            ""
        }
        episode = if ($Data.ContainsKey("from_episode")) {
            $Data["from_episode"]
        }
        elseif ($Data.ContainsKey("episode")) {
            $Data["episode"]
        }
        else {
            ""
        }
        release_order = if ($Data.ContainsKey("from_release_order")) {
            $Data["from_release_order"]
        }
        elseif ($Data.ContainsKey("release_order")) {
            $Data["release_order"]
        }
        else {
            ""
        ...
        elseif ($Data.ContainsKey("possession_status")) {
            $Data["possession_status"]
        }
        elseif ($Data.ContainsKey("access_status")) {
            $Data["access_status"]
        }
        elseif ($Data.ContainsKey("outcome_status")) {
            $Data["outcome_status"]
        }
        else {
            ""
        ...
        graph_visibility = if ($Data.ContainsKey("graph_visibility")) {
            $Data["graph_visibility"]
        }
        else {
            ""
        }
        adaptation_relationship = if ($Data.ContainsKey("adaptation_relationship")) {
            $Data["adaptation_relationship"]
        }
        else {
            ""
        }
    ...
    foreach ($block in Get-FencedYamlBlocks $Text) {
        ...
        foreach ($rawLine in $block.Text -split "`r?`n") {
            ...
            if ($indent -eq 0 -and -not $line.StartsWith("- ")) {
                if ($null -ne $availabilityItem) {
                    $availability += @(New-AvailabilityEntry $availabilityItem)
                }
                ...
                $inFrom = $false
                $parts = $line.Split(":", 2)
                $rootKey = if ((ConvertFrom-Scalar $parts[1]) -eq "") {
                    $parts[0].Trim()
                }
                else {
                    ""
            ...
            if ($rootKey -and $indent -eq 2 -and -not $line.StartsWith("- ")) {
                ...
                $inFrom = $false
                $parts = $line.Split(":", 2)
                $sectionKey = if ((ConvertFrom-Scalar $parts[1]) -eq "") {
                    $parts[0].Trim()
                }
                else {
                    ""
            ...
            if ($indent -eq 0 -and -not $line.StartsWith("- ")) {
                if ($null -ne $row) {
                    $beats += [pscustomobject]@{
                        medium = if ($row.ContainsKey("medium")) {
                            $row["medium"]
                        }
                        else {
                            ""
                        }
                        beat_type = if ($row.ContainsKey("beat_type")) {
                            $row["beat_type"]
                        }
                        else {
                            ""
                        }
                        title = if ($row.ContainsKey("title")) {
                            $row["title"]
                        }
                        else {
                            ""
                        ...
                        reader_knowledge_state = if ($row.ContainsKey("reader_knowledge_state")) {
                            $row["reader_knowledge_state"]
                        }
                        else {
                            ""
                        }
                        viewer_knowledge_state = if ($row.ContainsKey("viewer_knowledge_state")) {
                            $row["viewer_knowledge_state"]
                        }
                        else {
                            ""
                        }
                        graph_behavior = if ($graphDisplay.ContainsKey("behavior")) {
                            $graphDisplay["behavior"]
                        }
                        else {
                            ""
                        }
                        graph_label = if ($graphDisplay.ContainsKey("label")) {
                            $graphDisplay["label"]
                        }
                        else {
                            ""
                        }
                        status = if ($row.ContainsKey("status")) {
                            $row["status"]
                        }
                        else {
                            ""
                        }
                        confidence = if ($row.ContainsKey("confidence")) {
                            $row["confidence"]
                        }
                        else {
                            ""
...
function ConvertFrom-BoundedGraphSpec {
    ...
    $name = if ($values.ContainsKey("name") -and $values["name"]) {
        $values["name"]
    }
    else {
        "bounded-graph-$Index"
    }
    $medium = if ($values.ContainsKey("medium") -and $values["medium"]) {
        $values["medium"]
    }
    else {
        "novel"
    }
    $fileStemSource = if ($values.ContainsKey("file_stem") -and $values["file_stem"]) {
        $values["file_stem"]
    }
    else {
        $name
    ...
    return [pscustomobject]@{
        ...
        medium = $medium
        max_volume = if ($values.ContainsKey("max_volume")) {
            $values["max_volume"]
        }
        else {
            ""
        }
        max_chapter = if ($values.ContainsKey("max_chapter")) {
            $values["max_chapter"]
        }
        else {
            ""
        }
        max_season = if ($values.ContainsKey("max_season")) {
            $values["max_season"]
        }
        else {
            ""
        }
        max_episode = if ($values.ContainsKey("max_episode")) {
            $values["max_episode"]
        }
        else {
            ""
        }
        max_release_order = if ($values.ContainsKey("max_release_order")) {
            $values["max_release_order"]
        }
        else {
            ""
        }
        include_unknown_subjects = ConvertTo-BoolValue $(if ($values.ContainsKey("include_unknown_subjects")) {
                $values["include_unknown_subjects"]
            }
            else {
                "false"
            })
        include_unknown_positions = ConvertTo-BoolValue $(if ($values.ContainsKey("include_unknown_positions")) {
                $values["include_unknown_positions"]
            }
            else {
                "false"
            })
...
function ConvertFrom-BoundedPageSpec {
    ...
    return [pscustomobject]@{
        slug = $values["slug"]
        name = if ($values.ContainsKey("name")) {
            $values["name"]
        }
        else {
            ""
        }
        file_stem = if ($values.ContainsKey("file_stem")) {
            $values["file_stem"]
        }
        else {
            ""
        ...
        include_anonymous_preview = ConvertTo-BoolValue $(if ($values.ContainsKey("include_anonymous_preview")) {
                $values["include_anonymous_preview"]
            }
            else {
                "true"
            })
...
function Format-SourceLink {
    ...
    $sourceWithoutSuffix = if ($SourceFile.EndsWith(".md")) {
...
function Format-EdgeLine {
    ...
    $relationshipType = if ($Relationship.relationship_type) {
        $Relationship.relationship_type
    }
    else {
        "relationship"
    }
    $fieldName = if ($Incoming) {
...
function ConvertTo-FirstAppearanceBeatMarkdown {
    ...
    foreach ($beat in $Beats) {
        $beatTitle = @(if ($beat.beat_type) {
                $beat.beat_type
            }
            if ($beat.title) {
                $beat.title
            }) -join " - "
        $knowledge = @(if ($beat.reader_knowledge_state) {
                $beat.reader_knowledge_state
            }
            if ($beat.viewer_knowledge_state) {
                $beat.viewer_knowledge_state
            }) -join " / "
        $graphDisplay = $beat.graph_behavior
        if ($beat.graph_label) {
            $graphDisplay = if ($graphDisplay) {
                "${graphDisplay}: $($beat.graph_label)"
            }
            else {
                $beat.graph_label
            }
        }
        $status = @(if ($beat.status) {
                $beat.status
            }
            if ($beat.confidence) {
                $beat.confidence
...
function ConvertTo-NoteMarkdown {
    ...
    if ($outgoing.Count -gt 0) {
        foreach ($rel in $outgoing) {
            $lines += Format-EdgeLine $rel $Notes
        }
    }
    else {
        $lines += "- None generated."
    ...
    if ($incoming.Count -gt 0) {
        foreach ($rel in $incoming) {
            $lines += Format-EdgeLine $rel $Notes -Incoming
        }
    }
    else {
        $lines += "- None generated."
    ...
    if ($outgoingRefs.Count -gt 0) {
        foreach ($ref in $outgoingRefs) {
            $lines += Format-DataRefLine $ref $Notes
        }
    }
    else {
        $lines += "- None generated."
    ...
    if ($incomingRefs.Count -gt 0) {
        foreach ($ref in $incomingRefs) {
            $lines += Format-DataRefLine $ref $Notes -Incoming
        }
    }
    else {
        $lines += "- None generated."
...
function Get-RelationshipProvenanceLines {
    ...
    foreach ($rel in @($Relationships | Sort-Object @{ Expression = { Get-RelationshipSourceDomain $_.source_file } }, confidence, start_chapter, source_file)) {
        ...
        if ($rel.start_medium) {
            $parts += $rel.start_medium
        }
        if ($rel.confidence) {
            $parts += $rel.confidence
        }
        if ($rel.start_chapter) {
            $parts += "ch$($rel.start_chapter)"
        }
        if ($rel.status -and $rel.status -ne "active") {
            $parts += $rel.status
        }
        $lines += ($parts -join " ")
    }
...
function ConvertTo-LabeledRelationshipGraph {
    ...
    foreach ($line in Get-QAGraphClassDefinitions) {
...
function ConvertTo-RelationshipNodeGraph {
    ...
    foreach ($key in @($grouped.Keys | Sort-Object)) {
        $parts = $key -split "\|", 3
        $labelLines = @(if ($grouped[$key].Count -gt 1) {
                "$($parts[1]) x$($grouped[$key].Count)"
            }
            else {
                $parts[1]
            })
    ...
    foreach ($line in Get-QAGraphClassDefinitions -IncludeRelationship) {
        $lines.Add($line) | Out-Null
    }
    Add-QAGraphClassAssignments $lines $usedSlugs $Notes
    if ($index -gt 1) {
...
function Format-RelationshipSourceLine {
    ...
    $parts = @($domain, "seed")
    if ($Relationship.start_medium) {
        $parts += $Relationship.start_medium
    }
    if ($Relationship.start_chapter) {
        $parts += "ch$($Relationship.start_chapter)"
    }
    if ($Relationship.confidence) {
        $parts += $Relationship.confidence
    }
    if ($Relationship.status -and $Relationship.status -ne "active") {
...
function Format-AvailabilityEntry {
    ...
    elseif ($Entry.medium -eq "donghua") {
        ...
        if ($Entry.season -and $Entry.season -ne "TBD") {
            $parts += "s$($Entry.season)"
        }
        if ($Entry.episode -and $Entry.episode -ne "TBD") {
            $parts += "e$($Entry.episode)"
        }
        if ($Entry.release_order -and $Entry.release_order -ne "TBD") {
            $parts += "r$($Entry.release_order)"
        }
    }
    elseif ($Entry.medium) {
        $parts += $Entry.medium
...
function ConvertTo-DataReferenceIndex {
    ...
    foreach ($ref in @($DataReferences | Sort-Object source, target, context_key, yaml_block, source_file)) {
        $lines += "| " + (@(
...
function Get-OrphanAnalysis {
    ...
    foreach ($rel in $Relationships) {
        if ($rel.source) {
            [void]$relSources.Add($rel.source)
        }
        if ($rel.target) {
            [void]$relTargets.Add($rel.target)
        }
    }
    foreach ($ref in $DataReferences) {
        if ($ref.source) {
            [void]$dataSources.Add($ref.source)
        }
        if ($ref.target) {
            [void]$dataTargets.Add($ref.target)
        }
    }

    $allSources = New-Object 'System.Collections.Generic.HashSet[string]'
    foreach ($item in @($relSources + $dataSources)) {
        [void]$allSources.Add($item)
    }
    $allTargets = New-Object 'System.Collections.Generic.HashSet[string]'
    foreach ($item in @($relTargets + $dataTargets)) {
        [void]$allTargets.Add($item)
    }
...
function ConvertTo-OrphanReport {
    ...
    $sections = @(
        [pscustomobject]@{ Heading = "Unknown Relationship Sources"
            Key = "unknown_relationship_sources"
        },
        [pscustomobject]@{ Heading = "Unknown Relationship Targets"
            Key = "unknown_relationship_targets"
        },
        [pscustomobject]@{ Heading = "Unknown Data Block Targets"
            Key = "unknown_data_targets"
        },
        [pscustomobject]@{ Heading = "Canonical Notes With No Edges Or Data References"
            Key = "notes_without_any_edges_or_refs"
        },
        [pscustomobject]@{ Heading = "Canonical Notes With No Outgoing Relationship Seeds"
            Key = "notes_without_outgoing_relationships"
        }
    )
...
function ConvertTo-SuspiciousEdges {
    ...
    if ($data.same_type_known_edges.Count -gt 0) {
        foreach ($rel in $data.same_type_known_edges) {
            $sourceLink = Format-WikiLink $rel.source $Notes
            $targetLink = Format-WikiLink $rel.target $Notes
            $sourceType = $Notes[$rel.source].type_name
            $targetType = $Notes[$rel.target].type_name
            $lines += "- $sourceLink $($rel.relationship_type) -> $targetLink (``$sourceType`` to ``$targetType``)"
        }
    }
    else {
        $lines += "- None."
...
function Write-BoundedGraphs {
    ...
    foreach ($spec in @($Specs)) {
        ...
        if ($spec.max_volume) {
            $boundary["maxVolume"] = $spec.max_volume
        }
        if ($spec.max_chapter) {
            $boundary["maxChapter"] = $spec.max_chapter
        }
        if ($spec.max_season) {
            $boundary["maxSeason"] = $spec.max_season
        }
        if ($spec.max_episode) {
            $boundary["maxEpisode"] = $spec.max_episode
        }
        if ($spec.max_release_order) {
            $boundary["maxReleaseOrder"] = $spec.max_release_order
        }
...
function Get-MapKeys {
    ...
    if ($Map -is [string] -or $Map.GetType().IsPrimitive) {
        return @()
    }
    if ($Map -is [System.Collections.IDictionary]) {
        return @($Map.Keys)
    }
    if ($Map -is [System.Collections.IEnumerable]) {
        return @()
    }
...
function Get-ProfileYaml {
    ...
    if (-not $rootByType.ContainsKey($typeKey)) {
        return @{}
    }
...
function ConvertTo-NullableInt {
    ...
    if (-not $text -or $text.ToUpperInvariant() -eq "TBD") {
        return $null
    }
    $number = 0
...
function Get-BoundaryLabel {
    ...
    if ($medium -eq "novel") {
        $parts = @("novel")
        if ($Spec.max_volume) {
            $parts += "V$($Spec.max_volume)"
        }
        if ($Spec.max_chapter) {
            $parts += "Ch$($Spec.max_chapter)"
        }
        return ($parts -join " ")
    }
    if ($medium -eq "donghua") {
        $parts = @("donghua")
        if ($Spec.max_season) {
            $parts += "S$($Spec.max_season)"
        }
        if ($Spec.max_episode) {
            $parts += "E$($Spec.max_episode)"
        }
        if ($Spec.max_release_order) {
            $parts += "Release $($Spec.max_release_order)"
        }
        return ($parts -join " ")
    }
    return $Spec.medium
...
function Get-PositionFromMetadata {
    ...
    if ($mediumMatch.Success) {
        $position["medium"] = $mediumMatch.Groups[1].Value.ToLowerInvariant()
    }
    ...
    if ($volume.Success) {
        $position["volume"] = $volume.Groups[1].Value
    }
    if ($chapter.Success) {
        $position["chapter"] = $chapter.Groups[1].Value
    }
    if ($season.Success) {
        $position["season"] = $season.Groups[1].Value
    }
    if ($episode.Success) {
        $position["episode"] = $episode.Groups[1].Value
    }
    return $position
}
...
function Get-EntryPosition {
    param([object]$Entry)
    if (@(Get-MapKeys $Entry).Count -eq 0) {
        return @{}
    }
...
function Test-PositionVisible {
    ...
    if ($medium -and $medium -ne $Boundary["medium"]) {
        return $false
    }
    if ($Boundary["medium"] -eq "novel") {
        ...
        if ($null -eq $volume -and $null -eq $chapter) {
            return $false
        }
        ...
        if ($null -ne $maxChapter -and $null -ne $chapter) {
            return $chapter -le $maxChapter
        }
    ...
    if ($Boundary["medium"] -eq "donghua") {
        ...
        if ($null -ne $releaseOrder -and $null -ne $maxReleaseOrder) {
            return $releaseOrder -le $maxReleaseOrder
        }
        if ($null -eq $season -and $null -eq $episode) {
            return $false
        }
        if ($null -ne $maxSeason -and $null -ne $season) {
            if ($season -gt $maxSeason) {
                return $false
            }
            if ($season -lt $maxSeason) {
                return $true
            }
        }
        if ($null -ne $maxEpisode -and $null -ne $episode) {
            return $episode -le $maxEpisode
        }
...
function Get-RowVisibleAvailability {
    ...
    if (-not ($entries -is [System.Collections.IEnumerable]) -or $entries -is [string]) {
        return @()
    }
    $visible = @()
    foreach ($entry in @($entries)) {
...
function Test-RowVisible {
    ...
    if (@(Get-MapKeys $Row).Count -eq 0) {
        return $false
    }
    if (Test-MapHasKey $Row "availability") {
        return @((Get-RowVisibleAvailability $Row $Boundary)).Count -gt 0
    }
    ...
    foreach ($key in @("visibility", "from", "position")) {
        if (Test-MapHasKey $Row $key) {
            return Test-PositionVisible (Get-EntryPosition $Row) $Boundary
        }
    }
    ...
    if ($sourceRefs -is [System.Collections.IEnumerable] -and -not ($sourceRefs -is [string])) {
        foreach ($ref in @($sourceRefs)) {
            if ($ref -is [System.Collections.IDictionary] -and (Test-PositionVisible (Get-EntryPosition $ref) $Boundary)) {
                return $true
            }
        }
    }
...
function Get-FilteredProfileRowsForBoundary {
    ...
    foreach ($section in $Profile.Keys) {
        ...
        if ($rows -is [System.Collections.IDictionary] -or @(Get-MapKeys $rows).Count -gt 0) {
            $isSingleRow = (
                (Test-MapHasKey $rows "availability") -or
                (Test-MapHasKey $rows "source_refs") -or
                (Test-MapHasKey $rows "position") -or
                (Test-MapHasKey $rows "visibility") -or
                (Test-MapHasKey $rows "from") -or
                (Test-MapHasKey $rows "to")
            )
            if ($isSingleRow) {
                $rowCandidates = @($rows)
            }
...
function Format-Position {
    ...
    if (@(Get-MapKeys $Position).Count -eq 0) {
        return ""
    }
    $medium = Get-NormalizedMedium (Get-MapValue $Position "medium" "")
    ...
    if ($medium -eq "donghua" -or (Get-MapValue $Position "episode" "") -or (Get-MapValue $Position "season" "")) {
        ...
        if ($episodeValue) {
            $parts += "E$episodeValue"
        }
        if ($releaseOrderValue) {
            $parts += "Release $releaseOrderValue"
        }
        return ($parts -join " ")
    }
    $parts = @()
    foreach ($key in $Position.Keys) {
        if ($null -ne $Position[$key] -and ([string]$Position[$key]) -ne "") {
            $parts += "$key`: $($Position[$key])"
        }
    }
    return ($parts -join ", ")
}
...
function ConvertTo-TableValue {
    ...
    if (@(Get-MapKeys $Value).Count -gt 0) {
        return Format-Position $Value
    }
    if ($Value -is [System.Collections.IEnumerable] -and -not ($Value -is [string])) {
        $parts = @()
        foreach ($item in @($Value)) {
            $parts += ConvertTo-TableValue $item
        }
...
function Format-AvailabilityProgression {
    ...
    if ($sourceRefs -is [System.Collections.IEnumerable] -and -not ($sourceRefs -is [string])) {
        foreach ($ref in @($sourceRefs)) {
            if (-not ($ref -is [System.Collections.IDictionary])) {
                continue
            }
            $position = Format-Position $ref
            if ($position -and -not ($parts | Where-Object { $_.StartsWith($position) })) {
                $parts += "$position`: source-ref"
            }
        }
    }
    ...
    if (@(Get-MapKeys $graphDisplay).Count -gt 0) {
        ...
        if (@(Get-MapKeys $visibleFrom).Count -gt 0) {
            $parts += "graph visible from $(Format-Position $visibleFrom)"
        }
        if (@(Get-MapKeys $resolvesTo).Count -gt 0) {
            $parts += "resolves to canonical at $(Format-Position $resolvesTo)"
        }
    }
    return ($parts -join "<br>")
}
...
function Get-GraphDisplayForBoundary {
    ...
    if ($graphRows.Count -gt 0) {
        ...
        return [ordered]@{
            node_visible = if ($preferred.behavior -ne "hidden") {
                "yes"
            }
            else {
                "no"
...
function ConvertTo-GenerationStatsMarkdown {
    ...
    $rows = @(
        ...
        @("Canonical page visible", $(if ($PageVisible) {
                    "yes"
                }
                else {
                    "no"
...
$BoundedCharacterTables = @(
    [pscustomobject]@{ Title = "First Appearance Beats"
        Key = "first_appearance_beats"
        Optional = $false
        Columns = @(@("Title", "title"), @("Type", "beat_type"), @("State", "reader_knowledge_state"), @("Status", "status"), @("Confidence", "confidence"))
    },
    [pscustomobject]@{ Title = "Identity / Role"
        Key = "identities"
        Optional = $false
        Columns = @(@("Field", "field"), @("Value", "value"), @("Status", "status"), @("Confidence", "confidence"), @("Notes", "notes"))
    },
    [pscustomobject]@{ Title = "Physical Profile"
        Key = "physical_profile"
        Optional = $false
        Columns = @(@("Field", "field"), @("Value", "value"), @("Status", "status"), @("Confidence", "confidence"), @("Notes", "notes"))
    },
    [pscustomobject]@{ Title = "Status / Origin / Location"
        Key = "status_origin_location"
        Optional = $false
        Columns = @(@("Field", "field"), @("Value", "value"), @("Status", "status"), @("Confidence", "confidence"), @("Notes", "notes"))
    },
    [pscustomobject]@{ Title = "Affiliations"
        Key = "affiliations"
        Optional = $false
        Columns = @(@("Organization", "organization"), @("Target", "target"), @("Relationship", "relationship"), @("Status", "status"), @("Confidence", "confidence"))
    },
    [pscustomobject]@{ Title = "Pathway State"
        Key = "pathway_state"
        Optional = $false
        Columns = @(@("Pathway", "pathway"), @("Target", "target"), @("Relationship", "relationship"), @("Status", "status"), @("Confidence", "confidence"))
    },
    [pscustomobject]@{ Title = "Sequence State"
        Key = "sequence_state"
        Optional = $false
        Columns = @(@("Sequence", "sequence"), @("Name", "sequence_name"), @("Pathway", "related_pathway"), @("Status", "status"), @("Confidence", "confidence"))
    },
    [pscustomobject]@{ Title = "Associated Tarot Card"
        Key = "associated_tarot_card"
        Optional = $true
        Columns = @(
            @("Card", "card_name")
            @("Number", "card_number")
    ...
    [pscustomobject]@{ Title = "Mythical Creature Form State"
        Key = "mythical_creature_form_state"
        Optional = $true
        Columns = @(
    ...
    [pscustomobject]@{ Title = "Uniqueness State"
        Key = "uniqueness_state"
        Optional = $true
        Columns = @(@("Uniqueness", "uniqueness"), @("Relationship", "relationship_state"), @("Status", "status"), @("Confidence", "confidence"), @("Notes", "notes"))
    },
    [pscustomobject]@{ Title = "Ability State"
        Key = "ability_state"
        Optional = $true
        Columns = @(@("Field", "field"), @("Value", "value"), @("Status", "status"), @("Confidence", "confidence"), @("Notes", "notes"))
    },
    [pscustomobject]@{ Title = "Abilities"
        Key = "ability_index"
        Optional = $false
        Columns = @(@("Ability", "ability"), @("Source", "source"), @("Target", "target"), @("Status", "status"), @("Confidence", "confidence"), @("Notes", "notes"))
    },
    [pscustomobject]@{ Title = "Equipment / Artifacts / Items"
        Key = "equipment_artifacts"
        Optional = $false
        Columns = @(@("Item", "item"), @("Target", "target"), @("Type", "type"), @("Possession", "possession_status"), @("Significance", "item_significance"), @("Graph", "graph_relevance"))
    },
    [pscustomobject]@{ Title = "Knowledge Sources & Documents"
        Key = "knowledge_sources_documents"
        Optional = $true
        Columns = @(@("Source", "source"), @("Target", "target"), @("Type", "type"), @("Access", "access_status"), @("Significance", "source_significance"), @("Graph", "graph_relevance"))
    },
    [pscustomobject]@{ Title = "Personality"
        Key = "personality"
        Optional = $false
        Columns = @(@("Trait", "trait"), @("Evidence", "evidence"), @("Status", "status"), @("Confidence", "confidence"), @("Notes", "notes"))
    },
    [pscustomobject]@{ Title = "Relationships"
        Key = "relationships"
        Optional = $false
        Columns = @(@("Target", "target"), @("Relationship", "relationship"), @("Status", "status"), @("Confidence", "confidence"), @("Notes", "notes"))
    },
    [pscustomobject]@{ Title = "Messengers / Servants / Companions"
        Key = "messengers_servants_companions"
        Optional = $true
        Columns = @(@("Entity", "entity"), @("Label", "label"), @("Type", "type"), @("Function", "function"), @("Status", "status"), @("Confidence", "confidence"))
    },
    [pscustomobject]@{ Title = "Prayers & Ritual Access"
        Key = "prayers_ritual_access"
        Optional = $true
        Columns = @(@("Label", "label"), @("Type", "type"), @("Function", "function"), @("Concept Link", "concept_link"), @("Status", "status"), @("Confidence", "confidence"))
    },
    [pscustomobject]@{ Title = "Major Events"
        Key = "major_events_fights"
        Optional = $false
        Columns = @(@("Event", "event"), @("Target", "target"), @("Type", "event_type"), @("Part", "event_part"), @("Role", "role"), @("Outcome", "outcome_status"))
    },
    [pscustomobject]@{ Title = "Timeline Entries"
        Key = "timeline_entries"
        Optional = $false
        Columns = @(@("ID", "id"), @("Title", "title"), @("Type", "entry_type"), @("Summary", "summary"), @("Why It Matters", "why_it_matters"))
    }
)
...
function ConvertTo-BoundedCharacterPageMarkdown {
    ...
    if (-not $pageVisible) {
        ...
        foreach ($key in @($filtered.Keys)) {
            if ($key -ne "first_appearance_beats") {
                $filtered[$key] = @()
            }
        }
    }
    ...
    foreach ($table in $BoundedCharacterTables) {
        if ($table.Optional -and -not (Test-MapHasKey $profile $table.Key) -and -not (Test-MapHasKey $filtered $table.Key)) {
            continue
        }
        if ($table.Optional -and @(Get-MapValue $filtered $table.Key @()).Count -eq 0) {
            continue
        }
...
function Write-BoundedPages {
    ...
    foreach ($spec in @($Specs)) {
        if (-not $Notes.ContainsKey($spec.slug)) {
            ...
            $fileStemSource = if ($spec.file_stem) {
                $spec.file_stem
            }
            elseif ($spec.name) {
                $spec.name
        ...
        $boundaryStem = if ($spec.max_chapter) {
            "chapter-$($spec.max_chapter)"
        }
        else {
            ConvertTo-SafeSlug (Get-BoundaryLabel $spec) "boundary"
        }
        $defaultStem = "$($note.export_file_stem) - $boundaryStem"
        $fileStem = ConvertTo-SafeFileName $(if ($spec.file_stem) {
                $spec.file_stem
            }
            else {
                $defaultStem
...
$exportPath = if ([System.IO.Path]::IsPathRooted($selectedOutputDir)) {
    $selectedOutputDir
}
else {
    Join-Path $repoRoot $selectedOutputDir
}
...
Write-Output (
    (
        'QA: {0} unknown relationship sources, {1} unknown relationship targets, ' +
        '{2} unknown data targets, {3} self loops, {4} duplicate edge groups, ' +
        '{5} missing expected reciprocals.'
    ) -f
    $summary.unknown_relationship_sources,
    $summary.unknown_relationship_targets,
    $summary.unknown_data_targets,
    $summary.self_loops,
    $summary.duplicate_edge_groups,
