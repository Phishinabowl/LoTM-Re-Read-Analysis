...
function Assert-InvalidOccurrenceDeclarations{param([object]$Packs,[string]$Namespace,[object[]]$Values,[string]$Label);$invalid=[ordered]@{};foreach($key in $Packs.controlled_values.Keys){$invalid[$key]=@($Packs.controlled_values[$key])};$invalid[$Namespace]=@($Values);$rejected=$false;try{Assert-SchemaPackOccurrenceSemanticDeclarations $invalid}catch{$rejected=$true};if(-not $rejected){throw "Malformed semantic declaration unexpectedly loaded: $Label"}}
...
foreach($vector in @($expectations.resolved_effects)){$evaluation=Get-KnowledgeRecurrenceRuleEvaluation $fixture ([string]$vector[0]) ([string]$vector[1]) $vector[2];$expectedEffects=@($vector[3]);if(@($evaluation.effects).Count -ne $expectedEffects.Count){throw "Unexpected resolved effect count at '$($vector[1])'."};for($index=0;$index -lt $expectedEffects.Count;$index++){$actual=$evaluation.effects[$index];$expected=$expectedEffects[$index];if($actual.effect_kind -cne [string]$expected[0] -or $actual.target_type -cne [string]$expected[1] -or $actual.target_id -cne [string]$expected[2] -or $actual.repetition_policy -cne [string]$expected[3] -or [int]$actual.contribution_count -ne [int]$expected[4] -or [int]$actual.execution_count -ne [int]$expected[5]){throw "Unexpected resolved effect at '$($vector[1])' index $index."};Assert-OccurrenceIds @($actual.contributing_rule_ids) @($expected[6]) "Resolved effect rule contributors at '$($vector[1])'";Assert-OccurrenceIds @($actual.contributing_effect_ids) @($expected[7]) "Resolved effect row contributors at '$($vector[1])'"}}
...
if($owningEvaluation.status -cne 'conflict'){throw 'Owning-pattern extension did not produce a conflict.'};Assert-OccurrenceIds @($owningEvaluation.selected_rule_ids) @('outer-reset-rule','synthetic-pause-rule') 'Owning-pattern extension selected rules';Assert-OccurrenceIds @($owningEvaluation.conflicts) @('advance-iteration conflicts with pause-recurrence on recurrence-pattern:outer-loop-pattern') 'Owning-pattern extension conflicts'
...
$duplicateProbe=ConvertFrom-KnowledgeYamlFile $fixturePath 4 'occurrence fixture';$firstSignal=New-SyntheticOccurrenceRule 'first-signal-rule' 'signal' 'signal-recurrence' 'inner-loop-pattern' 'bell';$secondSignal=New-SyntheticOccurrenceRule 'second-signal-rule' 'signal' 'signal-recurrence' 'inner-loop-pattern' 'bell';$firstSignal['resolution_group']='first-signal-group';$secondSignal['resolution_group']='second-signal-group';$duplicateProbe['rules']=@($duplicateProbe['rules'])+@($firstSignal,$secondSignal)
$duplicateEvaluation=Get-KnowledgeRecurrenceRuleEvaluation (ConvertTo-KnowledgeOccurrenceRegistry $duplicateProbe $fixturePath $packs $chronologyFixture $subjectTargets $payloadTargets) 'outer-loop' 'bell-two';if(@($duplicateEvaluation.effects).Count -ne 1 -or $duplicateEvaluation.effects[0].contribution_count -ne 2 -or $duplicateEvaluation.effects[0].execution_count -ne 1){throw 'Unexpected idempotent effect resolution.'};Assert-OccurrenceIds @($duplicateEvaluation.effects[0].contributing_rule_ids) @('first-signal-rule','second-signal-rule') 'Idempotent effect contributors'
foreach($policyCase in @([pscustomobject]@{policy='accumulating';status='selected';execution_count=2;conflicts=@()},[pscustomobject]@{policy='invalid';status='conflict';execution_count=0;conflicts=@('duplicate signal-recurrence effect on recurrence-pattern:inner-loop-pattern is invalid')})){$policyValues=[ordered]@{};foreach($namespace in $packs.controlled_values.Keys){$policyValues[$namespace]=@($packs.controlled_values[$namespace])};$policyValues['occurrence.rule-effect-repetition-policy']=@($policyValues['occurrence.rule-effect-repetition-policy']|Where-Object {$_ -cne 'signal-recurrence-uses-idempotent'})+@("signal-recurrence-uses-$($policyCase.policy)");Assert-SchemaPackOccurrenceSemanticDeclarations $policyValues;$policyPacks=$packs.PSObject.Copy();$policyPacks.controlled_values=$policyValues;$policyEvaluation=Get-KnowledgeRecurrenceRuleEvaluation (ConvertTo-KnowledgeOccurrenceRegistry $duplicateProbe $fixturePath $policyPacks $chronologyFixture $subjectTargets $payloadTargets) 'outer-loop' 'bell-two';if($policyEvaluation.status -cne $policyCase.status -or $policyEvaluation.effects[0].execution_count -ne $policyCase.execution_count){throw "Unexpected '$($policyCase.policy)' effect resolution."};Assert-OccurrenceIds @($policyEvaluation.conflicts) @($policyCase.conflicts) "$($policyCase.policy) effect conflicts"}

$scopedValues=[ordered]@{};foreach($namespace in $packs.controlled_values.Keys){$scopedValues[$namespace]=@($packs.controlled_values[$namespace])};$scopedValues['occurrence.rule-effect-pattern-scope']=@('advance-iteration-uses-owning-pattern','terminate-recurrence-uses-owning-pattern','pause-recurrence-allows-external-pattern','signal-recurrence-allows-external-pattern');Assert-SchemaPackOccurrenceSemanticDeclarations $scopedValues;$scopedPacks=$packs.PSObject.Copy();$scopedPacks.controlled_values=$scopedValues
$scopedProbe=ConvertFrom-KnowledgeYamlFile $fixturePath 4 'occurrence fixture';$scopedProbe['rules']=@($scopedProbe['rules'])+@(New-SyntheticOccurrenceRule 'cross-target-pause-rule' 'pause' 'pause-recurrence' 'inner-loop-pattern' 'reset');$scopedEvaluation=Get-KnowledgeRecurrenceRuleEvaluation (ConvertTo-KnowledgeOccurrenceRegistry $scopedProbe $fixturePath $scopedPacks $chronologyFixture $subjectTargets $payloadTargets) 'outer-loop' 'reset-two';if($scopedEvaluation.status -cne 'selected' -or @($scopedEvaluation.conflicts).Count -ne 0){throw 'Cross-target same-target pair unexpectedly conflicted.'}

$globalValues=[ordered]@{};foreach($namespace in $scopedValues.Keys){$globalValues[$namespace]=@($scopedValues[$namespace])};$globalValues['occurrence.rule-effect-same-target-incompatibility-pair']=@('advance-iteration-with-terminate-recurrence');$globalValues['occurrence.rule-effect-global-incompatibility-pair']=@('advance-iteration-with-pause-recurrence');Assert-SchemaPackOccurrenceSemanticDeclarations $globalValues;$globalPacks=$packs.PSObject.Copy();$globalPacks.controlled_values=$globalValues
$globalEvaluation=Get-KnowledgeRecurrenceRuleEvaluation (ConvertTo-KnowledgeOccurrenceRegistry $scopedProbe $fixturePath $globalPacks $chronologyFixture $subjectTargets $payloadTargets) 'outer-loop' 'reset-two';Assert-OccurrenceIds @($globalEvaluation.conflicts) @('advance-iteration conflicts with pause-recurrence globally') 'Global effect incompatibility'
...
Assert-InvalidOccurrenceDeclarations $packs 'occurrence.rule-effect-repetition-policy' (@($packs.controlled_values['occurrence.rule-effect-repetition-policy'])+@('pause-recurrence-uses-accumulating')) 'ambiguous repetition'
Assert-InvalidOccurrenceDeclarations $packs 'occurrence.rule-effect-repetition-policy' @($packs.controlled_values['occurrence.rule-effect-repetition-policy']|Where-Object {$_ -cne 'pause-recurrence-uses-idempotent'}) 'missing repetition'
$duplicateScope=[ordered]@{};foreach($namespace in $packs.controlled_values.Keys){$duplicateScope[$namespace]=@($packs.controlled_values[$namespace])};$duplicateScope['occurrence.rule-effect-global-incompatibility-pair']=@('advance-iteration-with-pause-recurrence');$scopeRejected=$false;try{Assert-SchemaPackOccurrenceSemanticDeclarations $duplicateScope}catch{$scopeRejected=$true};if(-not $scopeRejected){throw 'Effect incompatibility pair unexpectedly accepted two scopes.'}
