...
foreach ($property in $expectations.state_scales.PSObject.Properties) {
    $scale = $fixture.state_scales[$property.Name]
    $expected = @($property.Value)
    if (
        $scale.kind -cne $expected[0] -or
        (@($scale.levels.id) -join '|') -cne (@($expected[1]) -join '|') -or
        $scale.minimum -ne $expected[2] -or
        $scale.maximum -ne $expected[3] -or
        $scale.unit -cne $expected[4]
    ) {
        throw "Unexpected state scale '$($property.Name)'."
    }
}
...
foreach ($property in $expectations.state_snapshots.PSObject.Properties) {
    ...
    $actual = @(
        ...
        $(if ($null -eq $transition.prior_capability) {
                $null
            }
            else {
                "$($transition.prior_capability.scale_id):$($transition.prior_capability.value)"
            })
        $(if ($null -eq $transition.resulting_capability) {
                $null
            }
            else {
                "$($transition.resulting_capability.scale_id):$($transition.resulting_capability.value)"
            })
    )
...
$summary = [ordered]@{
    ...
    state_scales=[int]$registry.state_scales.Count
