...
function ConvertTo-NormalizedResolution([object]$Resolution) {
    ...
    foreach ($branch in @($Resolution.branches)) {
        ...
        if ($null -ne $branch.canonical_target) {
            $target = "$($branch.canonical_target.target_type):$($branch.canonical_target.target_id)"
        }
        $branchList.Add(@([string]$branch.outcome, $target, @($branch.reconciliation_ids)))
    }
    return [ordered]@{target_type=$Resolution.requested_type
...
foreach ($name in $invalidMappingKeyFixtures) {
    $rejected = $false
    try {
        $null = ConvertFrom-KnowledgeYamlFile (Join-Path $strictYamlFixtures $name) 1 "Strict YAML fixture"
    }
    catch {
        $rejected = $true
    }
    if (-not $rejected) {
        throw "Noncanonical mapping-key fixture was accepted: $name"
    }
}
...
if ([string]$scalarRegistry.records[0].source_label -cne "on") {
    throw "Legacy YAML Boolean word did not remain a string."
}
...
foreach ($case in @($expectations.resolutions)) {
    ...
    if ($actualJson -ne $expectedJson) {
        throw "Reconciliation resolution vector failed for $($case.target_type):$($case.target_id).`nExpected: $expectedJson`nActual:   $actualJson"
    }
}

$invalidReconciliationFixtures = @(
...
foreach ($name in $invalidReconciliationFixtures) {
    $rejected = $false
    try {
        $null = Get-TestRegistry (Join-Path $fixtures $name)
    }
    catch {
        $rejected = $true
    }
    if (-not $rejected) {
        throw "Malformed reconciliation fixture was accepted: $name"
    }
}
...
$limitRejected = $false
try {
    $null = Resolve-KnowledgeReconciliationTarget $limited "category" "branch-limit-source"
}
catch {
    $limitRejected = $true
}
if (-not $limitRejected) {
    throw "Reconciliation branch limit did not stop expansion."
}

$stepLimited = Get-TestRegistry (Join-Path $fixtures "resolution-step-limit.yaml")
$stepRejected = $false
try {
    $null = Resolve-KnowledgeReconciliationTarget $stepLimited "category" "step-limit-a"
}
catch {
    $stepRejected = $true
}
if (-not $stepRejected) {
    throw "Reconciliation step limit did not stop traversal."
}
...
for ($i = 0; $i -lt $DeepChain; $i++) {
    ...
    if ($i + 1 -lt $DeepChain) {
        $targetId = "deep-{0:D4}" -f ($i + 1)
    }
    else {
        $targetId = "current-category"
    }
    $records.Add([ordered]@{
            id="deep-record-{0:D4}" -f $i
            source_type="category"
            source_id=$sourceId
            source_state="tombstone"
            source_label_mode="omitted"
            operation="redirect"
            targets=@([ordered]@{target_type="category"
                    target_id=$targetId
                })
            reason="renamed"
            status="active"
            audit=[ordered]@{mode = "repository-history" }
        })
}
...
try {
    foreach ($case in @(
            [pscustomobject]@{Name="invalid UTF-8"
                Bytes=[byte[]](0x73, 0x63, 0x68, 0x65, 0x6D, 0x61, 0x5F, 0x76, 0x65, 0x72, 0x73, 0x69, 0x6F, 0x6E, 0x3A, 0x20, 0x34, 0x0A, 0xFF)
            },
            [pscustomobject]@{Name="UTF-8 BOM"
                Bytes=[byte[]](@(0xEF, 0xBB, 0xBF) + [System.IO.File]::ReadAllBytes((Join-Path $fixtures "valid-v4.yaml")))
            }
        )) {
        [System.IO.File]::WriteAllBytes($byteTestPath, $case.Bytes)
        $rejected = $false
        try {
            $null = Get-TestRegistry $byteTestPath
        }
        catch {
            $rejected = $true
        }
        if (-not $rejected) {
            throw "Byte-level YAML fixture was accepted: $($case.Name)"
        }
    }
    $emoji = [char]::ConvertFromUtf32(0x1F600)
    $rejected = $false
    try {
        Assert-KnowledgeYamlSource "value: $emoji$emoji`n" "Test registry" $byteTestPath -MaxScalarBytes 7
    }
    catch {
        $rejected = $true
    }
    if (-not $rejected) {
        throw "UTF-8 scalar-byte budget did not reject two emoji."
    }
    $rejected = $false
    try {
        Assert-KnowledgeYamlSource "value: 12`n" "Test registry" $byteTestPath -MaxBytes 9
    }
    catch {
        $rejected = $true
    }
    if (-not $rejected) {
        throw "UTF-8 file-byte budget was not enforced."
    }

    [System.IO.File]::WriteAllText($tempPath, ([ordered]@{schema_version=4
                resolution=[ordered]@{max_branches=65536
                    max_records=[Math]::Max($DeepChain + 1, 100000)
                    max_targets_per_record=4096
                    max_resolution_steps=[Math]::Max($DeepChain + 1, 250000)
                }
    ...
    if ($result.outcome -ne "redirected" -or @($result.reconciliation_ids).Count -ne $DeepChain) {
        throw "Deep reconciliation chain did not resolve completely."
    }
}
finally {
    ...
    if (Test-Path -LiteralPath $byteTestPath) {
        Remove-Item -LiteralPath $byteTestPath -Force
    }
}
...
if ($Json) {
    ...
    Write-Output (
        (
            'Reconciliation conformance passed: {0} vectors, {1} malformed reconciliation fixtures, ' +
            '{2} malformed mapping-key fixtures, byte/scalar/key parity, branch and step limits, ' +
            '{3}-hop chain.'
        ) -f
        $summary.resolution_vectors,
        $summary.malformed_reconciliation_fixtures,
        $summary.malformed_mapping_key_fixtures,
        $summary.deep_chain_hops
    )
}
