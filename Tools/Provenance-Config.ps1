...
$script:SupportedProvenanceSchemaVersion = 1
...
function ConvertTo-ProvenanceLocator {
  ...
  if($source.locator_medium_ids -notcontains $mediumId){throw "Provenance registry '$Context.medium_id' is not allowed by the evidence source."}
  $evidenceMode=Get-RequiredSourceString $Raw "evidence_mode" $Context
  if($source.evidence_modes -notcontains $evidenceMode){throw "Provenance registry '$Context.evidence_mode' is not declared by the evidence source."}
...
function Get-KnowledgeProvenanceRegistry {
  param([object]$ProjectConfig,[object]$SourceRegistry,[object]$EntityRegistry,[object]$SchemaPackRegistry)
  ...
  $registry=ConvertFrom-Yaml -Yaml ([System.IO.File]::ReadAllText($path,[System.Text.UTF8Encoding]::new($true))) -Ordered
  ...
  if([int]$schemaVersion -ne $script:SupportedProvenanceSchemaVersion){throw "Unsupported provenance schema_version '$schemaVersion'; expected $($script:SupportedProvenanceSchemaVersion)."}
  $sourceSubjectTypes=@(Get-KnowledgeSourceProvenanceSubjectTypes);$entitySubjectTypes=@(Get-KnowledgeEntityProvenanceSubjectTypes);$duplicates=@($sourceSubjectTypes|Where-Object {$entitySubjectTypes -contains $_}|Sort-Object -Unique)
  if($duplicates.Count -gt 0){throw "Provenance subject types have multiple providers: $($duplicates -join ', ')."}
  $providedSubjectTypes=@($sourceSubjectTypes+$entitySubjectTypes+@("claim-supersession")|Sort-Object -Unique);$allowedSubjectTypes=@(Get-SchemaPackAllowedValues $SchemaPackRegistry "provenance.subject-type")
  $missingProviders=@($allowedSubjectTypes|Where-Object {$providedSubjectTypes -notcontains $_});$unregisteredProviders=@($providedSubjectTypes|Where-Object {$allowedSubjectTypes -notcontains $_})
  ...
  $shell=[pscustomobject]@{sources=$SourceRegistry;entities=$EntityRegistry;claim_supersessions=@($supersessions)}
  for($i=0;$i -lt $rawAssertions.Count;$i++){
    ...
    $fieldPath=Get-OptionalSourceString $item "field_path" $context;if($null -ne $fieldPath){if($fieldPath -notmatch $script:ProvenanceFieldPathPattern){throw "Provenance registry '$context.field_path' must be a dotted/indexed machine field path."};$null=Resolve-ProvenanceRecordFieldPath $target $fieldPath "$context.field_path"}
    ...
    $roles=@($links|ForEach-Object {$_.evidence_role}|Sort-Object -Unique);if($status -in @("verified","inferred") -and $roles -notcontains "supports"){throw "Provenance registry '$context' status '$status' requires supporting evidence."};if($status -eq "disputed" -and ($roles -notcontains "supports" -or $roles -notcontains "contradicts")){throw "Provenance registry '$context' disputed status requires supporting and contradicting evidence."}
    $assertions+=,[pscustomobject]@{id=$id;claim_key=$claimKey;subject_type=$subjectType;subject_id=$subjectId;claim_namespace=$namespace;field_path=$fieldPath;asserted_value=$assertedValue;assertion_status=$status;observed_at=ConvertTo-SourceTemporalWindow $item "observed_at" $context $SchemaPackRegistry;effective_window=ConvertTo-SourceTemporalWindow $item "effective_window" $context $SchemaPackRegistry;evidence_links=@($links)}
  }
  ...
  return [pscustomobject]@{path=$path;schema_version=[int]$schemaVersion;assertions=@($assertions);claim_supersessions=@($supersessions);sources=$SourceRegistry;entities=$EntityRegistry}
}
...
function Get-KnowledgeProvenanceApplicabilityDecision {
  ...
  foreach($scope in $ProvenanceRegistry.sources.applicability_scopes.Values){
    ...
    $territoryMatch=Get-KnowledgeApplicabilityTerritoryMatch $ProvenanceRegistry.sources $scope $TerritoryId;if($null -eq $territoryMatch){continue};$temporalMatch=Get-KnowledgeApplicabilityTemporalMatch $scope.effective_window $effectiveInstant;if($null -eq $temporalMatch){continue};$matches+=,[pscustomobject]@{scope_id=$scope.id;outcome=if($temporalMatch -eq "unknown"){"indeterminate"}else{"applicable"};target_match=$targetMatch;territory_match=$territoryMatch;temporal_match=$temporalMatch;precedence=[int]$scope.precedence}
  }
