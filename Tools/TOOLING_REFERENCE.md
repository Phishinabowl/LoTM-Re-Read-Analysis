...
Last parity check: 2026-07-31. Python, PowerShell 7, and Windows PowerShell 5.1 loaded manifest schema 3, schema-pack registry schema 2, two selected schema packs at pack schema 2, taxonomy schema 2, resource schema 1, and source schema 2 with five matching content roots, twelve resource roots, 16 active categories, two deferred categories, six active content types, six resource kinds, thirteen resource types, fifteen resource placements, three media, four work-group types, three work groups, two continuities, one continuity relationship, two authority profiles, six works, eight verified Book 1 volume ranges, five work relationships, eight evidence sources, fourteen source aliases, two source relationships, four source-resource bindings, and twelve work aliases. All runtimes resolved 25 declared capabilities, 24 available capabilities, and 24 explicitly enabled capabilities. `migration-services` matched as declared, planned, unavailable, and disabled; `graph-projection` matched as declared, available, and enabled. The 99 normalized capability and controlled-value records produced identical semantic hashes. Both QA exporters generated matching 28-file lists and summary counts; after stable-output normalization, only the three expected runtime-specific refresh artifacts differed.
