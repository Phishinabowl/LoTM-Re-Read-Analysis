...
| `Framework/Contracts/README.md` and `Framework/Contracts/narrative-source-registry.md` | Framework contract index and narrative source contract | Framework maintainers and future schema tooling | Framework maintainers | Record the executable configuration boundary, schema ownership, media axes, work segments, named ordering schemes, and adaptation mappings. | A configuration contract is introduced, stabilized, moved, or assigned a validator. |
| `Framework/Packs/core/pack.yaml` | Core schema pack | Schema-pack loaders and future validation/editor/wizard services | Framework maintainers | Declares domain-neutral platform capabilities plus controlled evidence-source roles and evidence-artifact relationships. | A reusable core capability or evidence primitive changes. |
...
| `Project_Config/sources.yaml` | Source registry | `Tools/source_config.py`, `Tools/Source-Config.ps1`, and future validation, content-index, visualization, editor, and migration services | Maintainers through reviewed edits; future source editors through the mutation service | Instantiates media facets, works, structural segments and overlapping groups, numbering/orderings, multi-input adaptation mappings, segment-scoped manifestations and version mappings, package-scoped components, release packages/runs/events, territories, localized platform catalogs/offerings, structured time, external identifiers, composite evidence sources, relationships, and resource bindings. | A media facet, work, segment/group, numbering/ordering, manifestation/mapping, component/package/run/release, territory/platform record, identifier, source, or binding changes. |
...
Last parity check: 2026-07-31. Python, PowerShell 7, and Windows PowerShell 5.1 loaded manifest schema 3, schema-pack registry schema 2, six selected packs (`core` 2, `narrative-media` 5, `narrative-publishing` 3, `narrative-screen-audio` 4, `narrative-adaptation` 3, and `narrative-distribution` 3), taxonomy schema 2, resource schema 1, and source schema 6. The current LoTM source registry matched at three media modalities, one cultural form, three release forms, four container formats, three medium profiles, four work-group types, three work groups, two continuities, one continuity relationship, two authority profiles, six works, zero registered segments, segment groups, or numbering schemes, two total ordering schemes, eight verified Book 1 volume ranges, five work relationships, zero adaptation mappings, six manifestations, zero manifestation relationships or segment mappings, two manifestation-scoped release components, and zero release packages, runs, territories, platforms, release events, catalog placements, platform offerings, identifier schemes, or external identifiers. It also matched at eight single-work evidence sources using the composite `work_ids` contract, fourteen source aliases, two source relationships, four source-resource bindings, and twelve work aliases. A populated v6 fixture passed all runtimes with two overlapping segment groups, one manifestation mapping, one package-only poster, a two-work package/source, one weekly release run with an exception, one concrete event, one localized segment-group catalog placement, one offering, and one external run identifier. Negative fixtures consistently rejected an orphaned package component and cyclic segment groups. All runtimes resolved 66 declared capabilities, 57 available capabilities, 57 explicitly enabled capabilities, and 429 controlled-value ownership records. The nine planned unavailable capabilities are `editorial-assembly`, `entity-incarnations`, `live-performance-productions`, `migration-services`, `performance-events`, `publication-runs`, `retcon-supersession`, `scoped-continuity`, and `textual-witnesses`. Both QA exporters generated matching 33-file lists and summary counts with one bounded graph and page; stable outputs matched after timestamps, output paths, and line endings were normalized, leaving only the four expected runtime-owned settings/snapshot artifacts.
...
`Project_Config/sources.yaml` separates media facets, work groups, continuities, creative works, work segments, named orderings, adaptation mappings, manifestations, release records, platform catalogs/offerings, and evidence sources. A medium is specifically a reader-position/citation profile and references one or more modalities plus compatible cultural forms. Works separately declare release form and lifecycle status; sources separately declare their concrete container formats. This allows a Donghua film or EPUB-contained illustration to retain every relevant axis without compound medium IDs. Work groups may represent franchises, ordered series, heterogeneous adaptation programs, or unordered collections and may nest beneath a parent group. A work defines stable identity, group and continuity memberships, display aliases, numbering mode, release form, status, and a verified, pending, or inapplicable volume catalog.

Segments provide stable configurable parts beneath a work. Their single parent owns structural containment; overlapping `segment_groups` separately collect segments across works into arcs, sagas, crossovers, publication units, collections, or reading paths and may use an exact ordering scheme. Numbering schemes assign display numbers and aliases without implying sequence, while named total or partial ordering schemes can order works or segments by release, publication, story, production, or recommendation without overwriting another order. Partial schemes use acyclic predecessor references and allow concurrency. Adaptation mappings connect one or more basis works and optional registered segments to a target using controlled correspondence and basis roles. Manifestations may cover a whole work or selected segments, and mappings between related manifestations describe retained, omitted, added, altered, replaced, reordered, combined, or split material. Release components may originate in a manifestation or exist through package membership; commercial packages, compact release runs, concrete events, and platform offerings own bundled material, schedules, launches, segment scope, and structured availability. Catalog placements target works, segments, segment groups, manifestations, or packages and carry localized provider titles without redefining canonical hierarchy. Hierarchical territories and external identifier schemes preserve regional and provider identity separately from internal stable IDs. A source selects one or more works and a medium profile, may bind to the manifestation, package, event, components, or offering it observes, and defines a concrete evidence role, containers, comparison group, priority, aliases, evidence modes, and optional resource bindings.
...
Authority profiles define accepted continuity memberships, continuity order, source-priority direction, and which derivative-work relationships are comparable. Reader positions use canonical `book` work IDs before work-local volume and chapter values. Under the default `lotm-adaptation-comparison` profile, lower source priorities have higher authority, the novel continuity precedes the Donghua continuity, source-scoped claims are preserved, and a disagreement is assigned to the derivative work rather than mutating the original.
...
The paired source loaders reject malformed IDs, invalid position/citation definitions, unknown media-facet/group/continuity/profile/work/segment/medium/container/resource/source references, incompatible cultural forms and modalities, invalid structural or overlapping group, territory, or catalog nesting and cycles, duplicate numbering or ordering targets, labels, or ordinals, missing or duplicate ordered-group ordinals, malformed or non-contiguous verified volume catalogs, unsupported roles/statuses/policies/mapping types, invalid manifestation mapping shapes, orphaned package-only components, malformed release-run cadence/exceptions, out-of-scope manifestation/package/run/event/offering segments, invalid structured times, composite sources outside their release scope, duplicate external identifiers, duplicate aliases, malformed inverse-type definitions, invalid or self-referential typed relationships, escaping or out-of-placement resource paths, and missing required bindings.
