...
Last parity check: 2026-08-05 for V47 implementation. The `baseline` profile passed all sixteen registered suites in Python, PowerShell 7, and Windows PowerShell 5.1 with matching suite IDs, statuses, and canonicalized semantic summaries. Individually measured runtimes were 43.4, 198.2, and 357.7 seconds. The baseline verifies the reviewed project identity, eight selected packs, 123 enabled and nine disabled capabilities, 25 reconciliation target types, 69 provenance subject types, exact registry schemas/counts and typed semantic declarations including four state profiles and 18 state-kind mappings, provider and object closure, deterministic repeated loads, disabled-capability absence, and fourteen invalid cross-registry compositions. Hosting conformance includes nine service assertions, 31 malformed registries, seven invalid queries, and generated 128-record carrier and occupancy probes. The `full-release` compatibility profile passed all six checks in 176.9 seconds, preserving 15 visualization nodes, 121 relationships, 16 QA notes, 34 normalized QA files, 12 root launches, artifact safety, 224-file isolated framework extraction, and identical nonblank rendering across runtimes. Source, entity, provenance, hosting, interpretation, and project composition remain outside the eight-suite `fast` profile because their repeated dependency composition is materially expensive.
