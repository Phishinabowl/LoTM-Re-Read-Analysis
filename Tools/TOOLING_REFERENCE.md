...
| `Project_Config/occurrences.yaml` | Occurrence and recurrence registry | `Tools/occurrence_config.py`, `Tools/Occurrence-Config.ps1`, provenance, paired occurrence conformance tools, and future content-index, visualization, and editor services | Maintainers through reviewed edits; future occurrence editors through the mutation service | Instantiates branches, templates, recurrences, iterations, concrete occurrences, chronology bindings, perspective tracks, transitions, causal relations, and carryover without encoding cycles in chronology. | A branch, occurrence identity, recurrence/iteration, coordinate binding, track, transition, causal relationship, or carryover changes. |
...
Last parity check: 2026-08-01. Python, PowerShell 7, and Windows PowerShell 5.1 loaded manifest schema 9, schema-pack registry schema 2, eight selected packs (`core` 22, `narrative-media` 17, `narrative-publishing` 10, `narrative-screen-audio` 11, `narrative-adaptation` 10, `narrative-distribution` 14, `narrative-production` 4, and `narrative-shared-universe` 9), taxonomy schema 2, resource schema 1, source schema 18, entity schema 4, reconciliation schema 4, provenance schema 3, chronology schema 1, occurrence schema 1, and lookup-key schema 1 with Unicode 16.0.0. V31 closes combined exact-order and transitive-equivalence defects, then adds paired occurrence/recurrence services while leaving chronology acyclic. The LoTM occurrence instance contains only `main`; no loop data is fabricated. Thirteen chronology comparisons, thirteen malformed chronology registries, ten occurrence query groups, and eleven malformed occurrence cases pass identically in all three runtimes alongside the permanent temporal, reconciliation, strict-ingestion, and lookup baselines.
...
`Project_Config/occurrences.yaml` keeps a concrete occurrence distinct from its repeatable template and chronology coordinates. Branch and recurrence parentage is acyclic; iteration ordinals are unique; nested iterations identify their parent iteration; reset and carryover records advance within one recurrence; tracks explicitly preserve subject experience or observation; and causal edges may cycle without entering chronology comparison. Repeated occurrences may bind the same world position. The paired query services answer iteration membership, coordinate reuse, previous/next track experience, incoming carryover, and occurrence recurrence identity. Every stable record shape is exposed to centralized provenance.
