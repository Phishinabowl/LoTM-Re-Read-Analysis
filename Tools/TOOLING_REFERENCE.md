...
| Select an entity's incarnations and an incarnation's bindings/relationships | `EntityRegistry.incarnations_for_entity`, `bindings_for_incarnation`, `relationships_for_incarnation` | `Get-KnowledgeEntityIncarnations`, `Get-KnowledgeIncarnationBindings`, `Get-KnowledgeIncarnationRelationships` |
...
Last parity check: 2026-07-31. Python, PowerShell 7, and Windows PowerShell 5.1 loaded manifest schema 4, schema-pack registry schema 2, eight selected packs (`core` 10, `narrative-media` 14, `narrative-publishing` 10, `narrative-screen-audio` 11, `narrative-adaptation` 10, `narrative-distribution` 12, `narrative-production` 4, and `narrative-shared-universe` 4), taxonomy schema 2, resource schema 1, source schema 15, and entity schema 1. All ten available pack files parsed successfully. All runtimes resolved 107 declared capabilities, 98 available capabilities, 98 explicitly enabled capabilities, and 610 controlled-value ownership records. The nine planned unavailable capabilities are `contributor-credits`, `crossover-events`, `editorial-assembly`, `live-performance-productions`, `migration-services`, `performance-events`, `publication-runs`, `rights-grants-and-restrictions`, and `textual-witnesses`. The empty LoTM entity registry matched at zero entities, zero incarnations, zero bindings/relationships, and eight relationship types. A populated V16 fixture matched for category/continuity validation, aliases, two incarnations, a scope-backed binding, an adaptation-counterpart relationship, and typed lookup APIs. Both QA exporters generated matching 33-file lists and summary counts with one bounded graph and page; all 27 stable outputs matched after timestamps, output paths, byte-order marks, and line endings were normalized, leaving only the six expected runtime-owned refresh/bounded report, settings, and snapshot artifacts. Existing V15 source-registry applicability and V14 storage fixture coverage remains the regression baseline for semantic scope, authority, scoped lineage/continuity, production contexts, and claim supersession.
...
`Project_Config/entities.yaml` separates continuity-independent conceptual entities from continuity-bound incarnations. Entities reference taxonomy categories. Incarnations reference one entity, one primary continuity, and one or more status-bearing continuity memberships. Scope-backed bindings identify where an incarnation is primary, appears, or crosses into registered material without duplicating source-registry target, territory, time, or precedence fields. Typed incarnation relationships preserve counterpart, branch, reboot, and derivation semantics with explicit reciprocal inverses.

The paired entity loaders require the selected and enabled `entity-incarnations` capability and consume normalized schema-pack, taxonomy, and source registries. They expose entity/incarnation alias resolution, entity-to-incarnation and incarnation-to-binding/relationship queries, plus stable provenance-target lookup for entities, incarnations, bindings, and relationships. The central provenance service does not yet accept those cross-registry targets, so evidence assertions remain out of `entities.yaml`. The current LoTM registry deliberately contains no entities or incarnations and eight relationship-type definitions; page linkage and existing-character migration remain later mutation-service work.

The loaders reject unsupported schema/capability state, malformed IDs, unknown categories/entities/continuities/scopes, duplicate aliases, missing primary memberships, unsupported statuses or binding/relationship types, inconsistent inverse definitions, unknown endpoints, self-relationships, and duplicate bindings or relationships.
