...
| `Project_Config/sources.yaml` | Source registry | `Tools/source_config.py`, `Tools/Source-Config.ps1`, and future validation, content-index, visualization, editor, and migration services | Maintainers through reviewed edits; future source editors through the mutation service | Instantiates media facets, works, structural segments and recursively nested content groups, numbering/orderings, multi-input adaptation mappings, segment-scoped manifestations and version mappings, package-scoped components and lineage, release packages/phased runs/events, territories, temporally scoped localized titles and platform catalogs/offerings, structured time, external identifiers, multi-target source observations, position-ranged coverage, evidence-located assertion snapshots, relationships, and resource bindings. | A media facet, work, content group, nested record, numbering/ordering, manifestation/mapping, component/package/run/release, territory/platform record, identifier, observation, source coverage, provenance assertion, or binding changes. |
...
Last parity check: 2026-07-31. Python, PowerShell 7, and Windows PowerShell 5.1 loaded manifest schema 3, schema-pack registry schema 2, six selected packs (`core` 4, `narrative-media` 7, `narrative-publishing` 3, `narrative-screen-audio` 4, `narrative-adaptation` 3, and `narrative-distribution` 5), taxonomy schema 2, resource schema 1, and source schema 8. The current LoTM source registry matched at three media modalities, one cultural form, three release forms, four container formats, three medium profiles, four work-group types, three work groups, two continuities, one continuity relationship, two authority profiles, six works, zero registered segments, content groups, or numbering schemes, two total ordering schemes, eight verified Book 1 volume ranges, five work relationships, zero adaptation mappings, six manifestations, zero manifestation relationships or segment mappings, two manifestation-scoped release components, and zero component relationships, release packages, runs, territories, platforms, release events, catalog placements, platform offerings, identifier schemes, external identifiers, observations, coverage entries, or provenance assertions. It also matched at eight single-work evidence sources, fourteen source aliases, two source relationships, four source-resource bindings, and twelve work aliases. A populated v8 fixture passed all runtimes with recursively nested content groups and globally stable member IDs, a validity-bounded localized title, two heterogeneous source observations, position-ranged group coverage, and a value-snapshot provenance assertion with observation/effective timing and an exact evidence locator. Negative fixtures consistently rejected nested group cycles, unknown locator fields, verified assertions without support, and duplicate nested IDs. All runtimes resolved 78 declared capabilities, 69 available capabilities, 69 explicitly enabled capabilities, and 484 controlled-value ownership records. The nine planned unavailable capabilities are `editorial-assembly`, `entity-incarnations`, `live-performance-productions`, `migration-services`, `performance-events`, `publication-runs`, `retcon-supersession`, `scoped-continuity`, and `textual-witnesses`. Both QA exporters generated matching 33-file lists and summary counts with one bounded graph and page; all 29 stable outputs matched after timestamps, output paths, and line endings were normalized, leaving only the four expected runtime-owned settings/snapshot artifacts.
