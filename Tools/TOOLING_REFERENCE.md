...
| `Framework/Data/unicode-lookup-16.0.0.json`, `Framework/Data/lookup-key-regression-vectors.json`, `Framework/Data/Strict-Yaml/`, and `Framework/Data/Reconciliation/` | Pinned framework runtime and conformance data | Strict YAML and lookup loaders, reconciliation loaders, paired conformance tools, parity checks, and future semantic identity consumers | Framework maintainers through reviewed data updates | Define deterministic lookup behavior, canonical mapping-key ingestion, and valid/malformed branch-aware stable-ID reconciliation vectors shared by every runtime. | The ingestion or lookup algorithm, pinned Unicode version, reconciliation result contract, or a discovered portability/identity edge case changes. |
...
Last parity check: 2026-08-01. Python, PowerShell 7, and Windows PowerShell 5.1 loaded manifest schema 7, schema-pack registry schema 2, eight selected packs (`core` 18, `narrative-media` 15, `narrative-publishing` 10, `narrative-screen-audio` 11, `narrative-adaptation` 10, `narrative-distribution` 13, `narrative-production` 4, and `narrative-shared-universe` 9), taxonomy schema 2, resource schema 1, source schema 16, entity schema 4, reconciliation schema 4, provenance schema 1, and lookup-key schema 1 with Unicode 16.0.0. V27 validates mapping keys before runtime-native dictionaries exist: only non-empty lowercase machine strings are accepted, ambiguous textual keys must be quoted, and case-colliding, non-string, complex, Unicode, punctuation-shaped, empty, or duplicate keys are rejected. Nine malformed mapping-key fixtures and one valid string-preservation fixture join V26 byte/scalar probes, 40 malformed reconciliation fixtures, branch/step limits, and the 1,500-hop chain. V25 closed registry shapes and V24 strict-ingestion, V23 branch-aware identity, V22 provider/provenance, V21 identity-phase, V20 provenance, V19 lookup, V18 entity-lineage, V15 source-applicability, and V14 storage fixtures remain regression baselines.
