...
Last focused parity check: 2026-08-05. Python, PowerShell 7, and Windows PowerShell 5.1 load manifest schema 9, schema-pack selection registry schema 2, schema-3 pack files, eight selected packs (`core` 33, `narrative-media` 23, `narrative-publishing` 10, `narrative-screen-audio` 11, `narrative-adaptation` 10, `narrative-distribution` 14, `narrative-production` 4, and `narrative-shared-universe` 9), taxonomy schema 2, resource schema 1, source schema 18, entity schema 4, reconciliation schema 4, provenance schema 3, chronology schema 2, occurrence schema 7, and lookup-key schema 1 with Unicode 16.0.0. V42 adds provenance-addressable append-only branch lifecycle, source-continuity membership closure, and branch-local state queries while preserving chronology and authority ownership boundaries. The current composition contains 126 atomic controlled-value namespaces, 972 values, 119 enabled capabilities, and 60 provenance subject types. The LoTM occurrence instance still contains only `main`; no loop, branch-state claim, participation, topology relation, or repetition count is fabricated. Focused chronology conformance passes 14 comparisons, 11 topology queries, 17 malformed fixtures, and a generated 128-relation cyclic topology; occurrence conformance passes 97 query/evaluation assertions, 116 malformed cases, and generated 128-branch-state, 128-cardinality, and 128-participation/track-entry probes with matching summaries across runtimes.
...
Last parity check: 2026-08-05 for V42 implementation confirmation. The `baseline` profile passed all fourteen registered suites in Python, PowerShell 7, and Windows PowerShell 5.1 with matching suite IDs, statuses, and canonicalized semantic summaries. Concurrently measured local runtimes were 40.4 seconds, 164.3 seconds, and 275.2 seconds respectively. The baseline verifies the reviewed project identity, eight selected packs, 119 enabled and nine disabled capabilities, 24 reconciliation target types, 60 provenance subject types, exact registry schemas/counts and typed semantic declarations, provider and object closure, deterministic repeated loads, disabled-capability absence, and ten invalid cross-registry compositions. Chronology conformance includes 14 comparison vectors, 11 context-topology queries covering all six core relation types, 17 malformed registries, typed-target and provenance closure, and a generated 128-relation topology cycle. Occurrence conformance includes 97 query/evaluation assertions, 116 malformed cases, branch-continuity composition closure, and 128-record branch-state, cardinality, and participation/track-entry scale probes. Source, entity, provenance, and project composition remain outside the eight-suite `fast` profile because their repeated dependency composition is materially expensive.
