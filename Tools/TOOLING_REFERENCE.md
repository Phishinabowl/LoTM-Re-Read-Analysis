...
| `Project_Config/occurrences.yaml` | Occurrence and recurrence registry | `Tools/occurrence_config.py`, `Tools/Occurrence-Config.ps1`, provenance, paired occurrence conformance tools, and future content-index, visualization, and editor services | Maintainers through reviewed edits; future occurrence editors through the mutation service | Instantiates branches, templates, recurrences, iterations, concrete occurrences, coherent chronology bindings, perspective tracks, profiled transitions, causal relations, and payload-bearing carryover without encoding cycles in chronology. | A branch, occurrence identity, recurrence/iteration, coordinate binding, track, transition profile, causal relationship, or carryover payload changes. |
...
| Query iteration contents, iteration contents in track order, iteration boundaries, coordinate reuse, track neighbors, carryover, and recurrence identity | `OccurrenceRegistry.occurrences_for_iteration`, `occurrences_for_iteration_on_track`, `previous_before_iteration`, `next_after_iteration`, `occurrences_at_position`, `previous_on_track`, `next_on_track`, `carryovers_into`, `recurrence_for_occurrence` | `Get-KnowledgeOccurrencesForIteration`, `Get-KnowledgeOccurrencesForIterationOnTrack`, `Get-KnowledgePreviousBeforeIteration`, `Get-KnowledgeNextAfterIteration`, `Get-KnowledgeOccurrencesAtPosition`, `Get-KnowledgePreviousTrackOccurrence`, `Get-KnowledgeNextTrackOccurrence`, `Get-KnowledgeCarryoversIntoIteration`, `Get-KnowledgeOccurrenceRecurrence` |
...
Last parity check: 2026-08-01. Python, PowerShell 7, and Windows PowerShell 5.1 loaded manifest schema 9, schema-pack registry schema 2, eight selected packs (`core` 23, `narrative-media` 18, `narrative-publishing` 10, `narrative-screen-audio` 11, `narrative-adaptation` 10, `narrative-distribution` 14, `narrative-production` 4, and `narrative-shared-universe` 9), taxonomy schema 2, resource schema 1, source schema 18, entity schema 4, reconciliation schema 4, provenance schema 3, chronology schema 1, occurrence schema 2, and lookup-key schema 1 with Unicode 16.0.0. V32 preserves acyclic chronology while adding coherent primary bindings, pack-registered core transition profiles, forward track semantics, explicit branch-fork lineage, payload-bearing carryover, semantic duplicate rejection, and iteration-plus-track boundary queries. The LoTM occurrence instance contains only `main`; no loop data is fabricated. Thirteen chronology comparisons, thirteen malformed chronology registries, eighteen occurrence query assertions, and twenty-seven malformed occurrence cases pass identically in all three runtimes alongside the permanent temporal, reconciliation, strict-ingestion, and lookup baselines.
...
`Project_Config/occurrences.yaml` keeps a concrete occurrence distinct from its repeatable template and chronology coordinates. Branch and recurrence parentage is acyclic; iteration ordinals are unique; nested iterations identify their parent iteration; primary bindings cannot contradict exact chronology; transition profiles enforce track, recurrence, and branch semantics; carryover advances between track-participating iterations and names an exact payload target; and causal edges may cycle without entering chronology comparison. Repeated occurrences may bind the same world position. The paired query services answer iteration membership, track-ordered iteration membership and boundaries, coordinate reuse, previous/next track experience, incoming carryover, and occurrence recurrence identity. Every stable record shape is exposed to centralized provenance.
