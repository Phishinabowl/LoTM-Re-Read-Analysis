...
Last parity check: 2026-08-05 for V41 implementation closure. The `baseline` profile passed all fourteen registered suites in Python, PowerShell 7, and Windows PowerShell 5.1 with matching suite IDs, statuses, and canonicalized semantic summaries. Measured local runtimes were 42.4 seconds, 165.5 seconds, and 276.9 seconds respectively. The baseline verifies the reviewed project identity, eight selected packs, 118 enabled and nine disabled capabilities, 24 reconciliation target types, 59 provenance subject types, exact registry schemas/counts and typed semantic declarations, provider and object closure, deterministic repeated loads, disabled-capability absence, and nine invalid cross-registry compositions. Chronology conformance includes 14 comparison vectors, five context-topology queries, 17 malformed registries, typed-target and provenance closure, and a generated 128-relation topology cycle; occurrence conformance retains 87 query/evaluation assertions, 102 malformed cases, and its cardinality and participation scale probes. Source, entity, provenance, and project composition remain outside the eight-suite `fast` profile because their repeated dependency composition is materially expensive.
