...
Last parity check: 2026-08-01. Python, PowerShell 7, and Windows PowerShell 5.1 loaded manifest schema 7, schema-pack registry schema 2, eight selected packs (`core` 17, `narrative-media` 15, `narrative-publishing` 10, `narrative-screen-audio` 11, `narrative-adaptation` 10, `narrative-distribution` 13, `narrative-production` 4, and `narrative-shared-universe` 9), taxonomy schema 2, resource schema 1, source schema 16, entity schema 4, reconciliation schema 4, provenance schema 1, and lookup-key schema 1 with Unicode 16.0.0. V26 makes registry ingestion byte-first: strict UTF-8 without BOM, UTF-8 byte budgets for files and scalars, explicit lowercase nulls, and canonical integers as the sole unquoted numeric form. Runtime-generated malformed-byte, BOM, and Unicode budget probes join 40 portable malformed fixtures, branch/step limits, and the 1,500-hop chain. V25 closed registry shapes and V24 strict-ingestion, V23 branch-aware identity, V22 provider/provenance, V21 identity-phase, V20 provenance, V19 lookup, V18 entity-lineage, V15 source-applicability, and V14 storage fixtures remain regression baselines.
...
Both load the canonical project composition, layer synthetic records into process-local fixture providers, validate the strict-ingestion, malformed-policy, byte-budget, branch/step-limit, and expected-result corpus in `Framework/Data/Reconciliation/`, create uniquely named malformed-byte and deep-chain files in the operating-system temporary directory, and remove them before exit.
