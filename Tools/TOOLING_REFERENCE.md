...
Last compatibility check: 2026-08-02 during Phase 8 CI consolidation. The `pull-request` profile passed in 71.530 seconds with Visualization, QA, 12 root-discovery launches, six unsafe-destination rejections, and complete artifact-lifecycle protection; the `full-release` profile passed in 78.769 seconds and added three byte-identical `298269`-byte SVG renders. QA matched 16 notes, 121 relationships, 71 data references, and all 34 normalized files. Redirected output was removed and canonical outputs remained unchanged.
...
Last parity check: 2026-08-04 for V38. The `baseline` profile passed all fourteen registered suites in Python, PowerShell 7, and Windows PowerShell 5.1 with matching suite IDs, statuses, and canonicalized semantic summaries. Measured local runtimes were 39.2 seconds, 154.8 seconds, and 239.0 seconds respectively. The baseline verifies the reviewed project identity, eight selected packs, 114 enabled and nine disabled capabilities, 24 reconciliation target types, 53 provenance subject types, exact registry schemas/counts and typed semantic declarations, provider and object closure, deterministic repeated loads, disabled-capability absence, and seven invalid cross-registry compositions. Source, entity, provenance, and project composition remain outside the eight-suite `fast` profile because their repeated dependency composition is materially expensive.
