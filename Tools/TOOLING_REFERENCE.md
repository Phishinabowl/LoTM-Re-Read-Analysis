...
| `Project_Config/sources.yaml` | Source registry | `Tools/source_config.py`, `Tools/Source-Config.ps1`, and future validation, content-index, visualization, editor, and migration services | Maintainers through reviewed edits; future source editors through the mutation service | Instantiates media facets, works, localized segments, numbering/orderings, multi-input adaptation mappings, segment-scoped manifestations, release components/packages/events, territories, platform catalogs/offerings, structured time, external identifiers, evidence sources, relationships, and resource bindings. | A media facet, work, segment, numbering/ordering, manifestation, package/release, territory/platform record, identifier, source, or binding changes. |
...
Last parity check: 2026-07-31. Python, PowerShell 7, and Windows PowerShell 5.1 loaded manifest schema 3, schema-pack registry schema 2, six selected packs (`core` 1, `narrative-media` 4, `narrative-publishing` 3, `narrative-screen-audio` 3, `narrative-adaptation` 3, and `narrative-distribution` 2), taxonomy schema 2, resource schema 1, and source schema 5. The current LoTM source registry matched at three media modalities, one cultural form, three release forms, four container formats, three medium profiles, four work-group types, three work groups, two continuities, one continuity relationship, two authority profiles, six works, zero registered segments or numbering schemes, two total ordering schemes, eight verified Book 1 volume ranges, five work relationships, zero adaptation mappings, six manifestations, zero manifestation relationships, two release components, and zero release packages, territories, platforms, release events, catalog placements, platform offerings, identifier schemes, or external identifiers. It also matched at eight evidence sources, fourteen source aliases, two source relationships, four source-resource bindings, and twelve work aliases. A populated v5 fixture passed both loaders with one segment, numbering scheme, package, release event, platform offering, and external identifier. All runtimes resolved 60 declared capabilities, 51 available capabilities, 51 explicitly enabled capabilities, and 402 controlled-value ownership records. The nine planned unavailable capabilities are `editorial-assembly`, `entity-incarnations`, `live-performance-productions`, `migration-services`, `performance-events`, `publication-runs`, `retcon-supersession`, `scoped-continuity`, and `textual-witnesses`. Both QA exporters generated matching 33-file lists and summary counts with one bounded graph and page; stable outputs matched after timestamps, output paths, and line endings were normalized, leaving only the four expected runtime-owned settings/snapshot artifacts.
...
Segments provide stable configurable parts beneath a work. Numbering schemes assign display numbers and aliases without implying sequence, while named total or partial ordering schemes can order works or segments by release, publication, story, production, or recommendation without overwriting another order. Partial schemes use acyclic predecessor references and allow concurrency. Adaptation mappings connect one or more basis works and optional registered segments to a target using controlled correspondence and basis roles. Manifestations may cover a whole work or selected segments; release components, commercial packages, release events, and platform offerings own bundled material, launches, segment scope, and structured availability. Catalog placements target works, segments, manifestations, or packages without redefining canonical work hierarchy. Hierarchical territories and external identifier schemes preserve regional and provider identity separately from internal stable IDs. A source selects a work and medium profile, may bind to the manifestation, package, event, components, or offering it observes, and defines a concrete evidence role, containers, comparison group, priority, aliases, evidence modes, and optional resource bindings.
