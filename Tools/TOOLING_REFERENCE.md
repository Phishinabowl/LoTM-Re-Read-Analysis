...
Last parity check: 2026-07-31. Python, PowerShell 7, and Windows PowerShell 5.1 loaded manifest schema 4, schema-pack registry schema 2, eight selected packs (`core` 10, `narrative-media` 14, `narrative-publishing` 10, `narrative-screen-audio` 11, `narrative-adaptation` 10, `narrative-distribution` 12, `narrative-production` 4, and `narrative-shared-universe` 6), taxonomy schema 2, resource schema 1, source schema 15, and entity schema 3. All ten bundled pack files parsed independently in every runtime. All runtimes resolved 109 declared capabilities, 98 available capabilities, 98 explicitly enabled capabilities, and 638 controlled-value ownership records. The eleven planned unavailable capabilities are `contributor-credits`, `crossover-events`, `editorial-assembly`, `entity-identity-phases`, `entity-identity-reconciliation`, `live-performance-productions`, `migration-services`, `performance-events`, `publication-runs`, `rights-grants-and-restrictions`, and `textual-witnesses`. The empty LoTM entity registry matched at zero entities, entity relationships, incarnations, bindings, or incarnation relationships, with twenty entity and eight incarnation relationship types. V18 fixtures matched for shared entity/incarnation aliases, exact-ID precedence, strict singular ambiguity errors, semantically canonical inverse deduplication in both relationship layers, acyclic lineage rejection, legal cyclic inspiration, independent scoped reversals, and global-plus-scoped cycle rejection; malformed canonical-direction pairs were rejected consistently. Both QA exporters generated matching 33-file lists and summary counts with one bounded graph and page; all 27 stable outputs matched after timestamps, output paths, byte-order marks, and line endings were normalized, leaving only the six expected runtime-owned refresh/bounded report, settings, and snapshot artifacts. Existing V15 source-registry applicability and V14 storage fixture coverage remains the regression baseline for semantic scope, authority, scoped lineage/continuity, production contexts, and claim supersession.
...
`registries.schema_packs`, `registries.taxonomy`, `registries.resources`, `registries.sources`, and `registries.entities` locate the selected-pack, content, resource, source, and entity/incarnation registries.
