...
Last focused parity check: 2026-08-05. Python, PowerShell 7, and Windows PowerShell 5.1 load manifest schema 9, schema-pack selection registry schema 2, schema-3 pack files, eight selected packs (`core` 32, `narrative-media` 22, `narrative-publishing` 10, `narrative-screen-audio` 11, `narrative-adaptation` 10, `narrative-distribution` 14, `narrative-production` 4, and `narrative-shared-universe` 9), taxonomy schema 2, resource schema 1, source schema 18, entity schema 4, reconciliation schema 4, provenance schema 3, chronology schema 2, occurrence schema 6, and lookup-key schema 1 with Unicode 16.0.0. V41 promotes chronology contexts into core and adds provenance-addressable non-precedence context relations with typed occurrence, branch, and applicability bindings while leaving position comparison unchanged. The current composition contains 124 atomic controlled-value namespaces, 955 values, 118 enabled capabilities, and 59 provenance subject types. The LoTM occurrence instance still contains only `main`; no loop, participation, topology relation, or repetition count is fabricated. Focused chronology conformance passes 14 comparisons, five topology queries, 17 malformed fixtures, and a generated 128-relation cyclic topology; occurrence conformance retains 87 query/evaluation assertions, 102 malformed cases, and its existing scale probes with matching summaries across runtimes.
...
Last compatibility check: 2026-08-05 during V41 pressure testing. The `full-release` profile passed all six registered checks in 168.632 seconds. Visualization matched 15 nodes and 121 relationships; QA matched 16 notes, 121 relationships, 71 data references, and all 34 normalized files; all 12 root-discovery launches passed; artifact lifecycle rejected six unsafe destinations; the isolated extraction copied 209 files and passed its portable suites in all three runtimes; and all three renderers produced the same nonblank `298269`-byte SVG with identical SHA-256 hashes. Redirected output was removed and canonical outputs remained unchanged.
...
Last parity check: 2026-08-05 for V41 pressure testing. The `baseline` profile passed all fourteen registered suites in Python, PowerShell 7, and Windows PowerShell 5.1 with matching suite IDs, statuses, and canonicalized semantic summaries. Measured local runtimes were 40.1 seconds, 161.3 seconds, and 253.8 seconds respectively. The baseline verifies the reviewed project identity, eight selected packs, 118 enabled and nine disabled capabilities, 24 reconciliation target types, 59 provenance subject types, exact registry schemas/counts and typed semantic declarations, provider and object closure, deterministic repeated loads, disabled-capability absence, and nine invalid cross-registry compositions. Chronology conformance includes 14 comparison vectors, 11 context-topology queries covering all six core relation types, 17 malformed registries, typed-target and provenance closure, and a generated 128-relation topology cycle; occurrence conformance retains 87 query/evaluation assertions, 102 malformed cases, and its cardinality and participation scale probes. Source, entity, provenance, and project composition remain outside the eight-suite `fast` profile because their repeated dependency composition is materially expensive.
