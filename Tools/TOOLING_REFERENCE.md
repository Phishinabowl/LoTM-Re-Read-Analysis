...
Last parity check: 2026-07-31. Python, PowerShell 7, and Windows PowerShell 5.1 loaded manifest schema 2, taxonomy schema 2, resource schema 1, and source schema 2 with five matching content roots, eleven resource roots, 16 active categories, two deferred categories, six active content types, six resource kinds, eleven resource types, thirteen resource placements, three media, four work-group types, three work groups, two continuities, one continuity relationship, two authority profiles, six works, eight verified Book 1 volume ranges, five work relationships, eight evidence sources, fourteen source aliases, two source relationships, four source-resource bindings, and twelve work aliases. All implementations selected only `glossary` and `volumes` for QA page discovery, leaving investigations, boards, the dashboard, and the navigation index indexed but excluded. Normalized IDs, lifecycle values, policies, paths, authority roles, group nesting and ordering, continuity memberships, volume ranges, source priorities, aliases, typed relationships, tracking modes, and QA/graph flags matched.
