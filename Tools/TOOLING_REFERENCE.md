...
Last parity check: 2026-07-31. Python, PowerShell 7, and Windows PowerShell 5.1 loaded manifest schema 3, schema-pack registry schema 2, seven selected packs (`core` 9, `narrative-media` 13, `narrative-publishing` 9, `narrative-screen-audio` 10, `narrative-adaptation` 9, `narrative-distribution` 11, and `narrative-production` 3), taxonomy schema 2, resource schema 1, and source schema 14. The current LoTM source registry matched at three media modalities, one cultural form, three release forms, four container formats, three medium profiles with explicit work-scope fields and one volume-catalog structural validator, four work-group types, three work groups, two continuities, one continuity relationship, two authority profiles with ten precedence-bearing claim-authority rules, six works, zero applicability scopes, scoped-continuity assertions, work-production contexts, or claim supersessions, registered segments, content groups, or numbering schemes, two total ordering schemes, eight verified Book 1 volume ranges, five work relationships, zero adaptation mappings, six manifestations, zero manifestation relationships or segment mappings, two manifestation-scoped release components, and zero component relationships, release packages, runs, territories, platforms, release events, catalog placements, platform offerings, identifier schemes, external identifiers, observations, coverage entries, or provenance assertions. It also matched at eight evidence sources with hierarchical controlled evidence modes and explicit locator-media allowlists, fourteen source aliases, two source relationships, four source-resource bindings, and twelve work aliases. A populated v14 fixture passed all runtimes with segment-scoped Dragon Ball Z Abridged parody lineage, broad and higher-precedence narrow fan-production/right contexts, a separate commercial Spaceballs context, claim-scoped continuity, shape-compatible acyclic claim supersession, and identical highest-precedence selection. Fourteen negative fixtures consistently rejected an unknown scope target, overlapping exact target/territory/precedence scopes, a work relationship scope outside its source work, a production scope outside its owning work, an invalid continuity target type, an unknown claim scope, shape-incompatible supersession, a supersession cycle, negative precedence, a missing production scope, a mapping where the applicability-scope collection must be a list, and each missing mandatory V14 collection. All runtimes resolved 104 declared capabilities, 95 available capabilities, 95 explicitly enabled capabilities, and 587 controlled-value ownership records. The nine planned unavailable capabilities are `contributor-credits`, `editorial-assembly`, `entity-incarnations`, `live-performance-productions`, `migration-services`, `performance-events`, `publication-runs`, `rights-grants-and-restrictions`, and `textual-witnesses`. Both QA exporters generated matching 33-file lists and summary counts with one bounded graph and page; all 29 stable outputs matched after timestamps, output paths, byte-order marks, and line endings were normalized, leaving only the four expected runtime-owned settings/snapshot artifacts.
