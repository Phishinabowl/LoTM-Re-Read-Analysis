...
Last parity check: 2026-08-02 after adding dedicated entity-registry conformance. The `baseline` profile passed all twelve registered suites in Python, PowerShell 7, and Windows PowerShell 5.1 with matching suite IDs, statuses, and canonicalized semantic summaries. Measured local runtimes were 31.4 seconds, 123.0 seconds, and 187.5 seconds respectively. The new baseline-only `entity` suite composed the neutral taxonomy/source dependencies and validated a vocabulary-neutral schema-4 fixture with three conceptual entities, four continuity-bound incarnations, four identity phases, ambiguity-safe aliases, typed relationship direction and cycles, lineage basis roles, scope bindings, phase ordering, and eight provenance target types; it rejected 86 malformed configurations and 15 invalid service queries and loaded a generated 128-entity composition identically in all three runtimes. The corpus permanently exposed and fixed PowerShell entity-list ingestion that had accepted null, scalar, and non-string values instead of preserving the strict distinction between an explicit empty list and malformed input. Source and entity remain outside the eight-suite `fast` profile because their repeated dependency composition is materially expensive.
