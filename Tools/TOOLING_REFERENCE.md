...
| `Framework/Data/unicode-lookup-16.0.0.json` | Pinned framework runtime data | `Tools/lookup_key_config.py`, `Tools/Lookup-Key-Config.ps1`, source/entity registry loaders, and future semantic lookup consumers | Framework maintainers through reviewed generated-data updates | Defines the trim set, canonical normalization data, and full default case-fold mappings used to produce identical semantic lookup keys in every supported runtime. | The lookup algorithm or pinned Unicode version changes; treat either as a reviewed migration and test newly colliding keys. |
