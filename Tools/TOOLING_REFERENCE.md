...
| `Framework/Data/unicode-lookup-16.0.0.json`, `Framework/Data/lookup-key-regression-vectors.json`, `Framework/Data/Strict-Yaml/`, `Framework/Data/Reconciliation/`, and `Framework/Data/Temporal/` | Pinned framework runtime and conformance data | Strict YAML, lookup, reconciliation, and temporal loaders; paired conformance tools; parity checks; and future semantic identity consumers | Framework maintainers through reviewed data updates | Define deterministic lookup behavior, canonical mapping-key ingestion, branch-aware stable-ID reconciliation vectors, and domain-neutral temporal match/overlap vectors shared by every runtime. | The ingestion, lookup, reconciliation, or temporal algorithm changes, or a discovered portability edge case requires a permanent vector. |
...
| Compare temporal windows and match an effective instant | `temporal_overlap_outcome`, `temporal_windows_overlap`, `temporal_window_match` | `Get-KnowledgeTemporalOverlap`, `Test-KnowledgeTemporalWindowsOverlap`, `Get-KnowledgeTemporalMatch` |
| Normalize an applicability instant | `normalize_effective_at` | `ConvertTo-KnowledgeTemporalInstant` |
| Run portable temporal conformance vectors | `test_temporal.py` | `Test-Temporal.ps1` |
...
Last parity check: 2026-08-01. Python, PowerShell 7, and Windows PowerShell 5.1 loaded manifest schema 7, schema-pack registry schema 2, eight selected packs (`core` 19, `narrative-media` 15, `narrative-publishing` 10, `narrative-screen-audio` 11, `narrative-adaptation` 10, `narrative-distribution` 14, `narrative-production` 4, and `narrative-shared-universe` 9), taxonomy schema 2, resource schema 1, source schema 17, entity schema 4, reconciliation schema 4, provenance schema 2, and lookup-key schema 1 with Unicode 16.0.0. V28 moves temporal ownership into a shared core kernel with open-start/open-end intervals, per-bound precision/certainty/inclusivity, RFC 3339 timezone normalization, and explicit overlap/disjoint/indeterminate/unknown outcomes. Ten valid windows, twelve malformed windows, eight point matches, and six overlap vectors pass identically in all three runtimes. V27 canonical mapping-key vectors, V26 byte/scalar probes, 40 malformed reconciliation fixtures, branch/step limits, and the 1,500-hop chain remain permanent regression baselines alongside V25 closed shapes, V24 strict ingestion, V23 branch-aware identity, V22 provider/provenance, V21 identity phases, V20 provenance, V19 lookup, V18 entity lineage, V15 source applicability, and V14 storage.
