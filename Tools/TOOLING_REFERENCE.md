...
| `Framework/Data/unicode-lookup-16.0.0.json` and `Framework/Data/lookup-key-regression-vectors.json` | Pinned framework runtime and conformance data | `Tools/lookup_key_config.py`, `Tools/Lookup-Key-Config.ps1`, source/entity registry loaders, parity checks, and future semantic lookup consumers | Framework maintainers through reviewed generated-data updates | Define the trim set, canonical normalization data, full default case-fold mappings, and equivalent/distinct vectors used to produce and verify identical semantic lookup keys in every supported runtime. | The lookup algorithm, pinned Unicode version, or a discovered comparison edge case changes; treat algorithm/data changes as reviewed migrations and test newly colliding keys. |
| `Framework/Contracts/README.md`, `Framework/Contracts/narrative-source-registry.md`, and `Framework/Contracts/narrative-entity-registry.md` | Framework contract index and narrative registry contracts | Framework maintainers and future schema tooling | Framework maintainers | Record executable configuration boundaries, schema ownership, media/work/provenance semantics, and conceptual entity/incarnation semantics. | A configuration contract is introduced, stabilized, moved, or assigned a validator. |
...
| `Project_Config/sources.yaml` | Source registry | `Tools/source_config.py`, `Tools/Source-Config.ps1`, and future validation, content-index, visualization, editor, and migration services | Maintainers through reviewed edits; future source editors through the mutation service | Instantiates media facets, works, reusable applicability scopes, scoped continuity and claim supersession, production/right contexts, structural segments and recursively nested role-bearing content groups, numbering/orderings, explainable hierarchical precedence-aware authority, multi-source and stable-claim evaluation, transformative and multi-input adaptation mappings, segment-scoped manifestations and version mappings, package-scoped components and lineage, release packages/phased runs/events, territories, historical localized titles and platform catalogs/offerings, ordered structured time, external identifiers, multi-target source observations, channel-bounded structurally validated coverage ranges, semantic provenance paths, mode-attributed point/range mixed-media locators, relationships, and resource bindings. | A media facet, work, applicability scope, continuity assertion, claim supersession, production context, content group, nested record, numbering/ordering, authority rule, position strategy, manifestation/mapping, component/package/run/release, territory/platform record, identifier, observation, source coverage, provenance assertion, or binding changes. |
| `Project_Config/entities.yaml` | Entity/incarnation registry | `Tools/entity_config.py`, `Tools/Entity-Config.ps1`, and future provenance, content-index, visualization, editor, and migration services | Maintainers through reviewed edits; future entity editors through the mutation service | Instantiates multi-category conceptual entities, ambiguity-preserving name indexes, semantically canonical and optionally acyclic entity lineage with basis roles, continuity-bound incarnations, scope-backed material bindings, and equivalently normalized incarnation relationships without conflating them with pages, portrayals, or manifestations. | A category membership, shared label/alias, relationship direction or cycle policy, same-continuity entity relationship, justified incarnation split, continuity membership, scoped appearance, or incarnation relationship changes. |
...
| Load and validate entity/incarnation registry | `load_entity_registry` in `entity_config.py` | `Get-KnowledgeEntityRegistry` in `Entity-Config.ps1` |
| Resolve every entity or incarnation matching a stable ID, label, or alias | `EntityRegistry.resolve_entity_ids`, `resolve_incarnation_ids` | `Resolve-KnowledgeEntityIds`, `Resolve-KnowledgeIncarnationIds` |
| Resolve one unambiguous entity or incarnation, rejecting shared-name ambiguity | `EntityRegistry.resolve_entity_id`, `resolve_incarnation_id` | `Resolve-KnowledgeEntityId`, `Resolve-KnowledgeIncarnationId` |
...
Last parity check: 2026-07-31. Python, PowerShell 7, and Windows PowerShell 5.1 loaded manifest schema 5, schema-pack registry schema 2, eight selected packs (`core` 11, `narrative-media` 14, `narrative-publishing` 10, `narrative-screen-audio` 11, `narrative-adaptation` 10, `narrative-distribution` 12, `narrative-production` 4, and `narrative-shared-universe` 6), taxonomy schema 2, resource schema 1, source schema 15, entity schema 3, and lookup-key schema 1 with Unicode 16.0.0. All ten bundled pack files parsed independently in every runtime. All runtimes resolved 110 declared capabilities, 99 available capabilities, 99 explicitly enabled capabilities, and 638 controlled-value ownership records. The eleven planned unavailable capabilities remain `contributor-credits`, `crossover-events`, `editorial-assembly`, `entity-identity-phases`, `entity-identity-reconciliation`, `live-performance-productions`, `migration-services`, `performance-events`, `publication-runs`, `rights-grants-and-restrictions`, and `textual-witnesses`. V19 matched 30,106 lookup vectors across all three runtimes, covering every pinned mapping source, every canonical composition pair, every Hangul syllable, targeted Unicode edge cases, and seeded mixed strings. Entity fixtures resolved sharp-s, composed/decomposed accents, and dotted-I identically through the real alias indexes. Both QA exporters generated matching 33-file lists and summary counts with one bounded graph and page; 29 normalized outputs matched exactly, leaving only the four expected runtime-owned settings and snapshot artifacts. Existing V18 entity-lineage, V15 source-applicability, and V14 storage fixtures remain regression baselines.
...
`registries.schema_packs`, `registries.taxonomy`, `registries.resources`, `registries.sources`, `registries.entities`, and `registries.lookup_keys` locate the selected-pack, content, resource, source, entity/incarnation, and pinned Unicode lookup-key registries.
...
The paired source loaders reject malformed or duplicate IDs, invalid position/citation/work-scope/structure definitions, unknown media-facet/group/continuity/profile/work/segment/medium/container/resource/source references, incompatible cultural forms and modalities, invalid structural or mixed-member group, territory, or catalog nesting and cycles, unsupported content-group roles, duplicate numbering or ordering targets, labels, or ordinals, missing or duplicate ordered-group ordinals, malformed or non-contiguous verified volume catalogs, unsupported statuses/policies/mapping/title/coverage/claim/locator/production values, invalid applicability targets, territories, precedence, or overlapping exact-scope windows, work relationships or production contexts whose scopes fall outside their owning work, invalid scoped-continuity targets, unknown or shape-incompatible supersession claims, supersession cycles, overlapping localized-title windows, tied winning authority rules, invalid manifestation mapping shapes, orphaned package-only components, malformed component lineage, release-run phase partitions, cadence, or exceptions, out-of-scope manifestation/package/run/event/offering segments, invalid structured times, reversed or structurally impossible coverage ranges, undeclared coverage channels, overlapping same-channel coverage, disallowed locator media, malformed point/range locators, locators outside their actual medium/mode coverage, out-of-scope or impossible locator positions, unresolved provenance field paths, missing or shape-inconsistent claim keys, invalid provenance targets/evidence, duplicate external identifiers, duplicate aliases, malformed inverse-type definitions, invalid or self-referential typed relationships, escaping or out-of-placement resource paths, and missing required bindings.

### Entity Registry Contract

`Project_Config/entities.yaml` separates continuity-independent conceptual entities from continuity-bound incarnations. Entities declare one primary taxonomy category and one or more total category memberships. Typed entity relationships preserve succession, namesakes, legacy/mantles, cloning, splinters, derivation/composites, inspiration, and class/instance identity; lineage records may qualify inherited semantic dimensions with controlled basis roles. Incarnations reference one entity, one primary continuity, and one or more status-bearing continuity memberships. Scope-backed bindings identify where an incarnation is primary, appears, or crosses into registered material without duplicating source-registry target, territory, time, or precedence fields. Typed incarnation relationships preserve counterpart, branch, reboot, and derivation semantics with explicit reciprocal inverses.

The paired entity loaders require the selected and enabled `entity-incarnations` capability and consume normalized schema-pack, taxonomy, and source registries. They expose ambiguity-preserving plural name resolution, strict singular resolution, entity relationships, entity-to-incarnation and incarnation-to-binding/relationship queries, plus stable provenance-target lookup for entities and both relationship layers. Relationship types explicitly declare their semantic canonical direction and may share scope-aware acyclic groups; inverse-form duplicates are one repeated fact in both layers. The central provenance service does not yet accept those cross-registry targets, so evidence assertions remain out of `entities.yaml`. The current LoTM registry deliberately contains no entities, entity relationships, or incarnations and provides twenty entity plus eight incarnation relationship-type definitions; page linkage and existing-subject migration remain later mutation-service work. Identity phases and stable-ID reconciliation are declared but remain unavailable until their own executable contracts exist.

The loaders reject unsupported schema/capability state, malformed IDs, unknown or empty category memberships, a primary category outside the membership list, unknown entities/continuities/scopes, duplicate aliases, missing primary continuity memberships, unsupported statuses, basis roles, or binding/relationship types, basis roles on non-lineage relationships, inconsistent inverse definitions, unknown endpoints, self-relationships, and duplicate or inverse-duplicate bindings/relationships.
