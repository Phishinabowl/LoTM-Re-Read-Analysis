...
| `Framework/Contracts/README.md`, narrative registry contracts, and `Framework/Contracts/identity-target-provider.md` | Framework contract index, registry contracts, and provider boundaries | Framework maintainers and future schema tooling | Framework maintainers | Record executable configuration boundaries, schema ownership, media/work/provenance semantics, identity phases, and typed identity/provenance provider boundaries. | A configuration contract or provider boundary is introduced, stabilized, moved, or assigned a validator. |
...
Last parity check: 2026-07-31. Python, PowerShell 7, and Windows PowerShell 5.1 loaded manifest schema 6, schema-pack registry schema 2, eight selected packs (`core` 12, `narrative-media` 14, `narrative-publishing` 10, `narrative-screen-audio` 11, `narrative-adaptation` 10, `narrative-distribution` 12, `narrative-production` 4, and `narrative-shared-universe` 8), taxonomy schema 2, resource schema 1, source schema 16, entity schema 4, provenance schema 1, and lookup-key schema 1 with Unicode 16.0.0. All runtimes resolved 111 declared capabilities, 101 available capabilities, 101 explicitly enabled capabilities, and 657 controlled-value ownership records. V21 fixtures validate entity- and incarnation-owned phases, scope-backed phase bindings, same-subject acyclic succession, ambiguity-safe phase lookup, typed identity-target lookup, and phase-targeted provenance in every runtime; malformed cross-subject, cross-continuity, duplicate-inverse, and cyclic phase relationships fail consistently. V20 provenance, V19 lookup, V18 entity-lineage, V15 source-applicability, and V14 storage fixtures remain regression baselines.
...
The paired entity loaders require the selected and enabled `entity-incarnations` and `entity-identity-phases` capabilities and consume normalized schema-pack, taxonomy, and source registries. They expose ambiguity-preserving plural name resolution, strict singular resolution, entity relationships, entity-to-incarnation, subject-to-phase, and binding/relationship queries; a narrow identity-target provider; and stable provenance-target lookup for all record layers. Relationship types explicitly declare semantic canonical direction and may share acyclic groups; inverse-form duplicates are one repeated fact. The standalone provenance service consumes those typed targets while keeping assertions and locators out of `entities.yaml`. The current LoTM registry deliberately contains no entities, incarnations, or phases and provides twenty entity, eight incarnation, and two phase relationship-type definitions. Page linkage, stable-ID reconciliation, and existing-subject migration remain later work.
