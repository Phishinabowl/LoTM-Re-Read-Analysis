...
Last focused parity check: 2026-08-05. Python, PowerShell 7, and Windows PowerShell 5.1 load manifest schema 9, schema-pack selection registry schema 2, schema-4 pack files, eight selected packs (`core` 35, `narrative-media` 24, `narrative-publishing` 10, `narrative-screen-audio` 11, `narrative-adaptation` 10, `narrative-distribution` 14, `narrative-production` 4, and `narrative-shared-universe` 9), taxonomy schema 2, resource schema 1, source schema 18, entity schema 4, reconciliation schema 4, provenance schema 3, chronology schema 2, occurrence schema 9, and lookup-key schema 1 with Unicode 16.0.0. V44 adds profile-governed capability values, registry-local qualitative and bounded-integer scales, explicit improvement/degradation, and credential separation while preserving V43 epistemic, chronology, and authority boundaries. The current composition contains 128 atomic controlled-value namespaces, 994 values, four state profiles, 18 state-kind mappings, 120 enabled capabilities, and 61 provenance subject types. Reusable profiles may remain dormant in reduced compositions, while every controlled state kind must map exactly once. The LoTM occurrence instance still contains only `main`; no loop, branch-state claim, participation, topology relation, epistemic or capability progression, or repetition count is fabricated. Focused occurrence conformance passes 128 query/evaluation assertions, 147 malformed cases, and generated 128-branch-state, 128-cardinality, and 128-participation/track-entry probes with matching summaries across runtimes; provenance now validates 20 portable assertions including nested capability values.
...
Last parity check: 2026-08-05 for Phase 1.9 temporal stabilization. The `baseline` profile passed all fourteen registered suites in Python, PowerShell 7, and Windows PowerShell 5.1 with matching suite IDs, statuses, and canonicalized semantic summaries. Sequentially measured local runtimes were 39.6 seconds, 182.4 seconds, and 325.1 seconds respectively. The baseline verifies the reviewed project identity, eight selected packs, 120 enabled and nine disabled capabilities, 24 reconciliation target types, 61 provenance subject types, exact registry schemas/counts and typed semantic declarations including four state profiles and 18 state-kind mappings, provider and object closure, deterministic repeated loads, disabled-capability absence, and eleven invalid cross-registry compositions. Occurrence conformance includes 128 query/evaluation assertions, 147 malformed cases, exact epistemic and capability snapshots, branch-continuity composition closure, and 128-record branch-state, cardinality, and participation/track-entry scale probes. The `full-release` compatibility profile passed all six checks in 159.184 seconds, preserving 15 visualization nodes, 121 relationships, 16 QA notes, 34 normalized QA files, 12 root launches, artifact safety, 209-file isolated framework extraction, and identical nonblank rendering across runtimes. Source, entity, provenance, and project composition remain outside the eight-suite `fast` profile because their repeated dependency composition is materially expensive.
