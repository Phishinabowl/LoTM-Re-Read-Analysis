...
| `Framework/Data/unicode-lookup-16.0.0.json`, `Framework/Data/lookup-key-regression-vectors.json`, `Framework/Data/Strict-Yaml/`, `Framework/Data/Reconciliation/`, `Framework/Data/Temporal/`, and `Framework/Data/Chronology/` | Pinned framework runtime and conformance data | Strict YAML, lookup, reconciliation, temporal, and chronology loaders; paired conformance tools; parity checks; and future semantic identity consumers | Framework maintainers through reviewed data updates | Define deterministic lookup behavior, canonical mapping-key ingestion, branch-aware stable-ID reconciliation vectors, civil-time match/overlap vectors, and chronology comparison vectors shared by every runtime. | An ingestion, lookup, reconciliation, temporal, or chronology algorithm changes, or a discovered portability edge case requires a permanent vector. |
...
Last parity check: 2026-08-01. Python, PowerShell 7, and Windows PowerShell 5.1 loaded manifest schema 8, schema-pack registry schema 2, eight selected packs (`core` 21, `narrative-media` 16, `narrative-publishing` 10, `narrative-screen-audio` 11, `narrative-adaptation` 10, `narrative-distribution` 14, `narrative-production` 4, and `narrative-shared-universe` 9), taxonomy schema 2, resource schema 1, source schema 18, entity schema 4, reconciliation schema 4, provenance schema 3, chronology schema 1, and lookup-key schema 1 with Unicode 16.0.0. V30 closes the RFC 3339 `-00:00` and exclusive calendar-extrema defects, makes civil-time and chronology capabilities explicit, and adds paired chronology registry/comparison services. The LoTM instance contains one Epoch coordinate system, five ordered eras, no fabricated concrete positions, spans, or mappings, and one source-validated novel-continuity story context. Seventeen valid temporal windows, twenty-one malformed temporal windows, twenty query matches, twelve overlap vectors, eleven chronology comparisons, and eleven malformed chronology registries pass identically in all three runtimes. V27 canonical mapping keys, V26 byte/scalar probes, 40 malformed reconciliation fixtures, branch/step limits, and the 1,500-hop chain remain permanent regression baselines alongside the earlier versions.
...
`registries.schema_packs`, `registries.taxonomy`, `registries.resources`, `registries.sources`, `registries.chronology`, `registries.entities`, `registries.reconciliation`, `registries.provenance`, and `registries.lookup_keys` locate the selected-pack, content, resource, source, chronology, entity/incarnation, stable-ID reconciliation, cross-registry provenance, and pinned Unicode lookup-key registries.
...
The paired comparison APIs return `before`, `after`, `concurrent`, or `incomparable`. They compare coordinates only within one axis, through an exact explicit relation, or through a direct exact equivalent mapping. They do not extrapolate mapping formulas, infer calendar conversions, impose total order across branches, or confuse publication, reader disclosure, causal order, and story chronology. The LoTM registry currently instantiates the five Epochs and a novel-story context but deliberately contains no unsupported dates or anchors.
