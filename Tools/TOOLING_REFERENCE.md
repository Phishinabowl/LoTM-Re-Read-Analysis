...
| `Project_Config/sources.yaml` | Source registry | `Tools/source_config.py`, `Tools/Source-Config.ps1`, and future validation, content-index, visualization, editor, and migration services | Maintainers through reviewed edits; future source editors through the mutation service | Instantiates media facets, works, structural segments and recursively nested role-bearing content groups, numbering/orderings, claim-aware authority, multi-input adaptation mappings, segment-scoped manifestations and version mappings, package-scoped components and lineage, release packages/phased runs/events, territories, historical localized titles and platform catalogs/offerings, structured time, external identifiers, multi-target source observations, semantic coverage ranges, mixed-media assertion provenance, relationships, and resource bindings. | A media facet, work, content group, nested record, numbering/ordering, authority rule, manifestation/mapping, component/package/run/release, territory/platform record, identifier, observation, source coverage, provenance assertion, or binding changes. |
...
| Resolve claim-specific source authority with priority fallback | `SourceRegistry.authority_rank` | `Get-KnowledgeSourceAuthorityRank` |
...
Last parity check: 2026-07-31. Python, PowerShell 7, and Windows PowerShell 5.1 loaded manifest schema 3, schema-pack registry schema 2, six selected packs (`core` 5, `narrative-media` 8, `narrative-publishing` 4, `narrative-screen-audio` 5, `narrative-adaptation` 4, and `narrative-distribution` 6), taxonomy schema 2, resource schema 1, and source schema 9. The current LoTM source registry matched at three media modalities, one cultural form, three release forms, four container formats, three medium profiles with explicit work-scope fields, four work-group types, three work groups, two continuities, one continuity relationship, two authority profiles with ten claim-authority rules, six works, zero registered segments, content groups, or numbering schemes, two total ordering schemes, eight verified Book 1 volume ranges, five work relationships, zero adaptation mappings, six manifestations, zero manifestation relationships or segment mappings, two manifestation-scoped release components, and zero component relationships, release packages, runs, territories, platforms, release events, catalog placements, platform offerings, identifier schemes, external identifiers, observations, coverage entries, or provenance assertions. It also matched at eight evidence sources with explicit locator-media allowlists, fourteen source aliases, two source relationships, four source-resource bindings, and twelve work aliases. A populated v9 fixture passed all runtimes with recursively nested role-bearing content groups, globally stable nested IDs, two nonoverlapping historical titles in one locale scope, semantic cross-medium work positions, two heterogeneous source observations, work-scoped group coverage, claim-aware authority, and one assertion evidence link carrying two stable mixed-media locators. Seven negative fixtures consistently rejected overlapping title windows, unsupported member roles, out-of-scope coverage ranges, disallowed locator media, out-of-scope locator positions, duplicate nested IDs, and ambiguous claim-authority rules. All runtimes resolved 83 declared capabilities, 74 available capabilities, 74 explicitly enabled capabilities, and 502 controlled-value ownership records. The nine planned unavailable capabilities are `editorial-assembly`, `entity-incarnations`, `live-performance-productions`, `migration-services`, `performance-events`, `publication-runs`, `retcon-supersession`, `scoped-continuity`, and `textual-witnesses`. Both QA exporters generated matching 33-file lists and summary counts with one bounded graph and page; all 29 stable outputs matched after timestamps, output paths, byte-order marks, and line endings were normalized, leaving only the four expected runtime-owned settings/snapshot artifacts.
...
A source selects one or more works, a primary medium profile, and an allowlist of locator media. Singular release bindings serve simple evidence artifacts, while repeatable typed observations identify every manifestation, package, event, component, or offering inspected by composite evidence. Stable coverage entries state how much material is present; range endpoints must name the same canonical work through the medium's configured work-scope field and remain inside the target's resolved work scope. Central provenance assertions declare a controlled claim namespace, target top-level or globally stable nested records, snapshot the asserted value plus observation/effective timing, and attach supporting, contradicting, or contextual evidence through one or more globally stable locators. Each locator explicitly selects an allowed medium and valid in-scope position. Verified and inferred assertions require supporting evidence; disputed assertions require both supporting and contradicting evidence.
...
Authority profiles define accepted continuity memberships, continuity order, source-priority direction, which derivative-work relationships are comparable, and optional claim-specific rules. Reader positions use canonical `book` work IDs before work-local volume and chapter values. Rules can rank sources by source ID, role, medium, or evidence mode for one controlled claim namespace; ordinary source priority remains the fallback. Under the default `lotm-adaptation-comparison` profile, source-scoped claims are preserved and authority varies appropriately among canonical content, dialogue, visual design, localization, and release metadata rather than forcing one artifact to lead every question.
...
The paired source loaders reject malformed or duplicate IDs, invalid position/citation/work-scope definitions, unknown media-facet/group/continuity/profile/work/segment/medium/container/resource/source references, incompatible cultural forms and modalities, invalid structural or mixed-member group, territory, or catalog nesting and cycles, unsupported content-group roles, duplicate numbering or ordering targets, labels, or ordinals, missing or duplicate ordered-group ordinals, malformed or non-contiguous verified volume catalogs, unsupported statuses/policies/mapping/title/coverage/claim types, overlapping localized-title windows, ambiguous authority rules, invalid manifestation mapping shapes, orphaned package-only components, malformed component lineage, release-run phase partitions, cadence, or exceptions, out-of-scope manifestation/package/run/event/offering segments, invalid structured times, semantically out-of-scope coverage ranges, disallowed locator media, out-of-scope locator positions, invalid provenance targets/evidence/field paths, duplicate external identifiers, duplicate aliases, malformed inverse-type definitions, invalid or self-referential typed relationships, escaping or out-of-placement resource paths, and missing required bindings.
