...
Last parity check: 2026-07-31. Python, PowerShell 7, and Windows PowerShell 5.1 loaded manifest schema 6, schema-pack registry schema 2, eight selected packs (`core` 12, `narrative-media` 14, `narrative-publishing` 10, `narrative-screen-audio` 11, `narrative-adaptation` 10, `narrative-distribution` 12, `narrative-production` 4, and `narrative-shared-universe` 7), taxonomy schema 2, resource schema 1, source schema 16, entity schema 3, provenance schema 1, and lookup-key schema 1 with Unicode 16.0.0. All runtimes resolved 111 declared capabilities, 100 available capabilities, 100 explicitly enabled capabilities, and 643 controlled-value ownership records. V20 fixtures validated a real entity-targeted assertion, semantic field path, novel locator, authority winner, exact claim applicability, and scoped supersession chain in every runtime; a missing entity target failed consistently. Both QA exporters produced matching 28-file sets and summary counts; 25 normalized semantic outputs matched exactly, leaving only the three expected runtime-owned refresh settings/report/snapshot artifacts. Existing V19 lookup, V18 entity-lineage, V15 source-applicability, and V14 storage fixtures remain regression baselines.
...
`registries.schema_packs`, `registries.taxonomy`, `registries.resources`, `registries.sources`, `registries.entities`, `registries.provenance`, and `registries.lookup_keys` locate the selected-pack, content, resource, source, entity/incarnation, cross-registry provenance, and pinned Unicode lookup-key registries.
...
`Project_Config/provenance.yaml` schema 1 is loaded after source and entity registries. The paired `load_provenance_registry` / `Get-KnowledgeProvenanceRegistry` APIs compose `SourceRegistry.provenance_target` / `Get-KnowledgeSourceProvenanceTarget` with `EntityRegistry.provenance_target` / `Get-KnowledgeEntityProvenanceTarget`. They own assertion parsing, semantic field-path resolution, evidence-link and locator validation, stable claim grouping, scope-backed acyclic claim supersession, and cross-registry target integrity. `ProvenanceRegistry.evaluate_claim_authority` / `Get-KnowledgeClaimAuthorityEvaluation` delegate evidence ranking to source authority decisions and then report winner, tie, conflict, or incomparable outcomes.
