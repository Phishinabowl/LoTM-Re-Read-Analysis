...
Last parity check: 2026-07-31. Python, PowerShell 7, and Windows PowerShell 5.1 loaded manifest schema 3, schema-pack registry schema 2, seven selected packs (`core` 8, `narrative-media` 12, `narrative-publishing` 8, `narrative-screen-audio` 9, `narrative-adaptation` 8, `narrative-distribution` 10, and `narrative-production` 2), taxonomy schema 2, resource schema 1, and source schema 13. The current LoTM source registry matched at three media modalities, one cultural form, three release forms, four container formats, three medium profiles with explicit work-scope fields and one volume-catalog structural validator, four work-group types, three work groups, two continuities, one continuity relationship, two authority profiles with ten precedence-bearing claim-authority rules, six works, zero work-production contexts, registered segments, content groups, or numbering schemes, two total ordering schemes, eight verified Book 1 volume ranges, five work relationships, zero adaptation mappings, six manifestations, zero manifestation relationships or segment mappings, two manifestation-scoped release components, and zero component relationships, release packages, runs, territories, platforms, release events, catalog placements, platform offerings, identifier schemes, external identifiers, observations, coverage entries, or provenance assertions. It also matched at eight evidence sources with hierarchical controlled evidence modes and explicit locator-media allowlists, fourteen source aliases, two source relationships, four source-resource bindings, and twelve work aliases. A populated v13 fixture passed all runtimes with distinct Dragon Ball Z Abridged and Spaceballs parody works/continuities, web-series and film forms, video-sharing-platform distribution, and independent fan-production versus third-party commercial production contexts without inferred authorization or rights basis. Seven negative fixtures consistently rejected a conflated legacy production-origin value, unknown production territory, unknown production work, duplicate production-context ID, overlapping contexts for one work/exact territory scope, a missing production-context collection, and a mapping where that collection must be a list. All runtimes resolved 102 declared capabilities, 92 available capabilities, 92 explicitly enabled capabilities, and 570 controlled-value ownership records. The ten planned unavailable capabilities are `contributor-credits`, `editorial-assembly`, `entity-incarnations`, `live-performance-productions`, `migration-services`, `performance-events`, `publication-runs`, `retcon-supersession`, `scoped-continuity`, and `textual-witnesses`. Both QA exporters generated matching 33-file lists and summary counts with one bounded graph and page; all 29 stable outputs matched after timestamps, output paths, byte-order marks, and line endings were normalized, leaving only the four expected runtime-owned settings/snapshot artifacts.
