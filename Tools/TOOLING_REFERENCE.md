...
| `Framework/Contracts/README.md` | Framework contract index | Framework maintainers and future schema tooling | Framework maintainers | Records the current executable contract boundary and where stabilized machine-readable configuration schemas belong. | A configuration contract is introduced, stabilized, moved, or assigned a validator. |
...
| `Framework/Packs/narrative-media/pack.yaml` | Domain schema pack | Schema-pack loaders, source loaders, and future narrative editors/wizards | Narrative-media pack maintainers | Declares narrative capabilities and controlled media, work-group, work, continuity, and creative-lineage vocabulary without containing LoTM instances. | A reusable narrative-media concept is introduced or revised. |
...
| `Project_Config/sources.yaml` | Source registry | `Tools/source_config.py`, `Tools/Source-Config.ps1`, and future validation, content-index, visualization, editor, and migration services | Maintainers through reviewed edits; future source editors through the mutation service | Defines work-group types and nesting, continuities, authority profiles, creative works, per-work volumes, media and position/citation schemas, evidence sources, aliases, evidence modes, comparison groups, priority, typed work/source relationships, and resource bindings. | A group/continuity/profile/work/volume/source/medium is added, renamed, reprioritized, related, assigned different citation/position behavior, or bound to a different resource. |
...
| Test capability availability and activation | `SchemaPackRegistry.capability_available`, `capability_enabled` | `Test-SchemaPackCapabilityAvailable`, `Test-SchemaPackCapabilityEnabled` |
...
Last parity check: 2026-07-31. Python, PowerShell 7, and Windows PowerShell 5.1 loaded manifest schema 3, schema-pack registry schema 2, two selected schema packs, taxonomy schema 2, resource schema 1, and source schema 2 with five matching content roots, twelve resource roots, 16 active categories, two deferred categories, six active content types, six resource kinds, thirteen resource types, fifteen resource placements, three media, four work-group types, three work groups, two continuities, one continuity relationship, two authority profiles, six works, eight verified Book 1 volume ranges, five work relationships, eight evidence sources, fourteen source aliases, two source relationships, four source-resource bindings, and twelve work aliases. All runtimes resolved 25 available capabilities and 24 explicitly enabled capabilities: `migration-services` remained available but disabled, while an unprovided medical capability remained unavailable. Controlled-value ownership, definitions, and broader-value hierarchies matched. Both QA exporters generated matching 28-file lists and summary counts; after stable-output normalization, only the three expected runtime-specific refresh artifacts differed.
...
Pack selection makes capabilities available; `capability_activation.enabled` activates only the available capabilities used by the project. `capability_activation.default` must be `disabled`. Missing capabilities therefore disappear cleanly from unrelated industry implementations, while missing pack dependencies and explicit references to unavailable or disabled contracts remain validation errors.

Controlled values use dotted ownership namespaces such as `source.work-type`. Values may be extended by multiple selected packs, but one exact namespace/value pair has one owner. A value may provide a display label, description, and broader value in the same namespace. The current `core` pack owns generic evidence-source roles and evidence-artifact relationships. The reusable `narrative-media` pack owns media-specific release/subtitle/dub vocabulary plus general narrative media, work-group, creative-work, continuity, creative-lineage, and embedded-visual-asset terms. It preserves `anime` and `donghua` beneath `animation`; `manga`, `manhwa`, and `manhua` beneath `comic`; and official EPUB artwork beneath `illustration`. Embedded visuals may originate in EPUBs, comics, scans, or future supported containers; extraction preserves source provenance while the image and any promoted page-ready derivative remain visual resources. The source loaders reject configured vocabulary outside the aggregate selected-pack contract.
...
`Project_Config/sources.yaml` separates work groups, continuities, creative works, media, and evidence sources. Work groups may represent franchises, ordered series, heterogeneous adaptation programs, or unordered collections and may nest beneath a parent group. An adaptation program can interleave seasons, specials, character features, and films while each remains an independent work in the shared adaptation continuity. A work defines stable identity, group and continuity memberships, display aliases, numbering mode, and a verified, pending, or inapplicable volume catalog. A medium defines position fields, required/sort fields, and citation templates. A source selects a work and medium and defines a concrete evidence role, comparison group, priority, aliases, evidence modes, and optional bindings to registered resources.
...
The paired source loaders reject malformed IDs, invalid position/citation definitions, unknown group/continuity/profile/work/medium/resource/source references, invalid group nesting or cycles, missing or duplicate ordered-group ordinals, malformed or non-contiguous verified volume catalogs, unsupported roles/statuses/policies, duplicate aliases, malformed inverse-type definitions, invalid or self-referential typed relationships, escaping or out-of-placement resource paths, and missing required bindings.
