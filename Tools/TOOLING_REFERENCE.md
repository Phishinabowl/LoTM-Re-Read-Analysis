...
Last parity check: 2026-07-31. Python, PowerShell 7, and Windows PowerShell 5.1 loaded manifest schema 3, schema-pack registry schema 2, seven selected packs (`core` 10, `narrative-media` 14, `narrative-publishing` 10, `narrative-screen-audio` 11, `narrative-adaptation` 10, `narrative-distribution` 12, and `narrative-production` 4), taxonomy schema 2, resource schema 1, and source schema 15. All ten available pack files also parsed individually in every runtime. The current LoTM source registry matched at three media modalities, one cultural form, three release forms, four container formats, three medium profiles with explicit work-scope fields and one volume-catalog structural validator, four work-group types, three work groups, two continuities, one continuity relationship, two authority profiles with ten precedence-bearing claim-authority rules, six works, zero applicability scopes, scoped-continuity assertions, work-production contexts, or claim supersessions, registered segments, content groups, or numbering schemes, two total ordering schemes, eight verified Book 1 volume ranges, five work relationships, zero adaptation mappings, six manifestations, zero manifestation relationships or segment mappings, two manifestation-scoped release components, and zero component relationships, release packages, runs, territories, platforms, release events, catalog placements, platform offerings, identifier schemes, external identifiers, observations, coverage entries, or provenance assertions. It also matched at eight evidence sources with hierarchical controlled evidence modes and explicit locator-media allowlists, fourteen source aliases, two source relationships, four source-resource bindings, and twelve work aliases. A populated V15 applicability fixture produced identical explainable decisions across all runtimes for structural segment ancestry, content-group containment, sibling isolation, manifestation components, release packages, provenance-claim subject inheritance, territory ancestry, inclusive effective windows, expired scopes, unknown-time indeterminacy, explicit precedence, and tied winners. Six negative query cases consistently rejected missing or unknown target types/IDs, unknown or empty territories, and malformed effective time. The V14 storage fixture coverage remains valid for scoped parody lineage, production/right contexts, scoped continuity, and acyclic shape-compatible claim supersession. All runtimes resolved 105 declared capabilities, 96 available capabilities, 96 explicitly enabled capabilities, and 587 controlled-value ownership records. The nine planned unavailable capabilities are `contributor-credits`, `editorial-assembly`, `entity-incarnations`, `live-performance-productions`, `migration-services`, `performance-events`, `publication-runs`, `rights-grants-and-restrictions`, and `textual-witnesses`. Both QA exporters generated matching 33-file lists and summary counts with one bounded graph and page; all 29 stable outputs matched after timestamps, output paths, byte-order marks, and line endings were normalized, leaving only the four expected runtime-owned settings/snapshot artifacts.
