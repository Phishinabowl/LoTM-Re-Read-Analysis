...
| Resolve controlled claim ancestry | `controlled_value_ancestors` | `Get-SourceControlledValueAncestors` |
| Validate and compare evidence positions | `validate_source_position`, `validate_structural_position`, `compare_positions` | `Assert-SourceEvidencePosition`, `Assert-SourceStructuralPosition`, `Compare-SourcePositions` |
...
Last parity check: 2026-07-31. Python, PowerShell 7, and Windows PowerShell 5.1 loaded manifest schema 3, schema-pack registry schema 2, six selected packs (`core` 6, `narrative-media` 9, `narrative-publishing` 5, `narrative-screen-audio` 6, `narrative-adaptation` 5, and `narrative-distribution` 7), taxonomy schema 2, resource schema 1, and source schema 10. The current LoTM source registry matched at three media modalities, one cultural form, three release forms, four container formats, three medium profiles with explicit work-scope fields and one volume-catalog structural validator, four work-group types, three work groups, two continuities, one continuity relationship, two authority profiles with ten precedence-bearing claim-authority rules, six works, zero registered segments, content groups, or numbering schemes, two total ordering schemes, eight verified Book 1 volume ranges, five work relationships, zero adaptation mappings, six manifestations, zero manifestation relationships or segment mappings, two manifestation-scoped release components, and zero component relationships, release packages, runs, territories, platforms, release events, catalog placements, platform offerings, identifier schemes, external identifiers, observations, coverage entries, or provenance assertions. It also matched at eight evidence sources with explicit locator-media allowlists, fourteen source aliases, two source relationships, four source-resource bindings, and twelve work aliases. A populated v10 fixture passed all runtimes with inherited claim authority, a higher-precedence narrow override, a semantically resolved indexed provenance path, structurally validated source coverage, one novel evidence point, one ordered novel evidence range, and one mixed-media illustration point. Nine negative fixtures consistently rejected tied winning authority rules, unresolved field paths, invalid point/range shapes, impossible chapters, incorrect volume/chapter assignments, mismatched range fields, reversed evidence ranges, impossible coverage positions, and reversed coverage ranges. All runtimes resolved 88 declared capabilities, 79 available capabilities, 79 explicitly enabled capabilities, and 505 controlled-value ownership records. The nine planned unavailable capabilities are `editorial-assembly`, `entity-incarnations`, `live-performance-productions`, `migration-services`, `performance-events`, `publication-runs`, `retcon-supersession`, `scoped-continuity`, and `textual-witnesses`. Both QA exporters generated matching 33-file lists and summary counts with one bounded graph and page; all 29 stable outputs matched after timestamps, output paths, byte-order marks, and line endings were normalized, leaving only the four expected runtime-owned settings/snapshot artifacts.
...
A source selects one or more works, a primary medium profile, and an allowlist of locator media. Singular release bindings serve simple evidence artifacts, while repeatable typed observations identify every manifestation, package, event, component, or offering inspected by composite evidence. Stable coverage entries state how much material is present; range endpoints must use identical fields, name the same canonical work through the medium's configured work-scope field, follow configured sort order, satisfy available structural data, and remain inside the target's resolved work scope. Central provenance assertions declare a controlled claim namespace, target top-level or globally stable nested records, resolve any dotted/indexed field path against that normalized target, snapshot the asserted value plus observation/effective timing, and attach supporting, contradicting, or contextual evidence through one or more globally stable locators. Each locator explicitly selects an allowed medium and is either one valid point or a correctly ordered range. Verified and inferred assertions require supporting evidence; disputed assertions require both supporting and contradicting evidence.
...
Authority profiles define accepted continuity memberships, continuity order, source-priority direction, which derivative-work relationships are comparable, and optional claim-specific rules. Reader positions use canonical `book` work IDs before work-local volume and chapter values. Rules can rank sources by source ID, role, medium, or evidence mode. A rule also applies to descendant claim namespaces; the highest explicit precedence wins and tied winners are rejected. Ordinary source priority remains the fallback. Under the default `lotm-adaptation-comparison` profile, source-scoped claims are preserved and authority varies appropriately among canonical content, dialogue, visual design, localization, and release metadata rather than forcing one artifact to lead every question.
