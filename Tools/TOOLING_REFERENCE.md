...
Last focused parity check: 2026-08-06 for V49 implementation. Python, PowerShell 7, and Windows PowerShell 5.1 load manifest schema 11, core pack version 40, occurrence schema 10, and hosted-identity schema 2 with ten selected packs, 138 controlled-value namespaces, 1,038 values, 124 enabled capabilities, 25 reconciliation target types, and 70 provenance subject types. The focused hosting suite composes core-only, foundation, narrative, simulation, compute, and combined pack sets and proves exact vocabulary isolation. Its combined fixture uses eight carriers, six bindings, ten occupancies, and three transitions; proves direct and transitive paths, child movement between parents, paired cross-track boundaries, reachable occupancy without direct-occupancy promotion, dormant co-residence, and co-control; rejects 46 invalid configurations and 14 invalid queries; and exercises 128 carriers, 128 occupancies, and a 127-binding chain. All three runtimes emitted matching semantic summaries.
...
Last compatibility check: 2026-08-06 for V49 implementation. The `full-release` profile passed all six registered checks in 189.0 seconds. Visualization matched 15 nodes and 121 relationships; QA matched 16 notes, 121 relationships, 71 data references, one bounded graph, two bounded pages, and all 34 normalized files; all 12 root-discovery launches passed; artifact lifecycle rejected six unsafe destinations; the isolated extraction copied 228 files and passed six portable suites in all three runtimes; and all three renderers produced the same nonblank `298269`-byte SVG with identical SHA-256 hashes. Redirected output was removed and canonical outputs remained unchanged.
...
Last parity check: 2026-08-06 for V49 implementation. The `baseline` profile passed all sixteen registered suites in Python, PowerShell 7, and Windows PowerShell 5.1 with matching suite IDs, statuses, and canonicalized semantic summaries. Exact-final runtimes were 51.7, 220.3, and 394.1 seconds. The baseline verifies the reviewed project identity, ten selected packs, 124 enabled and nine disabled capabilities, 25 reconciliation target types, 70 provenance subject types, exact registry schemas/counts and typed semantic declarations including four state profiles and 18 state-kind mappings, provider and object closure, deterministic repeated loads, disabled-capability absence, and fourteen invalid cross-registry compositions. Hosting conformance includes six exact pack compositions, 22 service assertions, 46 malformed registries, 14 invalid queries, and generated 128-carrier, 128-occupancy, and 127-binding probes. The `full-release` compatibility profile passed all six checks in 189.0 seconds, preserving 15 visualization nodes, 121 relationships, 16 QA notes, 34 normalized QA files, 12 root launches, artifact safety, 228-file isolated framework extraction, and identical nonblank rendering across runtimes. Source, entity, provenance, hosting, interpretation, and project composition remain outside the eight-suite `fast` profile because their repeated dependency composition is materially expensive.
