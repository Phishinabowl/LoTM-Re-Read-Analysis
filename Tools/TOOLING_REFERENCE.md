...
Last parity check: 2026-07-31. Python, PowerShell 7, and Windows PowerShell 5.1 loaded manifest schema 3, schema-pack registry schema 2, six selected packs (`core` 7, `narrative-media` 10, `narrative-publishing` 6, `narrative-screen-audio` 7, `narrative-adaptation` 6, and `narrative-distribution` 8), taxonomy schema 2, resource schema 1, and source schema 11. The current LoTM source registry matched at three media modalities, one cultural form, three release forms, four container formats, three medium profiles with explicit work-scope fields and one volume-catalog structural validator, four work-group types, three work groups, two continuities, one continuity relationship, two authority profiles with ten precedence-bearing claim-authority rules, six works, zero registered segments, content groups, or numbering schemes, two total ordering schemes, eight verified Book 1 volume ranges, five work relationships, zero adaptation mappings, six manifestations, zero manifestation relationships or segment mappings, two manifestation-scoped release components, and zero component relationships, release packages, runs, territories, platforms, release events, catalog placements, platform offerings, identifier schemes, external identifiers, observations, coverage entries, or provenance assertions. It also matched at eight evidence sources with controlled evidence modes and explicit locator-media allowlists, fourteen source aliases, two source relationships, four source-resource bindings, and twelve work aliases. A populated v11 fixture passed all runtimes with a non-lexical total segment order, segment-anchored range positions, declared partial coverage, locator-level evidence mode, and an inherited explainable authority decision. Nine negative fixtures consistently rejected an undeclared locator mode, a point outside declared coverage, an unknown segment anchor, a reversed temporal window, a range spanning disjoint coverage, a locator outside manifestation segment scope, tied mode-specific authority winners, a range crossing canonical works, and an unanchored locator that tried to widen complete segment coverage to its containing work. All runtimes resolved 94 declared capabilities, 85 available capabilities, 85 explicitly enabled capabilities, and 515 controlled-value ownership records. The nine planned unavailable capabilities are `editorial-assembly`, `entity-incarnations`, `live-performance-productions`, `migration-services`, `performance-events`, `publication-runs`, `retcon-supersession`, `scoped-continuity`, and `textual-witnesses`. Both QA exporters generated matching 33-file lists and summary counts with one bounded graph and page; all 29 stable outputs matched after timestamps, output paths, byte-order marks, and line endings were normalized, leaving only the four expected runtime-owned settings/snapshot artifacts.
