...
Last focused parity check: 2026-08-05. Python, PowerShell 7, and Windows PowerShell 5.1 load manifest schema 10, schema-pack selection registry schema 2, schema-4 pack files, eight selected packs (`core` 36, `narrative-media` 24, `narrative-publishing` 10, `narrative-screen-audio` 11, `narrative-adaptation` 10, `narrative-distribution` 14, `narrative-production` 4, and `narrative-shared-universe` 9), taxonomy schema 2, resource schema 1, source schema 18, entity schema 4, reconciliation schema 4, chronology schema 2, occurrence schema 9, interpretation schema 1, provenance schema 3, and lookup-key schema 1 with Unicode 16.0.0. V45 adds stable candidate structures, typed canonical and deferred-claim membership, interpretation-local relations, comparison sets, conservative unresolved decisions, and provenance-addressable interpretation records without mutating canonical graphs. The current composition contains 131 atomic controlled-value namespaces, 1,011 values, four state profiles, 18 state-kind mappings, 121 enabled capabilities, and 65 provenance subject types. The LoTM interpretation instance intentionally contains only the seven relation definitions and no fabricated candidate structures. Focused interpretation conformance passes 36 invalid configurations, eight invalid queries, and a generated 128-member/127-relation probe; provenance validates 21 composed assertions including the circular interpretation/claim load-order probe.
...
Last compatibility check: 2026-08-05 for V45 implementation. The `full-release` profile passed all six registered checks in 172.9 seconds. Visualization matched 15 nodes and 121 relationships; QA matched 16 notes, 121 relationships, 71 data references, one bounded graph, two bounded pages, and all 34 normalized files; all 12 root-discovery launches passed; artifact lifecycle rejected six unsafe destinations; the isolated extraction copied 217 files and passed six portable suites in all three runtimes; and all three renderers produced the same nonblank `298269`-byte SVG with identical SHA-256 hashes. Redirected output was removed and canonical outputs remained unchanged.
...
Last parity check: 2026-08-05 for V45 implementation. The `baseline` profile passed all fifteen registered suites in Python, PowerShell 7, and Windows PowerShell 5.1 with matching suite IDs, statuses, and canonicalized semantic summaries; the concurrent three-runtime run completed in 340.4 seconds. The baseline verifies the reviewed project identity, eight selected packs, 121 enabled and nine disabled capabilities, 24 reconciliation target types, 65 provenance subject types, exact registry schemas/counts and typed semantic declarations including four state profiles and 18 state-kind mappings, provider and object closure, deterministic repeated loads, disabled-capability absence, and twelve invalid cross-registry compositions. Structural-interpretation conformance includes three candidate structures, seven members, four local relations, three comparison modes, 36 invalid configurations, eight invalid queries, and a generated 128-member/127-relation probe. The `full-release` compatibility profile passed all six checks in 172.9 seconds, preserving 15 visualization nodes, 121 relationships, 16 QA notes, 34 normalized QA files, 12 root launches, artifact safety, 217-file isolated framework extraction, and identical nonblank rendering across runtimes. Source, entity, provenance, interpretation, and project composition remain outside the eight-suite `fast` profile because their repeated dependency composition is materially expensive.
