...
| `Project_Config/sources.yaml` | Source registry | `Tools/source_config.py`, `Tools/Source-Config.ps1`, and future validation, content-index, visualization, editor, and migration services | Maintainers through reviewed edits; future source editors through the mutation service | Instantiates media facets, works, structural segments and mixed-member content groups, numbering/orderings, multi-input adaptation mappings, segment-scoped manifestations and version mappings, package-scoped components and lineage, release packages/phased runs/events, territories, localized title variants and platform catalogs/offerings, structured time, external identifiers, precise composite evidence coverage, assertion provenance, relationships, and resource bindings. | A media facet, work, content group, numbering/ordering, manifestation/mapping, component/package/run/release, territory/platform record, identifier, source coverage, provenance assertion, or binding changes. |
...
Last parity check: 2026-07-31. Python, PowerShell 7, and Windows PowerShell 5.1 loaded manifest schema 3, schema-pack registry schema 2, six selected packs (`core` 3, `narrative-media` 6, `narrative-publishing` 3, `narrative-screen-audio` 4, `narrative-adaptation` 3, and `narrative-distribution` 4), taxonomy schema 2, resource schema 1, and source schema 7. The current LoTM source registry matched at three media modalities, one cultural form, three release forms, four container formats, three medium profiles, four work-group types, three work groups, two continuities, one continuity relationship, two authority profiles, six works, zero registered segments, content groups, or numbering schemes, two total ordering schemes, eight verified Book 1 volume ranges, five work relationships, zero adaptation mappings, six manifestations, zero manifestation relationships or segment mappings, two manifestation-scoped release components, and zero component relationships, release packages, runs, territories, platforms, release events, catalog placements, platform offerings, identifier schemes, external identifiers, or provenance assertions. It also matched at eight single-work evidence sources using the composite `work_ids` and typed coverage contracts, fourteen source aliases, two source relationships, four source-resource bindings, and twelve work aliases. A populated v7 fixture passed all runtimes with a mixed work/segment content group, manifestation mapping, revised component lineage, a two-work package/source with precise group coverage, a two-phase cadence-changing run, a localized provider title variant, and a field-addressable evidence-backed provenance assertion. Negative fixtures consistently rejected duplicate mixed membership, invalid phase partitioning, and missing provenance evidence. All runtimes resolved 71 declared capabilities, 62 available capabilities, 62 explicitly enabled capabilities, and 480 controlled-value ownership records. The nine planned unavailable capabilities are `editorial-assembly`, `entity-incarnations`, `live-performance-productions`, `migration-services`, `performance-events`, `publication-runs`, `retcon-supersession`, `scoped-continuity`, and `textual-witnesses`. Both QA exporters generated matching 33-file lists and summary counts with one bounded graph and page; stable outputs matched after timestamps, output paths, and line endings were normalized, leaving only the four expected runtime-owned settings/snapshot artifacts.
...
Segments provide stable configurable parts beneath a work. Their single parent owns structural containment; `content_groups` separately collect typed work and segment members into arcs, sagas, crossovers, publication units, collections, or reading paths and may use an exact mixed-target ordering scheme. Numbering schemes assign display numbers and aliases without implying sequence, while named total or partial ordering schemes can order works or segments by release, publication, story, production, or recommendation without overwriting another order. Partial schemes use acyclic predecessor references and allow concurrency. Adaptation mappings connect one or more basis works and optional registered segments to a target using controlled correspondence and basis roles. Manifestations may cover a whole work or selected segments, and mappings between related manifestations describe retained, omitted, added, altered, replaced, reordered, combined, or split material. Release components may originate in a manifestation or exist through package membership; typed component relationships preserve revision, translation, dub, and derivation lineage. Commercial packages, phased release runs, concrete events, and platform offerings own bundled material, schedules, launches, segment scope, and structured availability. Localized title variants distinguish title purpose, lifecycle, primary display, and romanization; catalog placements target works, segments, content groups, manifestations, or packages without redefining canonical hierarchy. Hierarchical territories and external identifier schemes preserve regional and provider identity separately from internal stable IDs. A source selects one or more works and a medium profile, may bind to the manifestation, package, event, components, or offering it observes, and defines a concrete evidence role, typed coverage, containers, comparison group, priority, aliases, evidence modes, and optional resource bindings. Central provenance assertions attach supporting, contradicting, or contextual source evidence to registry records or individual fields.
...
The paired source loaders reject malformed IDs, invalid position/citation definitions, unknown media-facet/group/continuity/profile/work/segment/medium/container/resource/source references, incompatible cultural forms and modalities, invalid structural or mixed-member group, territory, or catalog nesting and cycles, duplicate numbering or ordering targets, labels, or ordinals, missing or duplicate ordered-group ordinals, malformed or non-contiguous verified volume catalogs, unsupported roles/statuses/policies/mapping/title/coverage types, invalid manifestation mapping shapes, orphaned package-only components, malformed component lineage, release-run phase partitions, cadence, or exceptions, out-of-scope manifestation/package/run/event/offering segments, invalid structured times, composite source coverage outside declared works, invalid provenance targets/evidence/field paths, duplicate external identifiers, duplicate aliases, malformed inverse-type definitions, invalid or self-referential typed relationships, escaping or out-of-placement resource paths, and missing required bindings.
