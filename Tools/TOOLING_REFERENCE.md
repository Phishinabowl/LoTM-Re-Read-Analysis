...
Last parity check: 2026-08-05. Python, PowerShell 7, and Windows PowerShell 5.1 loaded manifest schema 9, schema-pack selection registry schema 2, schema-3 pack files, eight selected packs (`core` 29, `narrative-media` 21, `narrative-publishing` 10, `narrative-screen-audio` 11, `narrative-adaptation` 10, `narrative-distribution` 14, `narrative-production` 4, and `narrative-shared-universe` 9), taxonomy schema 2, resource schema 1, source schema 18, entity schema 4, reconciliation schema 4, provenance schema 3, chronology schema 1, occurrence schema 4, and lookup-key schema 1 with Unicode 16.0.0. V38 composes seven typed semantic declaration families, rejects the eight replaced delimiter-composed namespaces, preserves otherwise ambiguous member identities, and separates proposed effects from fail-closed authorization. The current composition contains 117 atomic controlled-value namespaces, 911 values, nine transition profiles, four outcome incompatibilities, four effect-target compatibilities, four rule-effect compatibilities, four effect policies, one effect incompatibility, and nine state-change profiles. The LoTM occurrence instance contains only `main`; no loop data is fabricated. All fourteen aggregate suites, including 68 occurrence query/evaluation assertions and 67 malformed occurrence cases, pass with matching semantic summaries in all three runtimes. Mixed selected-plus-indeterminate evaluation retains diagnostic proposals but authorizes nothing.
...
Last parity check: 2026-08-05 for V38 pressure testing. The `baseline` profile passed all fourteen registered suites in Python, PowerShell 7, and Windows PowerShell 5.1 with matching suite IDs, statuses, and canonicalized semantic summaries. Measured local runtimes were 41.7 seconds, 162.2 seconds, and 247.3 seconds respectively. The baseline verifies the reviewed project identity, eight selected packs, 114 enabled and nine disabled capabilities, 24 reconciliation target types, 53 provenance subject types, exact registry schemas/counts and typed semantic declarations, provider and object closure, deterministic repeated loads, disabled-capability absence, and seven invalid cross-registry compositions. Schema-pack pressure includes 64 generated typed declarations; occurrence conformance includes 68 query/evaluation assertions and 67 malformed cases. Source, entity, provenance, and project composition remain outside the eight-suite `fast` profile because their repeated dependency composition is materially expensive.
