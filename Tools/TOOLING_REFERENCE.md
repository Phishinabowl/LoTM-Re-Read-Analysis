...
Last parity check: 2026-08-05 for V39 pressure-test closure. The `baseline` profile passed all fourteen registered suites in Python, PowerShell 7, and Windows PowerShell 5.1 with matching suite IDs, statuses, and canonicalized semantic summaries. Measured local runtimes were 58.9 seconds, 216.9 seconds, and 331.8 seconds respectively. The baseline verifies the reviewed project identity, eight selected packs, 115 enabled and nine disabled capabilities, 24 reconciliation target types, 54 provenance subject types, exact registry schemas/counts and typed semantic declarations, provider and object closure, deterministic repeated loads, disabled-capability absence, and seven invalid cross-registry compositions. Schema-pack pressure includes 64 generated typed declarations; occurrence conformance includes 72 query/evaluation assertions, 87 malformed cases, and 128 generated cardinality records; provenance conformance includes 14 fixture assertions and five authority vectors. Source, entity, provenance, and project composition remain outside the eight-suite `fast` profile because their repeated dependency composition is materially expensive.
