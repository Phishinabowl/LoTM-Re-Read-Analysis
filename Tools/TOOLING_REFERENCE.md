...
Last parity check: 2026-08-02 after the Phase 3 runtime/module migration. The `baseline` profile passed all seven registered suites in Python, PowerShell 7, and Windows PowerShell 5.1 with matching suite IDs, statuses, and canonicalized semantic summaries. Measured local runtimes were 6.964 seconds, 69.525 seconds, and 111.814 seconds respectively. Compared with the pre-migration measurements of approximately 7, 69, and 121 seconds, module ownership produced no material aggregate speedup; the PowerShell cost remains dominated by isolated child-process startup and repeated suite initialization. The `project-root` suite passed all eleven precedence, invalid-input, marker, failure, and caller-location vectors from the repository root, `Tools/`, a nested framework directory, and an unrelated directory in all three runtimes. Ruff, both PowerShell formatter editions, actionlint, and diff validation passed. Three-runtime Visualization validation matched at 15 nodes and 121 relationships with no class or layout issues. Redirected QA generation matched summaries (`notes=16`, `relationships=121`, `data_references=71`, one bounded graph, and two bounded pages), complete 34-file inventories, and every normalized Markdown, Mermaid, report, settings, and snapshot artifact after only generated-time, newline, redirected-root, and JSON-format normalization. Clean regeneration removed stale and unrequested bounded folders; all runtimes rejected repository-root and outside-repository destinations; scoped cleanup preserved unrelated temporary data and removed only explicitly selected trees. Canonical Visualization and Obsidian outputs were not refreshed.
