...
| `Project_Config/sources.yaml` | Source registry | `Tools/source_config.py`, `Tools/Source-Config.ps1`, and future validation, content-index, visualization, editor, and migration services | Maintainers through reviewed edits; future source editors through the mutation service | Instantiates media facets, works, segments, total/partial orderings, adaptation mappings, manifestations, release components/events, platform catalogs/offerings, evidence sources, relationships, and resource bindings. | A media facet, work, segment, ordering, manifestation, release, platform record, source, or binding changes. |
...
Last parity check: 2026-07-31. Python, PowerShell 7, and Windows PowerShell 5.1 loaded manifest schema 3, schema-pack registry schema 2, six selected schema packs at pack schema 2, taxonomy schema 2, resource schema 1, and source schema 4. The source registry matched at three media modalities, one cultural form, three release forms, four container formats, three medium profiles, four work-group types, three work groups, two continuities, one continuity relationship, two authority profiles, six works, zero registered segments, two total ordering schemes, eight verified Book 1 volume ranges, five work relationships, zero segment adaptation mappings, six manifestations, zero manifestation relationships, two release components, zero registered territories, platforms, release events, catalog placements, or platform offerings, eight evidence sources, fourteen source aliases, two source relationships, four source-resource bindings, and twelve work aliases. All runtimes resolved 52 declared capabilities, 46 available capabilities, 46 explicitly enabled capabilities, and 348 controlled-value ownership records. The six planned unavailable capabilities are `migration-services`, `scoped-continuity`, `entity-incarnations`, `retcon-supersession`, `textual-witnesses`, and `editorial-assembly`. Both QA exporters generated matching 28-file lists and summary counts; the 25 stable outputs matched after timestamps and line endings were normalized, leaving only the three expected runtime-specific refresh artifacts.
...
Segments provide stable configurable parts beneath a work, while named total or partial ordering schemes can order works or segments by release, publication, story, production, or recommendation without overwriting another order. Partial schemes use acyclic predecessor references and allow concurrency. Adaptation mappings connect works and optional registered segments using controlled correspondence and basis roles. Manifestations own editions, translations, cuts, recuts, remasters, and builds; release components/events and platform offerings own tracks, launches, and availability. Catalog placements preserve provider presentation without redefining canonical work hierarchy. A source selects a work and medium profile, may bind to the manifestation or release material it observes, and defines a concrete evidence role, containers, comparison group, priority, aliases, evidence modes, and optional resource bindings.
