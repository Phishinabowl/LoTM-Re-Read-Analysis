...
class SourceRegistry:
    ...
    def provenance_targets(self) -> dict[str, dict[str, object]]:
        nested = {
            "content-group-member": (member for group in self.content_groups.values() for member in group.members),
            "localized-title": (
                title
                for owner in (
                    *self.works.values(),
            ...
            "release-run-phase": (phase for run in self.release_runs.values() for phase in run.phases),
            "source-coverage": (coverage for source in self.sources.values() for coverage in source.coverage),
            "source-observation": (
                observation for source in self.sources.values() for observation in source.observations
            ),
            ...
            "authority-rule": (
                rule for profile in self.authority_profiles.values() for rule in profile.claim_authority_rules
            ),
        }
        ...
        for subject_type, records in nested.items():
            ...
            if len(record_ids) != len(set(record_ids)):
                raise ValueError(f"Source registry {subject_type} IDs must be unique across owners.")
        return {
            ...
            "manifestation-relationship": {item.id: item for item in self.manifestation_relationships},
            "manifestation-segment-mapping": {item.id: item for item in self.manifestation_segment_mappings},
            "release-component": dict(self.release_components),
            "release-component-relationship": {item.id: item for item in self.release_component_relationships},
            ...
            **{subject_type: {record.id: record for record in records} for subject_type, records in nested.items()},
        }
    ...
    def resolve_source_id(self, value: str) -> str | None:
        ...
        if normalized in {self.lookup_keys.normalize(source_id) for source_id in self.sources}:
            return next(source_id for source_id in self.sources if self.lookup_keys.normalize(source_id) == normalized)
    ...
    def highest_precedence_scopes(self, scope_ids: tuple[str, ...] | list[str]) -> tuple[ApplicabilityScope, ...]:
        if not scope_ids:
            raise ValueError("At least one applicability scope ID is required.")
        unknown = set(scope_ids) - set(self.applicability_scopes)
        if unknown:
            raise ValueError("Unknown applicability scope IDs: " + ", ".join(sorted(unknown)) + ".")
        ...
        *,
        ...
        if territory_id is not None:
            if not isinstance(territory_id, str):
                raise ValueError("Applicability territory ID must be a string or None.")
        ...
        for scope in self.applicability_scopes.values():
            ...
            if target_match is None:
                continue
            territory_match = self._applicability_territory_match(scope, territory_id)
            ...
            matches.append(
                ApplicabilityScopeMatch(
                    scope_id=scope.id,
                    outcome=("indeterminate" if temporal_match_is_indeterminate(temporal_match) else "applicable"),
        ...
        applicable_matches = [item for item in matches if item.outcome == "applicable"]
        highest = applicable_matches[0].precedence if applicable_matches else None
        winners = tuple(item.scope_id for item in applicable_matches if item.precedence == highest)
        return ApplicabilityDecision(
            target_type=target_type,
            target_id=target_id,
            territory_id=territory_id,
            effective_at=effective_label,
            matching_scope_ids=tuple(item.scope_id for item in applicable_matches),
            indeterminate_scope_ids=tuple(item.scope_id for item in matches if item.outcome == "indeterminate"),
    ...
    def _validate_applicability_query_target(self, target_type: str, target_id: str) -> None:
        targets = {
            "work": self.works,
            "segment": self.segments,
            "content-group": self.content_groups,
        ...
        if target_id not in targets[target_type]:
            raise ValueError(f"Unknown {target_type} applicability target `{target_id}`.")
    ...
    def _target_work_ids(self, target_type: str, target_id: str) -> set[str]:
        ...
        if target_type == "content-group":
            ...
            for member in self.content_groups[target_id].members:
                result.update(self._target_work_ids(member.target_type, member.target_id))
            return result
        if target_type == "work-relationship":
            item = next(item for item in self.work_relationships if item.id == target_id)
            return {item.source_work_id}
        if target_type == "adaptation-mapping":
            item = next(item for item in self.adaptation_mappings if item.id == target_id)
        ...
        if target_type == "release-package":
            ...
            for component_id in package.release_component_ids:
                result.update(self._target_work_ids("release-component", component_id))
            return result
        return set()
    ...
    def _target_is_within_segment(self, target_type: str, target_id: str, segment_id: str) -> bool:
        if target_type == "segment":
            return self._segment_is_within(target_id, segment_id)
        if target_type == "content-group":
            members = self.content_groups[target_id].members
            return bool(members) and all(
                self._target_is_within_segment(member.target_type, member.target_id, segment_id) for member in members
            )
        if target_type == "adaptation-mapping":
            mapping = next(item for item in self.adaptation_mappings if item.id == target_id)
            return bool(mapping.target_segment_ids) and all(
                self._segment_is_within(item, segment_id) for item in mapping.target_segment_ids
            )
        if target_type == "manifestation":
            segment_ids = self.manifestations[target_id].segment_ids
            return bool(segment_ids) and all(self._segment_is_within(item, segment_id) for item in segment_ids)
        if target_type == "release-component":
            component = self.release_components[target_id]
            if component.segment_ids:
                return all(self._segment_is_within(item, segment_id) for item in component.segment_ids)
            if component.manifestation_id is not None:
                return self._target_is_within_segment("manifestation", component.manifestation_id, segment_id)
        return False
    ...
    def _content_group_contains(
        ...
        for member in self.content_groups[group_id].members:
            if (
                self._applicability_target_match(
                    member.target_type,
                    member.target_id,
                    target_type,
                    target_id,
    ...
    def _applicability_target_match(
        ...
        if scope_type == "segment":
            return "contained" if self._target_is_within_segment(target_type, target_id, scope_id) else None
        if scope_type == "content-group":
            return "contained" if self._content_group_contains(scope_id, target_type, target_id, active) else None
        ...
        if scope_type == "release-package":
            ...
            if target_type == "manifestation" and target_id in package.manifestation_ids:
                return "contained"
            if target_type == "release-component" and target_id in package.release_component_ids:
                return "contained"
            if target_type == "segment" and target_id in package.segment_ids:
                return "contained"
        return None

    def _applicability_territory_match(self, scope: ApplicabilityScope, territory_id: str | None) -> str | None:
    ...
    def authority_decision(
        ...
        if evidence_mode is not None and evidence_mode not in source.evidence_modes:
            raise ValueError(f"Evidence mode `{evidence_mode}` is not declared by source `{source_id}`.")
        ...
        matches = [
            ...
            and authority_rule_matches_source(rule, source, evidence_mode, self.evidence_mode_ancestors)
        ]
        if not matches:
        ...
        winners = [rule for rule in matches if rule.precedence == winning_precedence]
        if len(winners) > 1:
            raise ValueError(
        ...
        evidence_mode_inherited = (
            evidence_mode is not None and bool(winner.evidence_modes) and evidence_mode not in winner.evidence_modes
        )
        return AuthorityDecision(
            source.id,
            evidence_mode,
    ...
    def authority_rank(
        ...
        return self.authority_decision(profile_id, claim_namespace, source_id, evidence_mode).rank
    ...
    def compare_authority(
        ...
        decisions = tuple(
            AuthorityCandidateDecision(
                candidate_id,
                None,
                self.authority_decision(profile_id, claim_namespace, source_id, evidence_mode),
            )
            for candidate_id, source_id, evidence_mode in candidates
        )
        comparison_groups = {self.sources[item.decision.source_id].comparison_group for item in decisions}
        if len(comparison_groups) != 1:
            return AuthorityComparison("incomparable", profile_id, claim_namespace, None, (), decisions)
        profile = self.authority_profiles[profile_id]
        ranks = [item.decision.rank for item in decisions]
        best_rank = min(ranks) if profile.source_priority_order == "ascending" else max(ranks)
        winners = tuple(item.candidate_id for item in decisions if item.decision.rank == best_rank)
...
def authority_rule_matches_source(
    ...
    return (
        ...
        and (not rule.evidence_modes or any(mode in rule.evidence_modes for mode in applicable_modes))
    )
...
def optional_string(mapping: dict, key: str, context: str) -> str | None:
    ...
    if value is None:
        return None
    if not isinstance(value, str) or not value.strip():
        raise ValueError(f"Source registry `{context}.{key}` must be a non-empty string when present.")
    return value.strip()


def validate_language_tag(value: str, context: str) -> None:
    if not LANGUAGE_TAG_PATTERN.fullmatch(value):
        raise ValueError(f"Source registry `{context}` must be a BCP-47-style language tag: {value}")
...
def parse_localized_titles(
    ...
    if not isinstance(raw_titles, list):
        raise ValueError(f"Source registry `{context}.localized_titles` must be a list.")
    ...
    for index, raw_title in enumerate(raw_titles):
        ...
        if title_id in seen_ids:
            raise ValueError(f"Source registry `{context}.localized_titles` repeats ID `{title_id}`.")
        ...
        romanization_scheme = optional_string(value, "romanization_scheme", title_context)
        scope = (
        ...
        valid_window = parse_temporal_window(value, "valid_window", title_context, schema_packs)
        if any(temporal_windows_overlap(valid_window, existing) for existing in scope_windows.get(scope, [])):
            raise ValueError(
                f"Source registry `{context}.localized_titles` has overlapping validity windows in one locale scope."
            )
...
def validate_id(value: str, context: str) -> None:
    if not STABLE_ID_PATTERN.fullmatch(value):
        raise ValueError(f"Source registry `{context}` must be a lowercase kebab-case stable ID: {value}")
...
def validate_position_values(
    ...
    for field_id, value in values.items():
        ...
        if not valid:
            raise ValueError(f"Source registry `{context}.{field_id}` must match position field type `{field_type}`.")


def validate_field_id(value: str, context: str) -> None:
    if not FIELD_ID_PATTERN.fullmatch(value):
        raise ValueError(f"Source registry `{context}` must be a lowercase snake_case field ID: {value}")
...
def validate_pack_values(
    ...
    if not allowed:
        raise ValueError(
            f"Selected schema packs do not provide controlled namespace `{namespace}` required by `{context}`."
        )
...
def controlled_value_ancestors(schema_packs: SchemaPackRegistry, namespace: str) -> dict[str, tuple[str, ...]]:
    ...
    for field_id in medium.sort_fields:
        ...
        if (
            ...
            if ordering_schemes is None:
                raise ValueError(f"Source registry `{context}` requires ordering schemes for segment comparison.")
            scheme_field = validation.ordering_scheme_field
            if left[scheme_field] != right[scheme_field]:
                raise ValueError(f"Source registry `{context}` range endpoints must use the same ordering scheme.")
            scheme = ordering_schemes[str(left[scheme_field])]
            entry_ordinals = {entry.target_id: entry.ordinal for entry in scheme.entries}
...
def validate_structural_position(
    ...
    if validation.strategy == "work-segment-ordering":
        ...
        if segment_id not in segments:
            raise ValueError(
                f"Source registry `{context}.{validation.segment_field}` references unknown segment `{segment_id}`."
            )
        if segments[segment_id].work_id != work_id:
            raise ValueError(f"Source registry `{context}` assigns segment `{segment_id}` to the wrong work.")
        ...
        if scheme.ordering_mode != "total":
            raise ValueError(f"Source registry `{context}` requires a total ordering scheme.")
        if not any(entry.target_type == "segment" and entry.target_id == segment_id for entry in scheme.entries):
            raise ValueError(
                f"Source registry `{context}` segment `{segment_id}` is absent from ordering scheme `{scheme_id}`."
            )
        return
    if validation.strategy != "work-volume-catalog":
        raise ValueError(
            f"Source registry `{context}` uses unsupported structural validation strategy `{validation.strategy}`."
        )
    if work.volume_catalog_status != "verified":
        return
    ordinal = position[validation.ordinal_field]
    matching_volumes = [volume for volume in work.volumes if volume.chapter_start <= ordinal <= volume.chapter_end]
    ...
    if validation.partition_field in position and all(
        volume.number != position[validation.partition_field] for volume in matching_volumes
    ):
        raise ValueError(
...
def validate_source_position(
    ...
    if not position:
        raise ValueError(f"Source registry `{context}` must be a non-empty mapping.")
    unknown_fields = set(position) - set(medium.fields)
    if unknown_fields:
        raise ValueError(
            f"Source registry `{context}` references unknown position fields: {', '.join(sorted(unknown_fields))}."
        )
    missing_fields = set(medium.required_fields) - set(position)
    if missing_fields:
        raise ValueError(
            f"Source registry `{context}` omits required position fields: {', '.join(sorted(missing_fields))}."
        )
    ...
    if position[scope_field] not in source_work_ids:
        raise ValueError(f"Source registry `{context}` falls outside the evidence source work scope.")
    validate_structural_position(position, medium, works, segments, ordering_schemes, context)


def parse_lifecycle(mapping: dict, context: str) -> str:
    lifecycle = require_string(mapping, "lifecycle", context)
    if lifecycle not in LIFECYCLES:
        raise ValueError(f"Source registry `{context}.lifecycle` must be one of: {', '.join(sorted(LIFECYCLES))}.")
    return lifecycle
...
def parse_work(
    ...
    *,
    ...
    if medium_id not in mediums:
        raise ValueError(f"Source registry `{context}.medium_id` references unknown medium `{medium_id}`.")
    ...
    if release_form_id not in release_forms:
        raise ValueError(
            f"Source registry `{context}.release_form_id` references unknown release form `{release_form_id}`."
        )
    ...
    if not isinstance(raw_group_memberships, list) or not raw_group_memberships:
        raise ValueError(f"Source registry `{context}.group_memberships` must be a non-empty list.")
    ...
    for index, raw_membership in enumerate(raw_group_memberships):
        ...
        if group_id not in work_groups:
            raise ValueError(
                f"Source registry `{membership_context}.group_id` references unknown work group `{group_id}`."
            )
        if group_id in seen_groups:
            raise ValueError(f"Source registry `{context}` repeats work group `{group_id}`.")
        ...
        if ordered:
            ...
            raise ValueError(f"Source registry `{membership_context}.ordinal` is only valid for ordered work groups.")
    ...
    if not isinstance(raw_continuity_memberships, list) or not raw_continuity_memberships:
        raise ValueError(f"Source registry `{context}.continuity_memberships` must be a non-empty list.")
    ...
    for index, raw_membership in enumerate(raw_continuity_memberships):
        ...
        if continuity_id not in continuities:
            raise ValueError(
                f"Source registry `{membership_context}.continuity_id` references unknown continuity `{continuity_id}`."
            )
        if continuity_id in seen_continuities:
            raise ValueError(f"Source registry `{context}` repeats continuity `{continuity_id}`.")
    ...
    if volume_status == "verified" and not raw_volumes:
        raise ValueError(f"Source registry verified work `{work_id}` requires volume records.")
    ...
    for index, raw_volume in enumerate(raw_volumes):
        ...
        assert_allowed_keys(
            volume, {"id", "number", "label", "chapter_start", "chapter_end"}, f"Source registry `{volume_context}`"
        )
        volume_id = require_string(volume, "id", volume_context)
        validate_id(volume_id, f"{volume_context}.id")
        if volume_id in volume_ids:
            raise ValueError(f"Source registry `{volume_context}.id` duplicates `{volume_id}`.")
        ...
        for field_name, value in (
            ...
            if isinstance(value, bool) or not isinstance(value, int) or value < 1:
                raise ValueError(f"Source registry `{volume_context}.{field_name}` must be a positive integer.")
        if number in volume_numbers:
            raise ValueError(f"Source registry `{context}` duplicates volume number {number}.")
        volume_numbers.add(number)
        if chapter_end < chapter_start:
            raise ValueError(f"Source registry `{volume_context}` chapter range is reversed.")
        volumes.append(
            VolumeConfig(
                id=volume_id,
                number=number,
    ...
    if volume_status == "verified":
        for expected_number, volume in enumerate(sorted_volumes, start=1):
            if volume.number != expected_number:
                raise ValueError(f"Source registry `{context}` verified volume numbers must be contiguous from 1.")
        for previous, current in zip(sorted_volumes, sorted_volumes[1:]):
            if current.chapter_start != previous.chapter_end + 1:
                raise ValueError(
                    f"Source registry `{context}` verified chapter ranges must be contiguous and non-overlapping."
                )
    return WorkConfig(
        id=work_id,
        lifecycle=lifecycle,
...
def parse_medium(
    ...
    *,
    ...
    if not modality_ids:
        raise ValueError(f"Source registry `{context}.modality_ids` must not be empty.")
    ...
    work_scope_field = require_string(position, "work_scope_field", f"{context}.position")
    if work_scope_field not in fields or fields[work_scope_field] != "string":
        raise ValueError(
            f"Source registry `{context}.position.work_scope_field` must reference a configured string position field."
        )
    ...
    if work_scope_field not in required_fields:
        raise ValueError(
            f"Source registry `{context}.position.work_scope_field` must also be listed in required_fields."
        )
    ...
    if raw_structural_validation is not None:
        ...
        validation = require_mapping(raw_structural_validation, validation_context)
        assert_allowed_keys(
            validation,
        ...
        if strategy == "work-volume-catalog":
            partition_field = require_string(validation, "partition_field", validation_context)
            ordinal_field = require_string(validation, "ordinal_field", validation_context)
            configured_fields = (
                ("partition_field", partition_field, "integer"),
                ("ordinal_field", ordinal_field, "integer"),
            )
        elif strategy == "work-segment-ordering":
            segment_field = require_string(validation, "segment_field", validation_context)
            ordering_scheme_field = require_string(validation, "ordering_scheme_field", validation_context)
            configured_fields = (
            ...
            raise ValueError(
                f"Source registry `{validation_context}.strategy` is not implemented by this loader: `{strategy}`."
            )
        ...
        required_validation_fields = (
            {ordinal_field} if strategy == "work-volume-catalog" else {segment_field, ordering_scheme_field}
        )
        if ordered_field not in sort_fields:
            raise ValueError(
                f"Source registry `{validation_context}` ordered field must also be listed in sort_fields."
            )
        if not required_validation_fields.issubset(required_fields):
            raise ValueError(
                f"Source registry `{validation_context}` structural fields must be listed in required_fields."
            )
    ...
    if not isinstance(raw_formats, list) or not raw_formats:
        raise ValueError(f"Source registry `{context}.position.citation_formats` must be a non-empty list.")
    ...
    for index, raw_format in enumerate(raw_formats):
        ...
        if format_id in seen_format_ids:
            raise ValueError(f"Source registry `{format_context}.id` duplicates `{format_id}`.")
        ...
        if placeholders != set(citation_required):
            raise ValueError(f"Source registry `{format_context}` template placeholders must match `required_fields`.")
...
def resolve_resource_binding(
    ...
    if resource_type_id not in resources.types:
        raise ValueError(
            f"Source registry `{context}.resource_type_id` references unknown resource type `{resource_type_id}`."
        )
    ...
    if root_id not in roots:
        raise ValueError(f"Source registry `{context}.root_id` references unknown resource root `{root_id}`.")
    allowed_placements = tuple(
        placement for placement in resources.types[resource_type_id].placements if placement.root_id == root_id
    )
    ...
    if relative_path.is_absolute():
        raise ValueError(f"Source registry `{context}.relative_path` must be relative: {relative_path_value}")
    ...
    if path != root_path and root_path not in path.parents:
        raise ValueError(
            f"Source registry `{context}.relative_path` escapes resource root `{root_id}`: {relative_path_value}"
        )
    if not any(
        path == placement.path.resolve() or placement.path.resolve() in path.parents for placement in allowed_placements
    ):
        raise ValueError(
...
def assert_source_registry_shapes(registry: dict) -> None:
    ...
    nested_shapes = (
        ...
        (
            "sources",
            "coverage",
            {"id", "target_type", "target_id", "coverage_type", "medium_id", "evidence_modes", "position_ranges"},
        ),
        ...
        for parent_id, parent in parent_records:
            ...
            for index, child in enumerate(children):
                if isinstance(child, dict):
                    assert_allowed_keys(
                        child, allowed_fields, f"Source registry `{registry_name}.{parent_id}.{field_name}[{index}]`"
                    )
                        ...
                        for range_index, position_range in enumerate(ranges):
                            if isinstance(position_range, dict):
                                assert_allowed_keys(
                                    position_range,
                                    {"id", "start", "end"},
                                    f"Source registry `{registry_name}.{parent_id}.coverage[{index}]"
                                    f".position_ranges[{range_index}]`",
                                )
...
def load_source_registry(
    ...
    allowed_source_roles = set(schema_packs.allowed_values("source.source-role"))
    ...
    claim_namespace_ancestors = controlled_value_ancestors(schema_packs, "provenance.claim-namespace")
    if not claim_namespace_ancestors:
        raise ValueError("Selected schema packs do not provide controlled namespace `provenance.claim-namespace`.")
    evidence_mode_ancestors = controlled_value_ancestors(schema_packs, "provenance.evidence-mode")
    if not evidence_mode_ancestors:
        raise ValueError("Selected schema packs do not provide controlled namespace `provenance.evidence-mode`.")
    data = load_yaml_file(
        project.sources_registry, "source registry", expected_schema_version=SUPPORTED_SOURCE_SCHEMA_VERSION
    )
    ...
    if schema_version != SUPPORTED_SOURCE_SCHEMA_VERSION:
        raise ValueError(
            f"Unsupported source schema_version {schema_version!r}; expected {SUPPORTED_SOURCE_SCHEMA_VERSION}."
        )
    media_modalities = parse_labeled_registry(registry.get("media_modalities"), "media_modalities")
    validate_pack_values(
        schema_packs,
    ...
    raw_cultural_forms = require_mapping(registry.get("cultural_forms"), "cultural_forms")
    cultural_forms: dict[str, CulturalFormConfig] = {}
    for cultural_form_id, raw_cultural_form in raw_cultural_forms.items():
        ...
        if modality_id not in media_modalities:
            raise ValueError(
                f"Source registry `{context}.modality_id` references unknown media modality `{modality_id}`."
            )
    ...
    release_forms = parse_labeled_registry(registry.get("release_forms"), "release_forms")
    validate_pack_values(
        schema_packs,
        "source.release-form",
        release_forms,
    ...
    container_formats = parse_labeled_registry(registry.get("container_formats"), "container_formats")
    validate_pack_values(
        schema_packs,
    ...
    validate_pack_values(schema_packs, "source.medium", mediums, "mediums")

    raw_group_types = require_mapping(registry.get("work_group_types"), "work_group_types")
    work_group_types: dict[str, WorkGroupTypeConfig] = {}
    ...
    for group_id, raw_group in raw_groups.items():
        ...
        assert_allowed_keys(
            group,
            {"lifecycle", "label", "short_label", "group_type", "parent_group_id"},
            f"Source registry `{context}`",
        )
        group_type = require_string(group, "group_type", context)
        if group_type not in work_group_types:
            raise ValueError(
                f"Source registry `{context}.group_type` references unknown work group type `{group_type}`."
            )
    ...
    def visit_group(group_id: str, active: set[str]) -> None:
        if group_id in active:
            raise ValueError(f"Source registry contains a work-group parent cycle involving `{group_id}`.")
    ...
    continuity_id_keys = {lookup_keys.normalize(continuity_id) for continuity_id in raw_continuities}
    for continuity_id, raw_continuity in raw_continuities.items():
        ...
        assert_allowed_keys(
            continuity,
            {"lifecycle", "label", "short_label", "continuity_type", "aliases"},
            f"Source registry `{context}`",
        )
        ...
        for alias in aliases:
            ...
            if alias_key in continuity_aliases or alias_key in continuity_id_keys:
                raise ValueError(
                    f"Source registry continuity alias `{alias}` is duplicated or collides with a continuity ID."
                )
    ...
    if not isinstance(raw_continuity_relationships, list):
        raise ValueError("Source registry `continuity_relationships` must be a list.")
    ...
    for index, raw_relationship in enumerate(raw_continuity_relationships):
        ...
        assert_allowed_keys(
            relationship,
            {"id", "source_continuity_id", "target_continuity_id", "relationship_type", "status"},
            f"Source registry `{context}`",
        )
        ...
        if relationship_id in seen_relationship_ids:
            raise ValueError(f"Source registry relationship ID `{relationship_id}` is duplicated.")
        seen_relationship_ids.add(relationship_id)
        source_id = require_string(relationship, "source_continuity_id", context)
        target_id = require_string(relationship, "target_continuity_id", context)
        relationship_type = require_string(relationship, "relationship_type", context)
        if source_id not in continuities or target_id not in continuities:
            raise ValueError(f"Source registry `{context}` references an unknown continuity.")
        if source_id == target_id:
            raise ValueError(f"Source registry `{context}` cannot relate a continuity to itself.")
        if relationship_type not in continuity_relationship_types:
            raise ValueError(
                f"Source registry `{context}.relationship_type` references unknown type `{relationship_type}`."
            )
        status = require_string(relationship, "status", context)
        if status not in membership_statuses:
            raise ValueError(
                f"Source registry `{context}.status` must be one of: {', '.join(sorted(membership_statuses))}."
            )
        continuity_relationships.append(
            ContinuityRelationship(relationship_id, source_id, relationship_type, target_id, status)
        )

    raw_authority_profiles = require_mapping(registry.get("authority_profiles"), "authority_profiles")
    authority_profiles: dict[str, AuthorityProfile] = {}
    for profile_id, raw_profile in raw_authority_profiles.items():
        ...
        if len(set(continuity_order)) != len(continuity_order):
            raise ValueError(f"Source registry `{context}.continuity_order` contains duplicates.")
        ...
        accepted_statuses = require_string_list(profile, "accepted_membership_statuses", context)
        ...
        comparison_types = require_string_list(profile, "comparison_work_relationship_types", context)
        ...
        deviation_owner = require_string(profile, "derivative_deviation_owner", context)
        ...
        if not isinstance(raw_rules, list):
            raise ValueError(f"Source registry `{context}.claim_authority_rules` must be a list.")
        ...
        for rule_index, raw_rule in enumerate(raw_rules):
            ...
            assert_allowed_keys(
                ...
                f"Source registry `{rule_context}`",
            )
            rule_id = require_string(rule, "id", rule_context)
            validate_id(rule_id, f"{rule_context}.id")
            if rule_id in seen_rule_ids:
                raise ValueError(f"Source registry `{context}.claim_authority_rules` repeats ID `{rule_id}`.")
            seen_rule_ids.add(rule_id)
            claim_namespace = require_string(rule, "claim_namespace", rule_context)
            validate_pack_values(
                schema_packs,
            ...
            if isinstance(precedence, bool) or not isinstance(precedence, int) or precedence < 1:
                raise ValueError(f"Source registry `{rule_context}.precedence` must be a positive integer.")
            ...
            evidence_modes = require_string_list(rule, "evidence_modes", rule_context)
            if not any((source_ids, source_roles, medium_ids, evidence_modes)):
                raise ValueError(f"Source registry `{rule_context}` must declare at least one source selector.")
            validate_pack_values(
                schema_packs,
                "source.source-role",
                source_roles,
            ...
            if isinstance(rank, bool) or not isinstance(rank, int) or rank < 1:
                raise ValueError(f"Source registry `{rule_context}.rank` must be a positive integer.")
            rules.append(
                AuthorityRule(
                    rule_id,
                    claim_namespace,
        ...
        authority_profiles[profile_id] = AuthorityProfile(
            ...
            preserve_source_scoped_claims=require_bool(profile, "preserve_source_scoped_claims", context),
            claim_authority_rules=tuple(rules),
        )
    default_authority_profile_id = require_string(registry, "default_authority_profile_id", "root")
    ...
    for work in works.values():
        if work.parent_work_id is not None:
            if work.parent_work_id not in works:
                raise ValueError(
                    f"Source registry `works.{work.id}.parent_work_id` references unknown work `{work.parent_work_id}`."
                )
        ...
        for alias in work.aliases:
            ...
            if alias_key in work_aliases or alias_key in work_id_keys:
                raise ValueError(f"Source registry work alias `{alias}` is duplicated or collides with a work ID.")
    ...
    def visit_work(work_id: str, active: set[str]) -> None:
        if work_id in active:
            raise ValueError(f"Source registry contains a work-parent cycle involving `{work_id}`.")
    ...
    for segment_id, raw_segment in raw_segments.items():
        ...
        if work_id not in works:
            raise ValueError(f"Source registry `{context}.work_id` references unknown work `{work_id}`.")
        ...
        if ordinal is not None and (isinstance(ordinal, bool) or not isinstance(ordinal, int) or ordinal < 1):
            raise ValueError(f"Source registry `{context}.ordinal` must be a positive integer when present.")
    ...
    for segment in segments.values():
        ...
        if parent.work_id != segment.work_id:
            raise ValueError(f"Source registry segment `{segment.id}` and its parent must belong to the same work.")
    complete_segments: set[str] = set()

    def visit_segment(segment_id: str, active: set[str]) -> None:
        if segment_id in active:
            raise ValueError(f"Source registry contains a segment-parent cycle involving `{segment_id}`.")
    ...
    for segment in segments.values():
        ...
        work_segment_ids = {
            lookup_keys.normalize(item.id) for item in segments.values() if item.work_id == segment.work_id
        }
    ...
    raw_numbering_schemes = require_mapping(registry.get("numbering_schemes"), "numbering_schemes")
    numbering_schemes: dict[str, NumberingScheme] = {}
    for scheme_id, raw_scheme in raw_numbering_schemes.items():
        ...
        if scope_type == "none" and scope_id is not None:
            raise ValueError(f"Source registry `{context}.scope_id` must be omitted for none scope.")
        if scope_type == "work":
            if scope_id not in works:
                raise ValueError(f"Source registry `{context}.scope_id` references unknown work `{scope_id}`.")
            if target_type != "segment":
                raise ValueError(f"Source registry `{context}` work scope requires segment targets.")
        if scope_type == "work-group":
            if scope_id not in work_groups:
                raise ValueError(f"Source registry `{context}.scope_id` references unknown work group `{scope_id}`.")
            if target_type != "work":
                raise ValueError(f"Source registry `{context}` work-group scope requires work targets.")
        raw_entries = scheme.get("entries")
        if not isinstance(raw_entries, list) or not raw_entries:
            raise ValueError(f"Source registry `{context}.entries` must be a non-empty list.")
        ...
        for index, raw_entry in enumerate(raw_entries):
            ...
            if target_id not in targets:
                raise ValueError(
                    f"Source registry `{entry_context}.target_id` references unknown {target_type} `{target_id}`."
                )
            if scope_type == "work" and segments[target_id].work_id != scope_id:
                raise ValueError(f"Source registry `{entry_context}` target falls outside work scope `{scope_id}`.")
            if scope_type == "work-group" and not any(
                membership.group_id == scope_id for membership in works[target_id].group_memberships
            ):
                raise ValueError(
                    f"Source registry `{entry_context}` target falls outside work group scope `{scope_id}`."
                )
            display_number = require_string(entry, "display_number", entry_context)
            ...
            if target_id in seen_targets or seen_numbers.intersection(number_keys):
                raise ValueError(f"Source registry `{context}.entries` repeats a target or number.")
    ...
    raw_content_groups = require_mapping(registry.get("content_groups"), "content_groups")
    raw_ordering_schemes = require_mapping(registry.get("ordering_schemes"), "ordering_schemes")
    ordering_schemes: dict[str, OrderingScheme] = {}
    for scheme_id, raw_scheme in raw_ordering_schemes.items():
        ...
        if ordering_mode not in ORDERING_MODES:
            raise ValueError(
                f"Source registry `{context}.ordering_mode` must be one of: {', '.join(sorted(ORDERING_MODES))}."
            )
        raw_entries = scheme.get("entries")
        if not isinstance(raw_entries, list) or not raw_entries:
            raise ValueError(f"Source registry `{context}.entries` must be a non-empty list.")
        ...
        for index, raw_entry in enumerate(raw_entries):
            ...
            if entry_id in seen_entry_ids:
                raise ValueError(f"Source registry `{context}.entries` repeats entry ID `{entry_id}`.")
            seen_entry_ids.add(entry_id)
            target_type = require_string(entry, "target_type", entry_context)
            if target_type not in {"work", "segment", "content-group"}:
                raise ValueError(
                    f"Source registry `{entry_context}.target_type` must be `work` `segment`, or `content-group`."
                )
            target_id = require_string(entry, "target_id", entry_context)
            targets = works if target_type == "work" else segments if target_type == "segment" else raw_content_groups
            if target_id not in targets:
                raise ValueError(
                    f"Source registry `{entry_context}.target_id` references unknown {target_type} `{target_id}`."
                )
            ordinal = entry.get("ordinal")
            after_entry_ids = require_string_list(entry, "after_entry_ids", entry_context)
            for predecessor_id in after_entry_ids:
                validate_id(predecessor_id, f"{entry_context}.after_entry_ids")
            if ordering_mode == "total":
                if isinstance(ordinal, bool) or not isinstance(ordinal, int) or ordinal < 1:
                    raise ValueError(
                        f"Source registry `{entry_context}.ordinal` must be a positive integer for total ordering."
                    )
                if after_entry_ids:
                    raise ValueError(
                        f"Source registry `{entry_context}.after_entry_ids` must be empty for total ordering."
                    )
            elif ordinal is not None:
                raise ValueError(f"Source registry `{entry_context}.ordinal` must be omitted for partial ordering.")
            target_key = (target_type, target_id)
            if target_key in seen_targets or (ordinal is not None and ordinal in seen_ordinals):
                raise ValueError(f"Source registry `{context}.entries` repeats a target or ordinal.")
            seen_targets.add(target_key)
            if ordinal is not None:
                seen_ordinals.add(ordinal)
            entries.append(OrderingEntry(entry_id, target_type, target_id, ordinal, after_entry_ids))
        if ordering_mode == "partial":
            ...
            for entry in entries:
                ...
                if entry.id in entry.after_entry_ids:
                    raise ValueError(f"Source registry `{context}` entry `{entry.id}` cannot follow itself.")
            complete_entries: set[str] = set()

            def visit_ordering_entry(entry_id: str, active: set[str]) -> None:
                if entry_id in active:
                    raise ValueError(
                        f"Source registry `{context}` contains a partial-order cycle involving `{entry_id}`."
                    )
        ...
        ordering_schemes[scheme_id] = OrderingScheme(
            ...
            entries=tuple(
                sorted(entries, key=lambda entry: entry.ordinal or 0) if ordering_mode == "total" else entries
            ),
        )
    if ordering_schemes:
        validate_pack_values(
    ...
    for group_id, raw_group in raw_content_groups.items():
        ...
        if not isinstance(raw_members, list) or not raw_members:
            raise ValueError(f"Source registry `{context}.members` must be a non-empty list.")
        ...
        for index, raw_member in enumerate(raw_members):
            ...
            if member_id in seen_member_ids:
                raise ValueError(f"Source registry `{context}.members` repeats ID `{member_id}`.")
            seen_member_ids.add(member_id)
            target_type = require_string(member, "target_type", member_context)
            ...
            if target_type not in target_registries:
                raise ValueError(
                    f"Source registry `{member_context}.target_type` must be `work`, `segment`, or `content-group`."
                )
            if target_id not in target_registries[target_type]:
                raise ValueError(
                    f"Source registry `{member_context}.target_id` references unknown {target_type} `{target_id}`."
                )
            member_key = (target_type, target_id)
            if member_key in seen_members:
                raise ValueError(f"Source registry `{context}.members` contains duplicates.")
            ...
            members.append(ContentGroupMember(member_id, target_type, target_id, role))
        parent_group_ids = require_string_list(group, "parent_group_ids", context)
        if group_id in parent_group_ids or len(set(parent_group_ids)) != len(parent_group_ids):
            raise ValueError(f"Source registry `{context}.parent_group_ids` contains a self reference or duplicate.")
        ordering_scheme_id = optional_string(group, "ordering_scheme_id", context)
        if ordering_scheme_id is not None:
            ...
            ordered_targets = {
                (entry.target_type, entry.target_id) for entry in ordering_schemes[ordering_scheme_id].entries
            }
            if ordered_targets != seen_members:
                raise ValueError(
                    f"Source registry `{context}.ordering_scheme_id` must order exactly the group's members."
                )
        ...
        for alias in aliases:
            ...
            if normalized in content_group_aliases or normalized in {
                ...
                raise ValueError(
                    f"Source registry content-group alias `{alias}` is duplicated or collides with a group ID."
                )
    ...
    def visit_content_group(group_id: str, active: set[str]) -> None:
        if group_id in active:
            raise ValueError(f"Source registry contains a content-group cycle involving `{group_id}`.")
    ...
    for index, raw_relationship in enumerate(raw_work_relationships):
        ...
        if source_id == target_id:
            raise ValueError(f"Source registry `{context}` cannot relate a work to itself.")
        if relationship_type not in work_relationship_types:
            raise ValueError(
                f"Source registry `{context}.relationship_type` references unknown type `{relationship_type}`."
            )
        continuity_ids = require_string_list(relationship, "continuity_ids", context)
        ...
        work_relationships.append(
            WorkRelationship(
                ...
                continuity_ids,
                status,
                optional_string(relationship, "applicability_scope_id", context),
            )
        )
    ...
    for index, raw_mapping in enumerate(raw_adaptation_mappings):
        ...
        if mapping_id in seen_adaptation_mapping_ids:
            raise ValueError(f"Source registry adaptation mapping ID `{mapping_id}` is duplicated.")
        ...
        if target_work_id not in works:
            raise ValueError(f"Source registry `{context}.target_work_id` references an unknown work.")
        target_segment_ids = require_string_list(mapping, "target_segment_ids", context)
        for segment_id in target_segment_ids:
            if segment_id not in segments or segments[segment_id].work_id != target_work_id:
                raise ValueError(
        ...
        if not isinstance(raw_basis_inputs, list) or not raw_basis_inputs:
            raise ValueError(f"Source registry `{context}.basis_inputs` must be a non-empty list.")
        ...
        for basis_index, raw_basis in enumerate(raw_basis_inputs):
            ...
            if work_id not in works:
                raise ValueError(f"Source registry `{basis_context}.work_id` references unknown work `{work_id}`.")
            if work_id == target_work_id or work_id in seen_basis_works:
                raise ValueError(
                    f"Source registry `{context}.basis_inputs` repeats a work or uses the target as its own basis."
                )
            seen_basis_works.add(work_id)
            segment_ids = require_string_list(basis, "segment_ids", basis_context)
    ...
    if adaptation_mappings:
        ...
        validate_pack_values(
            ...
            (basis.basis_role for mapping in adaptation_mappings for basis in mapping.basis_inputs),
    ...
    for territory_id, raw_territory in raw_territories.items():
        ...
        for scheme_id, code in raw_codes.items():
            ...
            if not isinstance(code, str) or not code.strip():
                raise ValueError(f"Source registry `{context}.codes.{scheme_id}` must be a non-empty string.")
        ...
        territories[territory_id] = TerritoryConfig(
            ...
            parent_territory_id=optional_string(territory, "parent_territory_id", context),
            codes=codes,
        )
    if territories:
        validate_pack_values(
    ...
    for territory in territories.values():
        if territory.parent_territory_id is not None:
            ...
            if territory.parent_territory_id == territory.id:
                raise ValueError(f"Source registry territory `{territory.id}` cannot parent itself.")
    for territory_id in territories:
        seen_territories: set[str] = set()
        current_id: str | None = territory_id
        while current_id is not None:
            if current_id in seen_territories:
                raise ValueError(f"Source registry contains a territory cycle involving `{current_id}`.")
    ...
    for territory in territories.values():
        for scheme_id, code in territory.codes.items():
            ...
            if key in seen_territory_codes:
                raise ValueError(f"Source registry repeats territory code `{scheme_id}:{code}`.")
    ...
    if not isinstance(raw_production_contexts, list):
        raise ValueError("Source registry `work_production_contexts` must be a list.")
    work_production_contexts: dict[str, WorkProductionContext] = {}
    for index, raw_context in enumerate(raw_production_contexts):
        ...
        if context_id in work_production_contexts:
            raise ValueError(f"Source registry work-production-context ID `{context_id}` is duplicated.")
        work_id = require_string(production_context, "work_id", context)
        if work_id not in works:
            raise ValueError(f"Source registry `{context}.work_id` references unknown work `{work_id}`.")
        production_origin = require_string(production_context, "production_origin", context)
        authorization_status = require_string(production_context, "authorization_status", context)
        rights_basis = require_string(production_context, "rights_basis", context)
        commerciality = require_string(production_context, "commerciality", context)
        applicability_scope_id = require_string(production_context, "applicability_scope_id", context)
        validate_pack_values(
            schema_packs,
    ...
    for owner_context, localized_titles in (
        ...
        *((f"segments.{segment.id}", segment.localized_titles) for segment in segments.values()),
        *((f"content_groups.{group.id}", group.localized_titles) for group in content_groups.values()),
    ):
    ...
    for platform_id, raw_platform in raw_platforms.items():
        ...
        for alias in aliases:
            ...
            if normalized in platform_aliases or normalized in {
                ...
                raise ValueError(
                    f"Source registry platform alias `{alias}` is duplicated or collides with a platform ID."
                )
    ...
    raw_manifestations = require_mapping(registry.get("manifestations"), "manifestations")
    manifestations: dict[str, ManifestationConfig] = {}
    ...
    for manifestation_id, raw_manifestation in raw_manifestations.items():
        ...
        if work_id not in works:
            raise ValueError(f"Source registry `{context}.work_id` references unknown work `{work_id}`.")
        segment_ids = require_string_list(manifestation, "segment_ids", context)
        for segment_id in segment_ids:
            if segment_id not in segments or segments[segment_id].work_id != work_id:
                raise ValueError(
        ...
        manifestation_type = require_string(manifestation, "manifestation_type", context)
        container_format_ids = require_string_list(manifestation, "container_format_ids", context)
        ...
        for alias in aliases:
            ...
            if normalized in manifestation_aliases or normalized in {
                ...
                raise ValueError(
                    f"Source registry manifestation alias `{alias}` is duplicated or collides with a manifestation ID."
                )
            manifestation_aliases.add(normalized)
        language_tags = require_string_list(manifestation, "language_tags", context)
        for language_tag in language_tags:
            validate_language_tag(language_tag, f"{context}.language_tags")
        territory_ids = require_string_list(manifestation, "territory_ids", context)
        ...
        localized_titles = parse_localized_titles(manifestation, context, schema_packs)
        for localized_title in localized_titles:
            unknown_territories = set(localized_title.territory_ids) - set(territories)
            if unknown_territories:
                raise ValueError(
    ...
    if manifestations:
        validate_pack_values(
            ...
            (manifestation.manifestation_type for manifestation in manifestations.values()),
    ...
    if not isinstance(raw_manifestation_relationships, list):
        raise ValueError("Source registry `manifestation_relationships` must be a list.")
    ...
    for index, raw_relationship in enumerate(raw_manifestation_relationships):
        ...
        if relationship_id in seen_manifestation_relationship_ids:
            raise ValueError(f"Source registry manifestation relationship ID `{relationship_id}` is duplicated.")
        seen_manifestation_relationship_ids.add(relationship_id)
        source_id = require_string(relationship, "source_manifestation_id", context)
        target_id = require_string(relationship, "target_manifestation_id", context)
        relationship_type = require_string(relationship, "relationship_type", context)
        if source_id not in manifestations or target_id not in manifestations:
            raise ValueError(f"Source registry `{context}` references an unknown manifestation.")
        if source_id == target_id:
            raise ValueError(f"Source registry `{context}` cannot relate a manifestation to itself.")
        ...
        manifestation_relationships.append(
            ManifestationRelationship(relationship_id, source_id, relationship_type, target_id, status)
        )

    raw_manifestation_segment_mappings = registry.get("manifestation_segment_mappings")
    if not isinstance(raw_manifestation_segment_mappings, list):
        raise ValueError("Source registry `manifestation_segment_mappings` must be a list.")
    ...
    for index, raw_mapping in enumerate(raw_manifestation_segment_mappings):
        ...
        if mapping_id in seen_manifestation_mapping_ids:
            raise ValueError(f"Source registry manifestation segment mapping ID `{mapping_id}` is duplicated.")
        seen_manifestation_mapping_ids.add(mapping_id)
        source_id = require_string(mapping, "source_manifestation_id", context)
        target_id = require_string(mapping, "target_manifestation_id", context)
        if source_id not in manifestations or target_id not in manifestations:
            raise ValueError(f"Source registry `{context}` references an unknown manifestation.")
        if source_id == target_id:
            raise ValueError(f"Source registry `{context}` cannot map a manifestation to itself.")
        if manifestations[source_id].work_id != manifestations[target_id].work_id:
            raise ValueError(f"Source registry `{context}` must map manifestations of the same work.")
        if frozenset((source_id, target_id)) not in related_manifestation_pairs:
            raise ValueError(
                f"Source registry `{context}` requires a manifestation relationship between its source and target."
            )
        source_segment_ids = require_string_list(mapping, "source_segment_ids", context)
        target_segment_ids = require_string_list(mapping, "target_segment_ids", context)
            ...
            for segment_id in values:
                if (
                    ...
                    or (
                        ...
                        and segment_id not in manifestations[manifestation_id].segment_ids
                    )
                ):
                    raise ValueError(
        ...
        if not valid_shape:
            raise ValueError(
                f"Source registry `{context}` has segment lists incompatible with mapping type `{mapping_type}`."
            )
        status = require_string(mapping, "status", context)
        if status not in membership_statuses:
            raise ValueError(
    ...
    if manifestation_segment_mappings:
        validate_pack_values(
            ...
            (mapping.mapping_type for mapping in manifestation_segment_mappings),
            "manifestation_segment_mappings.*.mapping_type",
        )

    raw_release_components = require_mapping(registry.get("release_components"), "release_components")
    release_components: dict[str, ReleaseComponent] = {}
    for component_id, raw_component in raw_release_components.items():
        ...
        manifestation_id = optional_string(component, "manifestation_id", context)
        if manifestation_id is not None and manifestation_id not in manifestations:
            raise ValueError(
                f"Source registry `{context}.manifestation_id` references unknown manifestation `{manifestation_id}`."
            )
        segment_ids = require_string_list(component, "segment_ids", context)
        for segment_id in segment_ids:
            if segment_id not in segments:
                raise ValueError(f"Source registry `{context}.segment_ids` references segment `{segment_id}`.")
            if manifestation_id is not None:
                manifestation = manifestations[manifestation_id]
                if segments[segment_id].work_id != manifestation.work_id or (
                    manifestation.segment_ids and segment_id not in manifestation.segment_ids
                ):
                    raise ValueError(
                        f"Source registry `{context}.segment_ids` references segment "
                        f"`{segment_id}` outside manifestation scope."
    ...
    raw_component_relationships = registry.get("release_component_relationships")
    if not isinstance(raw_component_relationships, list):
        raise ValueError("Source registry `release_component_relationships` must be a list.")
    ...
    for index, raw_relationship in enumerate(raw_component_relationships):
        ...
        if relationship_id in seen_component_relationship_ids:
            raise ValueError(f"Source registry release-component relationship ID `{relationship_id}` is duplicated.")
        seen_component_relationship_ids.add(relationship_id)
        source_component_id = require_string(relationship, "source_component_id", context)
        target_component_id = require_string(relationship, "target_component_id", context)
        if source_component_id not in release_components or target_component_id not in release_components:
            raise ValueError(f"Source registry `{context}` references an unknown release component.")
        if source_component_id == target_component_id:
            raise ValueError(f"Source registry `{context}` cannot relate a component to itself.")
        relationship_type = require_string(relationship, "relationship_type", context)
    ...
    raw_release_packages = require_mapping(registry.get("release_packages"), "release_packages")
    release_packages: dict[str, ReleasePackage] = {}
    ...
    for package_id, raw_package in raw_release_packages.items():
        ...
        manifestation_ids = require_string_list(package, "manifestation_ids", context)
        ...
        for segment_id in segment_ids:
            if segment_id not in segments:
                raise ValueError(f"Source registry `{context}.segment_ids` references unknown segment `{segment_id}`.")
        component_ids = require_string_list(package, "release_component_ids", context)
        component_manifestation_ids: set[str] = set()
        for component_id in component_ids:
            if component_id not in release_components:
                raise ValueError(
                    f"Source registry `{context}.release_component_ids` references unknown component `{component_id}`."
                )
            component_manifestation_id = release_components[component_id].manifestation_id
        ...
        package_work_ids = {
            manifestations[manifestation_id].work_id for manifestation_id in effective_manifestation_ids
        }
        for segment_id in segment_ids:
        ...
        if not manifestation_ids and not segment_ids and not component_ids:
            raise ValueError(
                f"Source registry `{context}` must contain a manifestation, segment, or release component."
            )
        container_format_ids = require_string_list(package, "container_format_ids", context)
        unknown_formats = set(container_format_ids) - set(container_formats)
        if unknown_formats:
            raise ValueError(
                f"Source registry `{context}.container_format_ids` references unknown "
        ...
        for alias in aliases:
            ...
            if normalized in release_package_aliases or normalized in {
                ...
                raise ValueError(
                    f"Source registry release-package alias `{alias}` is duplicated or collides with a package ID."
                )
    ...
    packages_by_component: dict[str, set[str]] = {component_id: set() for component_id in release_components}
    ...
    for component in release_components.values():
        if component.manifestation_id is None and not packages_by_component[component.id]:
            raise ValueError(
        ...
        for package_id in packages_by_component[component.id]:
            ...
            package_work_ids = {
                manifestations[manifestation_id].work_id for manifestation_id in package.manifestation_ids
            } | {segments[segment_id].work_id for segment_id in package.segment_ids}
            if component.manifestation_id is not None:
                package_work_ids.add(manifestations[component.manifestation_id].work_id)
            component_work_ids = {segments[segment_id].work_id for segment_id in component.segment_ids}
            if package_work_ids and not component_work_ids.issubset(package_work_ids):
                raise ValueError(
    ...
    def release_package_work_ids(package_id: str) -> set[str]:
        package = release_packages[package_id]
        work_ids = {manifestations[manifestation_id].work_id for manifestation_id in package.manifestation_ids}
        work_ids.update(segments[segment_id].work_id for segment_id in package.segment_ids)
        for component_id in package.release_component_ids:
            component = release_components[component_id]
            if component.manifestation_id is not None:
                work_ids.add(manifestations[component.manifestation_id].work_id)
            work_ids.update(segments[segment_id].work_id for segment_id in component.segment_ids)
        return work_ids
    ...
    def validate_distribution_scope(
        ...
        if subject_type == "manifestation":
            if subject_id not in manifestations:
                raise ValueError(
                    f"Source registry `{context}.subject_id` references unknown manifestation `{subject_id}`."
                )
            ...
            for segment_id in segment_ids:
                if (
                    ...
                    or (subject.segment_ids and segment_id not in subject.segment_ids)
                ):
                    raise ValueError(
                        f"Source registry `{context}.segment_ids` references segment "
                        f"`{segment_id}` outside manifestation scope."
        ...
        if subject_id not in release_packages:
            raise ValueError(
                f"Source registry `{context}.subject_id` references unknown release package `{subject_id}`."
            )
    ...
    raw_release_runs = require_mapping(registry.get("release_runs"), "release_runs")
    release_runs: dict[str, ReleaseRun] = {}
    for run_id, raw_run in raw_release_runs.items():
        ...
        if not segment_ids or len(set(segment_ids)) != len(segment_ids):
            raise ValueError(f"Source registry `{context}.segment_ids` must be a non-empty duplicate-free list.")
        validate_distribution_scope(subject_type, subject_id, segment_ids, context)
        ordering_scheme_id = require_string(run, "ordering_scheme_id", context)
        if ordering_scheme_id not in ordering_schemes:
            raise ValueError(
        ...
        ordered_segment_ids = {entry.target_id for entry in ordering.entries if entry.target_type == "segment"}
        if (
        ...
        if not isinstance(raw_phases, list) or not raw_phases:
            raise ValueError(f"Source registry `{context}.phases` must be a non-empty list.")
        ...
        for index, raw_phase in enumerate(raw_phases):
            ...
            if phase_id in seen_phase_ids:
                raise ValueError(f"Source registry `{context}.phases` repeats phase ID `{phase_id}`.")
            seen_phase_ids.add(phase_id)
            phase_segment_ids = require_string_list(phase, "segment_ids", phase_context)
            if not phase_segment_ids:
                raise ValueError(f"Source registry `{phase_context}.segment_ids` must not be empty.")
            flattened_phase_segments.extend(phase_segment_ids)
            first_release_window = parse_temporal_window(phase, "first_release_window", phase_context, schema_packs)
            ...
            if isinstance(cadence_interval, bool) or not isinstance(cadence_interval, int) or cadence_interval < 1:
                raise ValueError(f"Source registry `{phase_context}.cadence.interval` must be a positive integer.")
            validate_pack_values(
                schema_packs,
            ...
            if (
                ...
                raise ValueError(
                    f"Source registry `{phase_context}.batch_size` must be between 1 and the number of phase segments."
                )
            phases.append(
                ReleaseRunPhase(
                    phase_id,
        ...
        ordered_run_segments = tuple(entry.target_id for entry in ordering.entries)
        ...
        if not isinstance(raw_exceptions, list):
            raise ValueError(f"Source registry `{context}.exceptions` must be a list.")
        ...
        for index, raw_exception in enumerate(raw_exceptions):
            ...
            exception_type = require_string(exception, "exception_type", exception_context)
            validate_pack_values(
                schema_packs,
            ...
            segment_id = require_string(exception, "segment_id", exception_context)
            if segment_id not in segment_ids:
                raise ValueError(f"Source registry `{exception_context}.segment_id` falls outside the release run.")
            if (exception_type, segment_id) in seen_exceptions:
                raise ValueError(f"Source registry `{context}.exceptions` repeats an exception.")
            ...
            if interval_count is not None and (
                isinstance(interval_count, bool) or not isinstance(interval_count, int) or interval_count < 1
            ):
                raise ValueError(
                    f"Source registry `{exception_context}.interval_count` must be a positive integer when present."
                )
        ...
        release_runs[run_id] = ReleaseRun(
            ...
            segment_ids=segment_ids,
            ordering_scheme_id=ordering_scheme_id,
            release_event_type=require_string(run, "release_event_type", context),
            phases=tuple(phases),
            territory_ids=territory_ids,
            platform_ids=platform_ids,
            availability_status=require_string(run, "availability_status", context),
    ...
    raw_release_events = require_mapping(registry.get("release_events"), "release_events")
    release_events: dict[str, ReleaseEvent] = {}
    for event_id, raw_event in raw_release_events.items():
        ...
        if release_run_id is not None:
            if release_run_id not in release_runs:
                raise ValueError(
                    f"Source registry `{context}.release_run_id` references unknown release run `{release_run_id}`."
                )
            ...
            if (
                ...
                or (release_run.platform_ids and not set(platform_ids).issubset(release_run.platform_ids))
                or (release_run.territory_ids and not set(territory_ids).issubset(release_run.territory_ids))
            ):
                raise ValueError(f"Source registry `{context}` falls outside its release run.")
        release_events[event_id] = ReleaseEvent(
            ...
            subject_id=subject_id,
            segment_ids=segment_ids,
            release_event_type=require_string(event, "release_event_type", context),
            release_window=parse_temporal_window(event, "release_window", context, schema_packs),
            territory_ids=territory_ids,
            platform_ids=platform_ids,
            availability_status=require_string(event, "availability_status", context),
    ...
    raw_catalog_placements = require_mapping(registry.get("catalog_placements"), "catalog_placements")
    catalog_placements: dict[str, CatalogPlacement] = {}
    for placement_id, raw_placement in raw_catalog_placements.items():
        ...
        if platform_id not in platforms:
            raise ValueError(f"Source registry `{context}.platform_id` references unknown platform `{platform_id}`.")
        ...
        if target_id not in catalog_targets[target_type]:
            raise ValueError(f"Source registry `{context}.target_id` references unknown {target_type} `{target_id}`.")
        ordinal = placement.get("ordinal")
        if ordinal is not None and (isinstance(ordinal, bool) or not isinstance(ordinal, int) or ordinal < 1):
            raise ValueError(f"Source registry `{context}.ordinal` must be a positive integer when present.")
        localized_titles = parse_localized_titles(placement, context, schema_packs)
        for localized_title in localized_titles:
            unknown_territories = set(localized_title.territory_ids) - set(territories)
            if unknown_territories:
                raise ValueError(
        ...
        catalog_placements[placement_id] = CatalogPlacement(
            ...
            parent_placement_id=optional_string(placement, "parent_placement_id", context),
            target_type=target_type,
            target_id=target_id,
            ordinal=ordinal,
    ...
    for placement in catalog_placements.values():
        ...
        if parent_id is not None:
            if parent_id not in catalog_placements:
                raise ValueError(
                    f"Source registry `catalog_placements.{placement.id}` references unknown parent `{parent_id}`."
                )
            if parent_id == placement.id:
                raise ValueError(f"Source registry catalog placement `{placement.id}` cannot parent itself.")
    ...
    def visit_placement(placement_id: str, active: set[str]) -> None:
        if placement_id in active:
            raise ValueError(f"Source registry contains a catalog-placement cycle involving `{placement_id}`.")
    ...
    raw_platform_offerings = require_mapping(registry.get("platform_offerings"), "platform_offerings")
    platform_offerings: dict[str, PlatformOffering] = {}
    for offering_id, raw_offering in raw_platform_offerings.items():
        ...
        validate_distribution_scope(subject_type, subject_id, segment_ids, context)
        release_event_id = optional_string(offering, "release_event_id", context)
        if platform_id not in platforms:
            raise ValueError(f"Source registry `{context}.platform_id` references unknown platform `{platform_id}`.")
        if release_event_id is not None:
            ...
            if (
                ...
                or (event.segment_ids and not set(segment_ids).issubset(event.segment_ids))
            ):
                raise ValueError(
                    f"Source registry `{context}` release event does not match its "
                    "subject, segment scope, and platform."
                )
        placement_ids = require_string_list(offering, "catalog_placement_ids", context)
        for placement_id in placement_ids:
            if placement_id not in catalog_placements or catalog_placements[placement_id].platform_id != platform_id:
                raise ValueError(
        ...
        platform_offerings[offering_id] = PlatformOffering(
            ...
            availability_status=require_string(offering, "availability_status", context),
            territory_ids=territory_ids,
            language_tags=language_tags,
            availability_window=parse_temporal_window(offering, "availability_window", context, schema_packs),
    ...
    raw_identifier_schemes = require_mapping(registry.get("identifier_schemes"), "identifier_schemes")
    identifier_schemes: dict[str, IdentifierScheme] = {}
    for scheme_id, raw_scheme in raw_identifier_schemes.items():
        ...
        if not target_types:
            raise ValueError(f"Source registry `{context}.target_types` must not be empty.")
        validate_pack_values(
            schema_packs,
    ...
    for source_id, raw_source in raw_sources.items():
        ...
        if medium_id not in mediums:
            raise ValueError(f"Source registry `{context}.medium_id` references unknown medium `{medium_id}`.")
        locator_medium_ids = require_string_list(source, "locator_medium_ids", context)
        if medium_id not in locator_medium_ids or len(set(locator_medium_ids)) != len(locator_medium_ids):
            raise ValueError(
                f"Source registry `{context}.locator_medium_ids` must be duplicate-free and include the source medium."
            )
        ...
        container_format_ids = require_string_list(source, "container_format_ids", context)
        if not container_format_ids:
            raise ValueError(f"Source registry `{context}.container_format_ids` must not be empty.")
        ...
        if role not in allowed_source_roles:
            raise ValueError(
                f"Source registry `{context}.role` must be one of: {', '.join(sorted(allowed_source_roles))}."
            )
        work_ids = require_string_list(source, "work_ids", context)
        if not work_ids or len(set(work_ids)) != len(work_ids):
            raise ValueError(f"Source registry `{context}.work_ids` must be a non-empty duplicate-free list.")
        unknown_works = set(work_ids) - set(works)
        if unknown_works:
            raise ValueError(
                f"Source registry `{context}.work_ids` references unknown works: {', '.join(sorted(unknown_works))}."
            )
        ...
        if manifestation_id is not None:
            ...
            if manifestations[manifestation_id].work_id not in work_ids:
                raise ValueError(
                    f"Source registry `{context}` manifestation belongs to a work outside the source scope."
                )
        release_package_id = optional_string(source, "release_package_id", context)
        if release_package_id is not None:
            ...
            if manifestation_id is not None and manifestation_id not in package_manifestation_ids:
                raise ValueError(f"Source registry `{context}` manifestation is not contained by its release package.")
            ...
            if package_work_ids and not set(work_ids).issubset(package_work_ids):
                raise ValueError(f"Source registry `{context}.work_ids` extends beyond its release package.")
        ...
        if release_event_id is not None:
            ...
            if (event.subject_type, event.subject_id) != expected_subject:
                raise ValueError(
                    f"Source registry `{context}` release event does not belong to its manifestation or package."
                )
        release_component_ids = require_string_list(source, "release_component_ids", context)
        for component_id in release_component_ids:
            ...
            component_work_ids = {
                segments[segment_id].work_id for segment_id in release_components[component_id].segment_ids
            }
            if component_manifestation_id is not None:
                component_work_ids.add(manifestations[component_manifestation_id].work_id)
            if component_work_ids and not component_work_ids.intersection(work_ids):
                raise ValueError(
                    f"Source registry `{context}` component `{component_id}` falls outside the source work scope."
                )
        platform_offering_id = optional_string(source, "platform_offering_id", context)
        if platform_offering_id is not None:
            ...
            if (offering.subject_type, offering.subject_id) != expected_subject:
                raise ValueError(
                    f"Source registry `{context}` platform offering does not belong to its manifestation or package."
                )
        incompatible_work_ids = {work_id for work_id in work_ids if works[work_id].medium_id != medium_id}
        if incompatible_work_ids and role not in {
            ...
            raise ValueError(
                f"Source registry `{context}` medium does not match works: {', '.join(sorted(incompatible_work_ids))}."
            )
        ...
        if isinstance(priority, bool) or not isinstance(priority, int) or priority < 1:
            raise ValueError(f"Source registry `{context}.priority` must be a positive integer.")
        source_aliases = require_string_list(source, "aliases", context)
        for alias in source_aliases:
            validate_id(alias, f"{context}.aliases")
            alias_key = lookup_keys.normalize(alias)
            if alias_key in aliases or alias_key in {lookup_keys.normalize(existing_id) for existing_id in raw_sources}:
                raise ValueError(f"Source registry alias `{alias}` is duplicated or collides with a source ID.")
        ...
        if not isinstance(raw_observations, list):
            raise ValueError(f"Source registry `{context}.observations` must be a list.")
        ...
        for index, raw_observation in enumerate(raw_observations):
            ...
            observation_id = require_string(observation, "id", observation_context)
            validate_id(observation_id, f"{observation_context}.id")
            target_type = require_string(observation, "target_type", observation_context)
            target_id = require_string(observation, "target_id", observation_context)
            if observation_id in seen_observation_ids:
                raise ValueError(f"Source registry `{context}.observations` repeats ID `{observation_id}`.")
            if target_type not in observation_targets or target_id not in observation_targets[target_type]:
                raise ValueError(
                    f"Source registry `{observation_context}` references unknown {target_type} `{target_id}`."
                )
            if (target_type, target_id) in seen_observation_targets:
                raise ValueError(f"Source registry `{context}.observations` repeats a target.")
            seen_observation_ids.add(observation_id)
            seen_observation_targets.add((target_type, target_id))
            observations.append(SourceObservation(observation_id, target_type, target_id))
        raw_coverage = source.get("coverage")
        if not isinstance(raw_coverage, list):
            raise ValueError(f"Source registry `{context}.coverage` must be a list.")
        coverage: list[SourceCoverage] = []
        seen_coverage_ids: set[str] = set()
        seen_coverage: dict[tuple[str, str, str, str], list[SourceCoverage]] = {}
        coverage_targets = {
            "work": works,
            "segment": segments,
            "content-group": content_groups,
        ...
        def coverage_work_ids(target_type: str, target_id: str) -> set[str]:
            ...
            if target_type == "content-group":
                ...
                for member in content_groups[target_id].members:
                    if member.target_type == "work":
                        ...
                        result.update(coverage_work_ids("content-group", member.target_id))
                return result
            if target_type == "manifestation":
            ...
            result = {segments[segment_id].work_id for segment_id in component.segment_ids}
            if component.manifestation_id is not None:
        ...
        for observation in observations:
            if observation.target_type == "release-event":
                ...
                observed_work_ids = coverage_work_ids(observation.target_type, observation.target_id)
            if observed_work_ids and not observed_work_ids.issubset(work_ids):
                raise ValueError(
                    f"Source registry `{context}.observations` includes material outside the source work scope."
                )

        for index, raw_coverage_entry in enumerate(raw_coverage):
            coverage_context = f"{context}.coverage[{index}]"
            coverage_entry = require_mapping(raw_coverage_entry, coverage_context)
            coverage_id = require_string(coverage_entry, "id", coverage_context)
            validate_id(coverage_id, f"{coverage_context}.id")
            if coverage_id in seen_coverage_ids:
                raise ValueError(f"Source registry `{context}.coverage` repeats ID `{coverage_id}`.")
            seen_coverage_ids.add(coverage_id)
            target_type = require_string(coverage_entry, "target_type", coverage_context)
            validate_pack_values(
                schema_packs,
            ...
            target_id = require_string(coverage_entry, "target_id", coverage_context)
            if target_type not in coverage_targets or target_id not in coverage_targets[target_type]:
                raise ValueError(
                    f"Source registry `{coverage_context}.target_id` references unknown {target_type} `{target_id}`."
                )
            coverage_type = require_string(coverage_entry, "coverage_type", coverage_context)
            validate_pack_values(
                schema_packs,
            ...
            coverage_medium_id = require_string(coverage_entry, "medium_id", coverage_context)
            if coverage_medium_id not in locator_medium_ids:
                raise ValueError(f"Source registry `{coverage_context}.medium_id` is not allowed by the source.")
            coverage_evidence_modes = require_string_list(coverage_entry, "evidence_modes", coverage_context)
            if not coverage_evidence_modes:
                raise ValueError(f"Source registry `{coverage_context}.evidence_modes` must not be empty.")
            unknown_coverage_modes = set(coverage_evidence_modes) - set(evidence_modes)
            ...
            if target_work_ids and not target_work_ids.issubset(work_ids):
                raise ValueError(f"Source registry `{coverage_context}` extends beyond the source work scope.")
            raw_ranges = coverage_entry.get("position_ranges")
            if not isinstance(raw_ranges, list):
                raise ValueError(f"Source registry `{coverage_context}.position_ranges` must be a list.")
            ...
            for range_index, raw_range in enumerate(raw_ranges):
                range_context = f"{coverage_context}.position_ranges[{range_index}]"
                ...
                if range_id in seen_range_ids:
                    raise ValueError(f"Source registry `{coverage_context}.position_ranges` repeats ID `{range_id}`.")
                seen_range_ids.add(range_id)
                start = require_mapping(position_range.get("start"), f"{range_context}.start")
                end = require_mapping(position_range.get("end"), f"{range_context}.end")
                if not start or set(start) != set(end):
                    raise ValueError(
                        f"Source registry `{range_context}` start/end fields must be non-empty and identical."
                    )
                ...
                if work_scope_field not in start:
                    raise ValueError(
                        f"Source registry `{range_context}` must include work-scope field `{work_scope_field}`."
                    )
                range_work_ids = {start[work_scope_field], end[work_scope_field]}
                if len(range_work_ids) != 1 or not range_work_ids.issubset(target_work_ids):
                    raise ValueError(f"Source registry `{range_context}` falls outside its coverage target work scope.")
                ...
                if (
                    ...
                    raise ValueError(f"Source registry `{range_context}` start must not follow end.")
                position_ranges.append(CoveragePositionRange(range_id, dict(start), dict(end)))
        ...
        if not isinstance(raw_bindings, list):
            raise ValueError(f"Source registry `{context}.resource_bindings` must be a list.")
        bindings = tuple(
    ...
    for profile in authority_profiles.values():
        for rule in profile.claim_authority_rules:
            if rule.id in seen_authority_rule_ids:
                raise ValueError(f"Source registry authority-rule ID `{rule.id}` is duplicated across profiles.")
        ...
        for claim_namespace, ancestors in claim_namespace_ancestors.items():
            for source in sources.values():
                for evidence_mode in (None, *source.evidence_modes):
                    ...
                    if not matches:
                        continue
                    highest_precedence = max(rule.precedence for rule in matches)
                    winners = [rule for rule in matches if rule.precedence == highest_precedence]
                    if len(winners) > 1:
    ...
    for index, raw_relationship in enumerate(raw_source_relationships):
        ...
        if source_id not in sources or target_id not in sources:
            raise ValueError(f"Source registry `{context}` references an unknown source.")
        if source_id == target_id:
            raise ValueError(f"Source registry `{context}` cannot relate a source to itself.")
        if relationship_type not in source_relationship_types:
            raise ValueError(
                f"Source registry `{context}.relationship_type` references unknown type `{relationship_type}`."
            )
        source_relationships.append(SourceRelationship(relationship_id, source_id, relationship_type, target_id))
    ...
    applicability_scopes: dict[str, ApplicabilityScope] = {}
    scope_windows: dict[tuple[str, str, tuple[str, ...], int], list[TemporalWindow | None]] = {}
    for index, raw_scope in enumerate(raw_applicability_scopes):
        ...
        if scope_id in applicability_scopes:
            raise ValueError(f"Source registry applicability-scope ID `{scope_id}` is duplicated.")
        ...
        if target_type != "provenance-claim" and (
            target_type not in applicability_targets or target_id not in applicability_targets[target_type]
        ):
            raise ValueError(f"Source registry `{context}.target_id` references unknown {target_type} `{target_id}`.")
        ...
        if isinstance(precedence, bool) or not isinstance(precedence, int) or precedence < 0:
            raise ValueError(f"Source registry `{context}.precedence` must be a non-negative integer.")
        effective_window = parse_temporal_window(scope, "effective_window", context, schema_packs)
        window_key = (
            target_type,
            target_id,
            tuple(sorted(territory_ids)),
        ...
        if any(temporal_windows_overlap(effective_window, existing) for existing in scope_windows.get(window_key, [])):
            raise ValueError(
    ...
    def applicability_target_work_ids(scope: ApplicabilityScope) -> set[str]:
        ...
        if target_type == "adaptation-mapping":
            ...
            return {mapping.target_work_id} | {basis.work_id for basis in mapping.basis_inputs}
        if target_type == "manifestation":
            return {manifestations[target_id].work_id}
        if target_type == "release-component":
            component = release_components[target_id]
    ...
    for relationship in work_relationships:
        ...
        relationship_scope = applicability_scopes[relationship.applicability_scope_id]
        if applicability_target_work_ids(relationship_scope) != {relationship.source_work_id}:
            raise ValueError(
                f"Source registry work relationship `{relationship.id}` scope must resolve only to its source work."
            )
    ...
    for production_context in work_production_contexts.values():
        ...
        production_scope = applicability_scopes[production_context.applicability_scope_id]
        if production_context.work_id not in applicability_target_work_ids(production_scope):
            raise ValueError(
                f"Source registry work production context "
    ...
    if not isinstance(raw_continuity_assertions, list):
        raise ValueError("Source registry `scoped_continuity_assertions` must be a list.")
    scoped_continuity_assertions: dict[str, ScopedContinuityAssertion] = {}
    ...
    for index, raw_assertion in enumerate(raw_continuity_assertions):
        ...
        if assertion_id in scoped_continuity_assertions:
            raise ValueError(f"Source registry scoped-continuity-assertion ID `{assertion_id}` is duplicated.")
        scope_id = require_string(assertion, "applicability_scope_id", context)
        if scope_id not in applicability_scopes:
            raise ValueError(
                f"Source registry `{context}.applicability_scope_id` references unknown scope `{scope_id}`."
            )
        ...
        if scope.target_type not in {
            ...
            raise ValueError(
                f"Source registry `{context}` scope target type `{scope.target_type}` cannot carry continuity."
            )
        continuity_id = require_string(assertion, "continuity_id", context)
        if continuity_id not in continuities:
            raise ValueError(
                f"Source registry `{context}.continuity_id` references unknown continuity `{continuity_id}`."
            )
        ...
        if continuity_scope in seen_continuity_scopes:
            raise ValueError(
                f"Source registry repeats continuity `{continuity_id}` for applicability scope `{scope_id}`."
            )
    ...
    if not isinstance(raw_external_identifiers, list):
        raise ValueError("Source registry `external_identifiers` must be a list.")
    ...
    for index, raw_identifier in enumerate(raw_external_identifiers):
        ...
        if identifier_id in seen_external_identifier_ids:
            raise ValueError(f"Source registry external identifier ID `{identifier_id}` is duplicated.")
        ...
        if scheme_id not in identifier_schemes:
            raise ValueError(f"Source registry `{context}.scheme_id` references unknown scheme `{scheme_id}`.")
        target_type = require_string(identifier, "target_type", context)
        if target_type not in identifier_schemes[scheme_id].target_types:
            raise ValueError(
                f"Source registry `{context}.target_type` is not allowed by identifier scheme `{scheme_id}`."
            )
        ...
        normalized_value = value if identifier_schemes[scheme_id].case_sensitive else lookup_keys.normalize(value)
        scheme_value = (scheme_id, normalized_value)
        if scheme_value in seen_scheme_values:
            raise ValueError(f"Source registry repeats value `{value}` in identifier scheme `{scheme_id}`.")
        seen_scheme_values.add(scheme_value)
        territory_ids = require_string_list(identifier, "territory_ids", context)
        unknown_territories = set(territory_ids) - set(territories)
        if unknown_territories:
            raise ValueError(
                f"Source registry `{context}.territory_ids` references unknown "
    ...
    return SourceRegistry(
        ...
        manifestation_segment_mappings=tuple(manifestation_segment_mappings),
        release_components=release_components,
        release_component_relationship_types=(release_component_relationship_types),
        release_component_relationships=tuple(release_component_relationships),
