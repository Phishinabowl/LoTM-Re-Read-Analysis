"""Dependency-light framework-root discovery primitives."""
...
FRAMEWORK_MANIFEST_PATH = Path("Framework") / "framework.yaml"
FRAMEWORK_ROOT_ENVIRONMENT_VARIABLE = "KNOWLEDGE_FRAMEWORK_ROOT"


def is_framework_root(path: str | Path) -> bool:
    candidate = Path(path)
    return candidate.is_dir() and (candidate / FRAMEWORK_MANIFEST_PATH).is_file()


def _path_and_parents(path: str | Path):
...
def _validated_root(value: str | Path, source: str, *, require_absolute: bool = False) -> Path:
    ...
    if not is_framework_root(resolved):
        raise RuntimeError(
            f"Framework root from {source} is missing required manifest "
            f"{FRAMEWORK_MANIFEST_PATH.as_posix()}: {resolved}"
        )
    return resolved
...
def resolve_framework_root(
    ...
    *,
    ...
    """Resolve a framework root without changing the process working directory."""
    ...
    environment_root = environment_values.get(FRAMEWORK_ROOT_ENVIRONMENT_VARIABLE, "").strip()
    if environment_root:
        return _validated_root(
            environment_root,
            f"environment variable {FRAMEWORK_ROOT_ENVIRONMENT_VARIABLE}",
    ...
    for _, start in search_starts:
        for candidate in _path_and_parents(start):
    ...
    raise RuntimeError(
        f"Could not auto-detect the framework root from {starts}. "
        f"Expected manifest: {FRAMEWORK_MANIFEST_PATH.as_posix()}. "
        f"Pass the root explicitly or set {FRAMEWORK_ROOT_ENVIRONMENT_VARIABLE} to an absolute framework path."
    )
