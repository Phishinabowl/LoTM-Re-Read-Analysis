...
function Assert-KnowledgeJsonObjectKeys {
    ...
    if ($unknown.Count -gt 0) {
        throw "Lookup-key registry '$Context' contains unsupported field(s): $($unknown -join ', ')."
    }
}
...
function Add-KnowledgeCanonicalDecomposition {
    ...
    if ($hangulIndex -ge 0 -and $hangulIndex -lt $script:HangulSCount) {
        ...
        if ($trailingIndex -ne 0) {
            $Output.Add($script:HangulTBase + $trailingIndex)
        }
        return
    }
...
function ConvertTo-KnowledgeNfcCodePoints {
    ...
    for ($index = 1; $index -lt $decomposed.Count; $index += 1) {
        ...
        while ($cursor -gt 0) {
            ...
            if ($previousClass -eq 0 -or $previousClass -le $combiningClass) {
                break
            }
...
function ConvertTo-KnowledgeLookupKey {
    ...
    while ($start -lt $end -and $LookupKeyConfig.trim_codepoints.Contains([int]$codePoints[$start])) {
        $start += 1
    }
    while ($end -gt $start -and $LookupKeyConfig.trim_codepoints.Contains([int]$codePoints[$end - 1])) {
        $end -= 1
    }
    if ($end -le $start) {
        return , ([string]::Empty)
    }
    $trimmed = if ($end -gt $start) {
        @($codePoints[$start..($end - 1)])
    }
    else {
        @()
    }
...
function Get-KnowledgeLookupKeyRegistryConfig {
    ...
    Assert-KnowledgeJsonObjectKeys $registry $rootKeys "root"
    ...
    foreach ($property in $registry.canonical_combining_class.PSObject.Properties) {
        ...
        if (-not (Test-KnowledgeJsonInteger $property.Value)) {
            throw "Lookup-key registry canonical combining classes must be integers from 1 through 255."
        }
        $value = [int]$property.Value
        if ($value -le 0 -or $value -gt 255) {
            throw "Lookup-key registry combining classes must be integers from 1 through 255."
        }
    ...
    foreach ($property in $registry.canonical_composition.PSObject.Properties) {
        ...
        if ($parts.Count -ne 2) {
            throw "Lookup-key registry composition key '$($property.Name)' must contain two code points."
        }
        ...
        if (-not (Test-KnowledgeJsonInteger $property.Value)) {
            throw "Lookup-key registry composition '$($property.Name)' must target an integer code point."
        }
