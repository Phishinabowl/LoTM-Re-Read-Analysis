...
$script:KnowledgeNumericLikePattern = (
    '^[+-]?(?:[0-9][0-9_]*|0[xX][0-9a-fA-F_]+|0[oO][0-7_]+|0[bB][01_]+|' +
    '[0-9][0-9_]*(?::[0-9_]+)+|(?:[0-9][0-9_]*\.[0-9_]*|\.[0-9_]+)' +
    '(?:[eE][+-]?[0-9_]+)?|[0-9][0-9_]*[eE][+-]?[0-9_]+|' +
...
function Get-KnowledgeYamlSchemaToken {
    ...
    if ($matches.Count -ne 1) {
        throw "$Context must declare exactly one root schema_version scalar."
    }
...
function Assert-KnowledgeYamlSource {
    ...
    if ($byteCount -gt $MaxBytes) {
        throw "$Context exceeds the $MaxBytes-byte YAML limit: $Path"
    }
    ...
    try {
        while ($parser.MoveNext()) {
            ...
            if ($type -eq 'AnchorAlias') {
                throw "$Context uses unsupported YAML aliases: $Path"
            }
            if (($type -eq 'DocumentStart' -or $type -eq 'DocumentEnd') -and -not $event.IsImplicit) {
                throw "$Context must be one implicit YAML document without document markers: $Path"
            }
            if ($event -is [YamlDotNet.Core.Events.NodeEvent]) {
                if (-not $event.Anchor.IsEmpty -or -not $event.Tag.IsEmpty) {
                    throw "$Context uses unsupported YAML anchors or tags: $Path"
                }
            }
            if ($type -eq 'MappingStart' -or $type -eq 'SequenceStart') {
                if ($isMappingKey) {
                    throw "$Context mapping keys must be scalar strings: $Path"
                }
                $depth++
                $nodeCount++
                if ($depth -gt $MaxDepth) {
                    throw "$Context exceeds YAML nesting depth $MaxDepth`: $Path"
                }
                if ($type -eq 'MappingStart') {
                    ...
                    $frames.Add([pscustomobject]@{kind='sequence'
                            expect_key=$false
                            seen_keys=$null
                        })
                }
            ...
            elseif ($type -eq 'Scalar') {
                ...
                if ($scalarBytes -gt $MaxScalarBytes) {
                    throw "$Context contains a scalar larger than $MaxScalarBytes UTF-8 bytes: $Path"
                }
                if ($isMappingKey) {
                    ...
                    if ($value -cnotmatch $script:KnowledgeCanonicalMappingKeyPattern) {
                        throw "$Context contains noncanonical mapping key '$value'; keys must be lowercase machine identifiers: $Path"
                    }
                    if (-not $parent.seen_keys.Add($value)) {
                        throw "$Context contains duplicate mapping key '$value': $Path"
                    }
                }
                if ($event.Style -eq [YamlDotNet.Core.ScalarStyle]::Plain) {
                    if ($value.Length -eq 0) {
                        throw "$Context must write null explicitly as lowercase 'null': $Path"
                    }
                    if ($value -ceq '<<') {
                        throw "$Context uses unsupported YAML merge keys: $Path"
                    }
                    if ($value -ceq '~' -or ($value.ToLowerInvariant() -eq 'null' -and $value -cne 'null')) {
                        throw "$Context must use lowercase 'null': $Path"
                    }
                    if (@('true', 'false') -contains $value.ToLowerInvariant() -and @('true', 'false') -cnotcontains $value) {
                        throw "$Context must use lowercase Boolean scalars: $Path"
                    }
                    if ($value -cmatch $script:KnowledgeNumericLikePattern -and $value -cnotmatch $script:KnowledgeCanonicalIntegerPattern) {
                        throw "$Context contains noncanonical numeric scalar '$value': $Path"
                    }
                    if ($value -cmatch $script:KnowledgeTimestampLikePattern) {
                        throw "$Context must quote date and timestamp strings: $Path"
                    }
                }
            }
            if ($nodeCount -gt $MaxNodes) {
                throw "$Context exceeds the $MaxNodes-node YAML limit: $Path"
            }
        }
    }
    catch {
...
function Assert-KnowledgeSchemaVersion {
    ...
    $value = $null
    if ($Mapping -is [System.Collections.IDictionary] -and $Mapping.Contains('schema_version')) {
        $value = $Mapping['schema_version']
    }
    elseif ($null -ne $Mapping) {
        $property = $Mapping.PSObject.Properties['schema_version']
        if ($null -ne $property) {
...
function ConvertFrom-KnowledgeYamlFile {
    ...
    if ($bytes.Length -gt $script:KnowledgeMaxYamlBytes) {
        throw "$Context exceeds the $($script:KnowledgeMaxYamlBytes)-byte YAML limit: $Path"
    }
    ...
    try {
        ...
        throw "$Context must be valid UTF-8: $Path`: $($_.Exception.Message)"
    }
    Assert-KnowledgeYamlSource $text $Context $Path
    try {
        $mapping = ConvertFrom-Yaml -Yaml $text -Ordered
    }
    catch {
        throw "Unable to parse $Context $Path`: $($_.Exception.Message)"
    }
    if ($mapping -isnot [System.Collections.IDictionary]) {
        throw "$Context root must be a mapping: $Path"
    }
...
function Test-KnowledgeRfc3339Timestamp {
    param([string]$Value)
    if ($Value -cnotmatch $script:KnowledgeRfc3339Pattern) {
        return $false
    }
    if ($Matches.sign -ceq '-' -and $Matches.hour -ceq '00' -and $Matches.minute -ceq '00') {
        return $false
    }
    $timeParts = $Matches.time.Split(':')
    if ([int]$timeParts[0] -gt 23 -or [int]$timeParts[1] -gt 59 -or [int]$timeParts[2] -gt 59) {
        return $false
    }
    if ($Matches.hour) {
        $hour = [int]$Matches.hour
        $minute = [int]$Matches.minute
        if ($minute -gt 59 -or $hour -gt 14 -or ($hour -eq 14 -and $minute -ne 0)) {
            return $false
        }
    }
