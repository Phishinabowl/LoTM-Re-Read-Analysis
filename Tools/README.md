...
Run the occurrence vectors after changing branches, templates, occurrence participations, track entries, recurrence patterns or executions, aggregate cardinalities, phases, schedules, lifecycle, bindings, tracks, transition profiles, causality, outcomes, scoped rules, deterministic evaluation, state acquisition, carryover, chronology composition, or provenance targets. Both implementations validate the empty LoTM project registry, compose `Framework/Data/Occurrence/` with controlled chronology contexts, answer the same participation, track-entry, ambiguity, cardinality, occurrence, boundary, phase, schedule, outcome, state, selection, suppression, and conflict-trace queries, and reject the same 102 malformed mutations. Participation records preserve subject, role, perspective, status, and optional chronology context independently from occurrence and subjective order. Cardinality claims cover exact, minimum, maximum, range, and unknown bounds with complete, representative, or unmaterialized history; generated 128-cardinality and 128-participation/track-entry probes protect bounded scale. Causal cycles are intentionally accepted; chronology, participation ownership, contiguous track-entry order, transition semantics, containment, monotonic recurrence order, lifecycle, typed rule targets, applicability, outcome compatibility, state-chain continuity, and carryover applicability remain constrained.
