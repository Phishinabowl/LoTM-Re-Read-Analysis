...
Run the dedicated schema-pack suite after changing pack shape, dependency composition, capability lifecycle or activation, capability groups or relationships, controlled-value ownership/hierarchy, typed semantic declarations, architectural classification, presentation metadata, or the declarative plugin and executable-extension boundaries. Both implementations load the canonical schema-6 project composition, validate all 14 catalog packs, 17 groups, and 136 capability presentations, then consume the independent schema-6 three-pack fixture, one-pack schema-4 compatibility fixture, 117 structured malformed mutations, and two delimiter-collision identity probes in `Framework/Data/Schema-Packs/`. The corpus includes capability-group ownership/contributions, relationship parity and hard-dependency cycles, explicit rejection of executable-extension manifests, package references, implementation bindings, hooks, permissions, credentials, commercial offerings, and entitlement fields, a capability-free bridge whose `pack_kind` remains independent from its architectural role, localized presentation, one legal vocabulary-only extension, typed state profiles, and state-kind mappings with required, optional, and forbidden dimension validation. A generated 64-pack composition with one owned typed declaration and complete presentation metadata per pack provides a bounded scale check; all operating-system temporary data is removed automatically.
...
Run the dedicated provenance suite after changing assertions, typed provenance subjects, field paths, evidence links or roles, point/range locators, source scope or coverage, observation/effective timing, claim applicability, supersession, or authority evaluation. Both implementations compose the canonical project dependencies and neutral source/entity/chronology/occurrence fixtures, then consume the schema-3 provenance fixture and 68 structured malformed mutations in `Framework/Data/Provenance/`. Twenty fixture assertions, including recurrence-cardinality, occurrence-participation, and occurrence-track-entry targets, five authority vectors, five invalid service queries, and a generated 128-assertion composition verify decision semantics, service boundaries, and bounded scale; all operating-system temporary data is removed automatically.
...
Run the occurrence vectors after changing branch identity, continuity, or lifecycle, templates, occurrence participations, track entries, recurrence patterns or executions, aggregate cardinalities, phases, schedules, bindings, tracks, transition profiles, causality, outcomes, scoped rules, deterministic evaluation, state profiles or acquisition, carryover, chronology composition, or provenance targets. Both implementations validate the empty LoTM project registry, compose `Framework/Data/Occurrence/` with controlled chronology contexts and continuity targets, answer the same branch-state, participation, track-entry, ambiguity, cardinality, occurrence, boundary, phase, schedule, outcome, state, selection, suppression, and conflict-trace queries, validate exact epistemic snapshots, and reject the same 125 malformed mutations. Branch-state transitions preserve append-only state history, activation occurrences, and required fork or merge triggers independently from chronology and provenance authority. Subject-state profiles keep availability, completeness, and attitude requirements explicit; change shape remains separate from mechanism; dimension chains remain continuous; and subject belief or access never establishes objective truth. Participation records preserve subject, role, perspective, status, and optional chronology context independently from occurrence and subjective order. Cardinality claims cover exact, minimum, maximum, range, and unknown bounds with complete, representative, or unmaterialized history; generated 128-branch-state, 128-cardinality, and 128-participation/track-entry probes protect bounded scale. Causal cycles are intentionally accepted; chronology, branch lineage and lifecycle, participation ownership, contiguous track-entry order, transition semantics, containment, monotonic recurrence order, typed rule targets, applicability, outcome compatibility, state-chain continuity, and carryover applicability remain constrained.
