...
Run the occurrence vectors after changing branches, templates, recurrence patterns or executions, aggregate cardinalities, phases, schedules, lifecycle, bindings, tracks, transition profiles, causality, outcomes, scoped rules, deterministic evaluation, state acquisition, carryover, chronology composition, or provenance targets. Both implementations validate the empty LoTM project registry, compose `Framework/Data/Occurrence/` with the chronology fixture, answer the same cardinality, occurrence, boundary, phase, schedule, outcome, state, selection, suppression, and conflict-trace queries, and reject the same malformed mutations. Cardinality claims cover exact, minimum, maximum, range, and unknown bounds with complete, representative, or unmaterialized history; a generated 128-record probe protects bounded scale. Causal cycles are intentionally accepted; chronology, transition semantics, containment, monotonic track order, lifecycle, typed rule targets, applicability, outcome compatibility, state-chain continuity, and carryover applicability remain constrained.
