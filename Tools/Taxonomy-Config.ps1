...
function ConvertTo-ContentTypeConfig {
    ...
    Assert-KnowledgeMapKeys $RawContentType $contentTypeKeys "Taxonomy registry '$context'"
...
function ConvertTo-CategoryConfig {
    ...
    Assert-KnowledgeMapKeys $RawCategory $categoryKeys "Taxonomy registry '$context'"
...
function Get-KnowledgeTaxonomyReconciliationTargets {
    ...
    return [ordered]@{"content-type"=$TaxonomyConfig.content_types
        category=$TaxonomyConfig.categories
    }
}

function Get-KnowledgeTaxonomyReconciliationProvider {
    param([object]$TaxonomyConfig)
    return [pscustomobject]@{provider_id="taxonomy"
        targets=(Get-KnowledgeTaxonomyReconciliationTargets $TaxonomyConfig)
        aliases=[ordered]@{}
    }
}
...
function Get-KnowledgeTaxonomyReconciliationTarget {
    ...
    $targets = switch ($TargetType) {
        ...
        default {
            throw "Unsupported taxonomy reconciliation target type '$TargetType'."
        }
    }
