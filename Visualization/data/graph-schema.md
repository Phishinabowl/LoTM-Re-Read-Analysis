...
Type-specific data blocks may expose `first_appearance_beats` or a type-local equivalent as the structured companion to visible First Appearance prose. These rows are positioned reveal beats, not relationship records and not continuing state rows by default. They support future website rendering, Obsidian QA mirror display, and reader-boundary graph display through `position`, `source_refs`, and optional `graph_display` rather than through a relationship-style availability ladder. A row with `graph_display.behavior: anonymized-node` may generate an opaque presentation node before the canonical page/title is eligible under `Subject Visible From`; once the configured boundary reaches `graph_display.resolves_to_canonical_at`, graph generation should use the canonical node instead. Add row-level `availability` to a first-appearance beat only when the beat itself has a meaningful changing interpretation that separate reveal beats cannot express.
...
Graph generators should apply page visibility, first-appearance display overrides, row availability, and relationship display state in that order. First-appearance display overrides come from positioned reveal fields such as `position` and `graph_display`, while relationship/current-state filtering comes from projected row `availability`. Hidden source or target pages should suppress the edge unless an eligible anonymized first-appearance display node can stand in for that canonical endpoint, or unless the current availability entry explicitly provides an anonymized or partial display label. Anonymized nodes are presentation artifacts, not canonical glossary nodes.
