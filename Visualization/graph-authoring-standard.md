...
When novel EPUB source expansion is available and suitable for the graph request, use `Tools/Commands/Media/search_epub.py` as the preferred first search path for bounded chapter sweeps, counts, snippets, and repeatable checks. If Python is unavailable, use the Windows PowerShell fallback `Tools/Commands/Media/Search-Epub.ps1`. Fall back to another structured EPUB search only if both helpers are missing or unusable, and report that degraded path in the output.
...
If EPUB source access is available, run the source-search portions of this pass through `Tools/Commands/Media/search_epub.py` when possible, or `Tools/Commands/Media/Search-Epub.ps1` as the Windows PowerShell fallback. Use bounded chapter ranges, count sweeps, chapter-ordered snippets, and repeated vocabulary expansion instead of one-off manual text scans.
...
- Run the availability checks in [rendering.md](rendering.md) before choosing a helper on unfamiliar machines.
- Preferred Python: `python Visualization\visualize.py --mode Refresh` for canonical refreshes.
- Preferred Python: `python Visualization\visualize.py --mode Render` for manual or agent-drafted Mermaid files.
- PowerShell fallback: `Visualization/visualize.ps1` supports both modes.
