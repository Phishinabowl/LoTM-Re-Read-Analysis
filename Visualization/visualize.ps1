...
function Resolve-VisualizationMode {
    ...
    switch -Regex ($Name) {
        '^(?i:refresh|update|generate)$' {
            return "Refresh"
        }
        '^(?i:render|manual-render|pure-render)$' {
            return "Render"
        }
        '^(?i:validate|check|test)$' {
            return "Validate"
        }
        '^(?i:qa-?relationship|qa-relationship-graph|unbounded-relationship)$' {
            return "QaRelationship"
        }
        default {
            throw (
                "Unsupported visualization mode: $Name. Use Refresh, Render, Validate, or QaRelationship. " +
                'Aliases include Update, Generate, Manual-Render, Pure-Render, Check, Test, ' +
                'Qa-Relationship-Graph, and Unbounded-Relationship.'
            )
        }
    }
}
...
function Get-MermaidRenderSize {
    ...
    $unit = if ($Settings.autoSize.complexityUnit) {
        [double]$Settings.autoSize.complexityUnit
    }
    else {
        40.0
    }
    ...
    $widthStep = if ($Settings.autoSize.widthStep) {
        [int]$Settings.autoSize.widthStep
    }
    else {
        1200
    }
    $heightStep = if ($Settings.autoSize.heightStep) {
        [int]$Settings.autoSize.heightStep
    }
    else {
        600
    }
    $maxWidth = if ($Settings.autoSize.maxWidth) {
        [int]$Settings.autoSize.maxWidth
    }
    else {
        $width
    }
    $maxHeight = if ($Settings.autoSize.maxHeight) {
        [int]$Settings.autoSize.maxHeight
    }
    else {
        $height
    }
    $fanOutThreshold = if ($Settings.autoSize.fanOutThreshold) {
        [int]$Settings.autoSize.fanOutThreshold
    }
    else {
        6
    }
    $fanOutWidthStep = if ($Settings.autoSize.fanOutWidthStep) {
        [int]$Settings.autoSize.fanOutWidthStep
    }
    else {
        900
    }
...
function Get-MermaidLayoutValidation {
    ...
    $sectionClasses = if ($null -ne $layoutSettings -and $null -ne $layoutSettings.sectionClassNames) {
        @($layoutSettings.sectionClassNames)
    }
    else {
        @("group")
    }
    $crossSectionTargetClasses = if ($null -ne $layoutSettings -and $null -ne $layoutSettings.crossSectionTargetClasses) {
        @($layoutSettings.crossSectionTargetClasses)
    }
    else {
        @("holder", "sequence")
    }
    $duplicateLabelIgnoreClasses = if ($null -ne $layoutSettings -and $null -ne $layoutSettings.duplicateLabelIgnoreClasses) {
        @($layoutSettings.duplicateLabelIgnoreClasses)
    }
    else {
        @("relationship")
    }
    $proxyNodeIdPatterns = if ($null -ne $layoutSettings -and $null -ne $layoutSettings.proxyNodeIdPatterns) {
        @($layoutSettings.proxyNodeIdPatterns)
    }
    else {
        @("(^|_)ref(erence)?$", "(^|_)proxy$", "_ref_", "_proxy_")
    }
    $proxyLabelPatterns = if ($null -ne $layoutSettings -and $null -ne $layoutSettings.proxyLabelPatterns) {
        @($layoutSettings.proxyLabelPatterns)
    }
    else {
        @("reference", "proxy", "see ", "reconstruction", "summary")
    }
    $denseGraphSettings = if ($null -ne $layoutSettings -and $null -ne $layoutSettings.denseGraphValidation) {
        $layoutSettings.denseGraphValidation
    }
    else {
        $null
    }
    $orderedSeriesSettings = if ($null -ne $layoutSettings -and $null -ne $layoutSettings.orderedSeriesValidation) {
        $layoutSettings.orderedSeriesValidation
    }
    else {
        $null
    }
    ...
    if ($null -ne $denseGraphSettings -and [bool]$denseGraphSettings.enabled) {
        ...
        $minNodeCount = if ($denseGraphSettings.minNodeCount) {
            [int]$denseGraphSettings.minNodeCount
        }
        else {
            20
        }
        if ($graphNodeIds.Count -ge $minNodeCount) {
            if ([bool]$denseGraphSettings.requireClassDefinitions -and $classDefs.Count -eq 0) {
                $issues += (
                    (
                        'Dense graph has {0} nodes but no `classDef` styling. Use styled node classes ' +
                        'so readers can distinguish groups, entities, relationships, evidence, ' +
                        'uncertainty, and other semantic roles.'
                    ) -f $graphNodeIds.Count
                )
            }

            $maxSubgraphCount = if ($denseGraphSettings.maxSubgraphCount -ne $null) {
                [int]$denseGraphSettings.maxSubgraphCount
            }
            else {
                4
            }
            if ($subgraphCount -gt $maxSubgraphCount) {
                $issues += (
                    (
                        'Dense graph uses {0} Mermaid subgraph clusters. Dense knowledge maps should usually ' +
                        'use styled group nodes and a connected semantic spine; reserve subgraph clusters ' +
                        'for a few large regions or explicitly requested cluster views.'
                    ) -f $subgraphCount
                )
            }
            ...
            $maxDisconnectedComponents = if ($denseGraphSettings.maxDisconnectedComponents -ne $null) {
                [int]$denseGraphSettings.maxDisconnectedComponents
            }
            else {
                2
            }
            if ($componentCount -gt $maxDisconnectedComponents) {
                $issues += (
                    (
                        'Dense graph has {0} disconnected components. Dense knowledge maps should usually ' +
                        'have a connected semantic spine, such as root -> group -> entity -> detail, ' +
                        'unless the user explicitly requests separate disconnected diagrams.'
                    ) -f $componentCount
                )
            }
        }
    ...
    foreach ($edge in $edges) {
        ...
        if ($otherIncoming.Count -gt 0) {
            ...
            $issues += (
                (
                    'Section node `{0}` links directly to `{1}`, but `{1}` already has non-section ' +
                    'incoming owner(s): {2}. Use a local reference/proxy node inside the section instead.'
                ) -f
                $edge.Source,
                $edge.Target,
                $owners
    ...
    if ($null -ne $orderedSeriesSettings -and [bool]$orderedSeriesSettings.enabled) {
        $maxDirectChildren = if ($orderedSeriesSettings.maxDirectChildren) {
            [int]$orderedSeriesSettings.maxDirectChildren
        }
        else {
            2
        }
        ...
        foreach ($edgeGroup in @($edges | Group-Object Source)) {
            ...
            if ($orderedChildren.Count -gt $maxDirectChildren) {
                $issues += (
                    (
                        'Node `{0}` has {1} direct ordered-series children: {2}. Ordered ladders, timelines, ' +
                        'ranks, phases, chapters, steps, and sequences should usually chain child-to-child ' +
                        'or use intermediate grouping nodes instead of wide sibling fan-out.'
                    ) -f
                    $edgeGroup.Name,
    ...
    if ($null -ne $validationSettings -and $null -ne $validationSettings.uncertaintyClasses) {
        $stylePattern = if ($validationSettings.uncertaintyClassStylePattern) {
            [string]$validationSettings.uncertaintyClassStylePattern
        }
        else {
            "stroke-dasharray"
        }
...
function Invoke-MermaidRender {
    ...
    Write-Output (
        'Rendering {0} -> {1} ({2}x{3}, nodes={4}, edges={5}, maxFanOut={6})' -f
        $InputPath,
        $OutputPath,
        $renderSize.Width,
        $renderSize.Height,
        $renderSize.NodeCount,
        $renderSize.EdgeCount,
        $renderSize.MaxFanOut
    )
    & mmdc @args
    ...
    foreach ($file in $files) {
        ...
        foreach ($block in Get-FencedYamlBlocks $text) {
            ...
            function Add-FirstAppearanceDisplay {
                ...
                $behavior = if ($GraphDisplay.ContainsKey("behavior")) {
                    $GraphDisplay["behavior"]
                }
                else {
                    ""
                }
                $label = if ($GraphDisplay.ContainsKey("label")) {
                    $GraphDisplay["label"]
                }
                else {
                    ""
                ...
                $medium = if ($GraphDisplay.ContainsKey("visible_from_medium")) {
                    $GraphDisplay["visible_from_medium"]
                }
                elseif ($Row.ContainsKey("position_medium")) {
                    $Row["position_medium"]
                }
                elseif ($Row.ContainsKey("medium")) {
                    $Row["medium"]
                }
                else {
                    ""
                }
                $volume = if ($GraphDisplay.ContainsKey("visible_from_volume")) {
                    $GraphDisplay["visible_from_volume"]
                }
                elseif ($Row.ContainsKey("position_volume")) {
                    $Row["position_volume"]
                }
                else {
                    ""
                }
                $chapter = if ($GraphDisplay.ContainsKey("visible_from_chapter")) {
                    $GraphDisplay["visible_from_chapter"]
                }
                elseif ($Row.ContainsKey("position_chapter")) {
                    $Row["position_chapter"]
                }
                else {
                    ""
                }
                ...
                $Displays[$CanonicalNodeId] += [pscustomobject]@{
                    ...
                    volume = $volume
                    chapter = $chapter
                    season = if ($GraphDisplay.ContainsKey("visible_from_season")) {
                        $GraphDisplay["visible_from_season"]
                    }
                    elseif ($Row.ContainsKey("position_season")) {
                        $Row["position_season"]
                    }
                    else {
                        ""
                    }
                    episode = if ($GraphDisplay.ContainsKey("visible_from_episode")) {
                        $GraphDisplay["visible_from_episode"]
                    }
                    elseif ($Row.ContainsKey("position_episode")) {
                        $Row["position_episode"]
                    }
                    else {
                        ""
                    }
                    release_order = if ($GraphDisplay.ContainsKey("visible_from_release_order")) {
                        $GraphDisplay["visible_from_release_order"]
                    }
                    elseif ($Row.ContainsKey("position_release_order")) {
                        $Row["position_release_order"]
                    }
                    else {
                        ""
                    }
                    resolves_medium = if ($GraphDisplay.ContainsKey("resolves_to_canonical_at_medium")) {
                        $GraphDisplay["resolves_to_canonical_at_medium"]
                    }
                    else {
                        ""
                    }
                    resolves_volume = if ($GraphDisplay.ContainsKey("resolves_to_canonical_at_volume")) {
                        $GraphDisplay["resolves_to_canonical_at_volume"]
                    }
                    else {
                        ""
                    }
                    resolves_chapter = if ($GraphDisplay.ContainsKey("resolves_to_canonical_at_chapter")) {
                        $GraphDisplay["resolves_to_canonical_at_chapter"]
                    }
                    else {
                        ""
                    }
            ...
            foreach ($rawLine in ($block.Text -split "`r?`n")) {
                ...
                if ($indent -eq 0 -and -not $line.StartsWith("- ")) {
                    ...
                    $rootKey = if ([string]::IsNullOrWhiteSpace((ConvertFrom-Scalar $value))) {
                        $key.Trim()
                    }
                    else {
                        ""
                ...
                if ($rootKey -and $indent -eq 2 -and -not $line.StartsWith("- ")) {
                    ...
                    $graphDisplay = @{}
                    $key, $value = $line.Split(":", 2)
                    $sectionKey = if ([string]::IsNullOrWhiteSpace((ConvertFrom-Scalar $value))) {
                        $key.Trim()
                    }
                    else {
                        ""
...
function Get-AnonymizedNodeDisplays {
    ...
    $nodeAliases = @{}
    if ($null -eq $Boundary) {
        return [pscustomobject]@{ Nodes = $displayNodes
            Aliases = $nodeAliases
        }
    }
...
function Format-AvailabilityHistory {
    ...
    foreach ($entry in @($Entries)) {
        ...
        $medium = if ([string]::IsNullOrWhiteSpace($entry.medium)) {
            "unknown"
        }
        else {
            $entry.medium
...
function Get-RelationshipScore {
    ...
    if (-not [string]::IsNullOrWhiteSpace($Relationship.projection_source)) {
        $score += 100
    }
    if ($Relationship.projection_scope -eq "canonical") {
        $score += 10
    }
...
function Select-RelationshipsForBoundary {
    ...
    foreach ($relationship in @($Relationships)) {
        ...
        $rendered | Add-Member -NotePropertyName render_source_node -NotePropertyValue $(if ($NodeAliases.ContainsKey($sourceNode)) {
                $NodeAliases[$sourceNode]
            }
            else {
                $sourceNode
            }) -Force
        $rendered | Add-Member -NotePropertyName render_target_node -NotePropertyValue $(if ($NodeAliases.ContainsKey($targetNode)) {
                $NodeAliases[$targetNode]
            }
            else {
                $targetNode
            }) -Force
        ...
        $hasProjection = (
            $DataProjections.ContainsKey($namespacedProjectionSource) -or
            $DataProjections.ContainsKey($relationship.projection_source)
        )
        if (-not [string]::IsNullOrWhiteSpace($relationship.projection_source) -and $hasProjection) {
...
function Write-MermaidGraph {
    ...
    foreach ($relationship in $Relationships) {
        $source = if ($relationship.PSObject.Properties.Name -contains "render_source_node" -and -not [string]::IsNullOrWhiteSpace($relationship.render_source_node)) {
            $relationship.render_source_node
        }
        else {
            Convert-SlugToNodeId $relationship.source
        }
        $target = if ($relationship.PSObject.Properties.Name -contains "render_target_node" -and -not [string]::IsNullOrWhiteSpace($relationship.render_target_node)) {
            $relationship.render_target_node
        }
        else {
            Convert-SlugToNodeId $relationship.target
        }
    ...
    foreach ($nodeId in @($Nodes.Keys | Sort-Object)) {
        ...
        if ($missingEndpointNodes.Contains($nodeId)) {
            $className = if ($pendingEndpointNodes.Contains($nodeId)) {
                "pendingEndpoint"
            }
            else {
                "missingEndpoint"
...
function Write-UnboundedRelationshipGraph {
    ...
    $graphArguments = @{
        GraphPath = $OutputGraphPath
        Nodes = $nodes.Clone()
        Relationships = $filteredRelationships
        KnownNodeIds = @($nodes.Keys)
        PendingNodeIds = $pendingNodeIds
        PendingEndpointNodeIds = $pendingEndpointNodeIds
        IncludeConfirmedConfidence = $IncludeConfirmedConfidence
    }
    Write-MermaidGraph @graphArguments
}
...
function Update-MermaidGraphs {
    ...
    foreach ($view in $Views) {
        ...
        $relationshipArguments = @{
            ...
            Boundary = $view.readerBoundary
            VisibleNodeIds = @($viewNodes.Keys)
            KnownNodeIds = @($nodes.Keys)
            DataProjections = $dataProjections
            TimingSpoilerFree = $timingSpoilerFree
            NodeAliases = $displayState.Aliases
        }
        $viewRelationships = Select-RelationshipsForBoundary @relationshipArguments
        ...
        $graphArguments = @{
            GraphPath = $graphPath
            Nodes = $viewNodes.Clone()
            Relationships = $viewRelationships
            TimingSpoilerFree = $timingSpoilerFree
            KnownNodeIds = @($nodes.Keys)
            PendingNodeIds = $visiblePendingNodeIds
            PendingEndpointNodeIds = $pendingEndpointNodeIds
        }
        Write-MermaidGraph @graphArguments
    }
...
function Invoke-RefreshMode {
    ...
    $previousViewCount = if ($null -eq $previousSnapshot) {
        0
    }
    else {
        @($previousSnapshot.views).Count
    }
    $previousRenderedCount = if ($null -eq $previousSnapshot) {
        0
    }
    else {
        @($previousSnapshot.rendered_files).Count
    }
    $previousBrokenLinkCount = if ($null -eq $previousSnapshot) {
        0
    }
    else {
        @($previousSnapshot.broken_links).Count
    }
    $previousPendingCount = if ($null -eq $previousSnapshot) {
        0
    }
    else {
        @($previousSnapshot.pending_nodes).Count
    }
    ...
    for ($index = 0; $index -lt $viewStats.Count; $index += 1) {
        ...
        $changedRelationships = if ($null -eq $previousSnapshot) {
            @()
        }
        else {
            Get-ChangedRelationships @($previousView.relationships) @($view.relationships)
        }
    ...
    foreach ($viewReport in $viewReports) {
        ...
        $sections = @(
            [pscustomobject]@{ Title = "Added Nodes"
                Items = @($viewReport.node_diff.Added)
            },
            [pscustomobject]@{ Title = "Removed Nodes"
                Items = @($viewReport.node_diff.Removed)
            },
            [pscustomobject]@{ Title = "Added Relationships"
                Items = @($viewReport.relationship_diff.Added)
            },
            [pscustomobject]@{ Title = "Removed Relationships"
                Items = @($viewReport.relationship_diff.Removed)
            },
            [pscustomobject]@{ Title = "Duplicate Relationships"
                Items = @($viewReport.duplicate_relationships)
            },
            [pscustomobject]@{ Title = "Orphan Nodes"
                Items = @($viewReport.view.orphan_nodes)
            }
        )
