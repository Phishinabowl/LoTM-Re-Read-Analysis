...
`Project_Config/sources.yaml` is the machine-readable authority for work groups, works, structural work segments, overlapping segment groups, numbering and ordering schemes, continuities, authority profiles, per-work volumes, adaptation mappings, manifestations and version mappings, release components/packages/runs/events, territories, platforms, provider catalog placements and offerings, external identifiers, evidence sources, media facets, typed relationships, position schemas, citation formats, aliases, evidence modes, comparison groups, priority, container formats, and bindings to registered resources. Work groups represent franchises, ordered series, heterogeneous adaptation programs, or collections. Works represent creative units; release forms distinguish novels, television seasons, specials, films, and other packaging while medium profiles retain reader-position behavior. Segments provide configurable chapter, issue, episode, scene, or other work-part identity. `parent_segment_id` owns structural containment only. Use `segment_groups` when one segment belongs to overlapping arcs, sagas, crossovers, publication units, collections, or reading paths. Numbering schemes preserve labels such as issue numbers and production codes without implying order. Named total or partial ordering schemes preserve publication, release, story, production, and recommended orders independently. Continuities model compatible canon scopes. Manifestations represent whole-work or segment-scoped editions, translations, cuts, recuts, remasters, or builds; manifestation segment mappings record structural differences between related versions. Release components may originate in a manifestation or exist only through package membership. Packages group commercial material; release runs describe regular schedules and exceptions; concrete events record observed launches. Platform offerings record access windows; localized provider catalog placements preserve service-specific grouping. Sources remain concrete evidence artifacts, declare every work in scope through `work_ids`, and identify the release material they observe. Do not collapse those layers.

Territory references use stable hierarchical IDs registered in `territories`; language values use BCP-47-style tags. Provider catalog structure is not canonical creative hierarchy. A streaming service's “Season,” “Part,” “Volume,” or collection label belongs in `catalog_placements` unless the underlying unit independently qualifies as a work. Localized titles may belong to works, segments, segment groups, manifestations, packages, or provider placements; localized entity names remain deferred until entity-level localization has an executable contract. Release runs, events, and offering times use structured windows with precision and certainty instead of opaque date prose.
