...
`Project_Config/sources.yaml` is the machine-readable authority for work groups, works, work segments, named ordering schemes, continuities, authority profiles, per-work volumes, adaptation mappings, manifestations, release components and events, platforms, provider catalog placements and offerings, evidence sources, media facets, typed relationships, position schemas, citation formats, aliases, evidence modes, comparison groups, priority, container formats, and bindings to registered resources. Work groups represent franchises, ordered series, heterogeneous adaptation programs, or collections. Works represent creative units; release forms distinguish novels, television seasons, specials, films, and other packaging while medium profiles retain reader-position behavior. Segments provide configurable chapter, issue, episode, scene, or other work-part identity. Named total or partial ordering schemes preserve publication, release, story, production, and recommended orders independently, including concurrent or incomparable branches. Continuities model compatible canon scopes. Manifestations represent editions, translations, cuts, recuts, remasters, or builds of a work. Release components represent tracks and bundled components; release events record launches; platform offerings record access windows; provider catalog placements preserve service-specific grouping. Sources remain concrete evidence artifacts and declare the manifestation or release material they observe. Do not collapse those layers.

Territory references use stable IDs registered in `territories`; language values use BCP-47-style tags. Provider catalog structure is not canonical creative hierarchy. A streaming service's “Season,” “Part,” “Volume,” or collection label belongs in `catalog_placements` unless the underlying unit independently qualifies as a work. Localized work titles belong to manifestations or provider placements; localized entity names remain deferred until entity-level localization has an executable contract.
