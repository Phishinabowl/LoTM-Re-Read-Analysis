...
Continuity membership and relationship statuses come from the selected schema pack rather than a loader-local allowlist. Work-level primary, adaptation, and parody continuities are executable, as are work-production/rights contexts. Segment- or claim-scoped continuity, entity incarnations, retcons, branching routes, preservation state, contributor credits, and textual witnesses remain explicitly planned capabilities. Do not approximate those unavailable contracts with page-local fields; implement paired Python and PowerShell validation and promote the capability before storing dependent records.
