...
Human-facing aliases and explicitly case-insensitive semantic values must use the manifest-selected lookup-key registry through `Tools/lookup_key_config.py` or `Tools/Lookup-Key-Config.ps1`, then compare normalized keys ordinally. Do not use runtime-default lowercase/case-fold APIs or case-insensitive dictionaries for those values. Do not apply semantic lookup normalization to canonical stable IDs, schema keys, paths, or BCP-47 language tags, and do not treat it as fuzzy matching. Updating the Unicode version or normalization algorithm requires a reviewed migration and Python, PowerShell 7, and Windows PowerShell 5.1 parity testing.
...
- `Tools`: Repeatable local maintenance, source-search, artwork, visualization, Obsidian export, formatting, and cleanup helpers. Prefer Python helpers when Python is available and repository Python requirements from `requirements-python.txt` are installed; use documented PowerShell fallbacks when Python is unavailable. Paired validation and conformance commands that print summaries must provide matching `--json` / `-Json` structured output. Use `Tools/Test-Python.ps1` and `Tools/Test-PowerShell.ps1` as read-only environment probes for unfamiliar machines or fresh agent sessions, then retain the results as session state instead of rerunning them before every command. Some PowerShell tools have module requirements tracked in `requirements-powershell.txt`. The frozen package/module, command, conformance, compatibility, and static-tool boundaries live in `ARCHITECTURE.md`; until the atomic migration, do not add another root-level loader or opportunistically move one command outside that plan.
