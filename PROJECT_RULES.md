...
`Project_Config/reconciliation.yaml` is the machine-readable audit authority for stable IDs that redirect, merge, split, or retire. Never delete a historical identity and reuse its ID, and never approximate reconciliation by collapsing aliases. A tombstone source must be absent from its current provider while its record reserves the ID; a present source must still resolve in its provider. Operations preserve target type, active chains must be acyclic and terminate at current records, and splits remain explicitly ambiguous. The reconciliation loaders resolve identity only. File/folder moves, slug changes, reference rewrites, and alias updates remain explicit migration-service work.
