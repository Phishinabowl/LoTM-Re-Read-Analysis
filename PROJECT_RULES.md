...
Use the shared temporal contract for registry timing. `Tools/temporal_config.py` and `Tools/Temporal-Config.ps1` own parsing and comparison; `Framework/Packs/core/pack.yaml` owns temporal window-kind, precision, and certainty vocabulary. A consumer may reference the shared kernel but must not introduce a registry-local temporal shape or value allowlist. Preserve open bounds, inclusivity, precision, certainty, and explicitly unknown time; never silently turn uncertain or unknown timing into an exact winner. Run both temporal conformance tools after changing this behavior. See `Framework/Contracts/temporal-model.md`.
