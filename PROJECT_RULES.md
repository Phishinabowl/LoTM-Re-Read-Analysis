...
Record creative lineage as a typed work relationship, not as a source derivation. Use `sequel-to`, `spinoff-of`, `side-story-of`, `adaptation-of`, `remake-of`, `retelling-of`, `crossover-with`, containment/compilation, or inspiration relationships as appropriate. Record evidence-artifact lineage separately with source relationships such as `edition-of`, `translation-of`, `transcript-of`, `subtitle-track-of`, `dub-of`, `scan-of`, `extract-from`, or `packaged-with`. Add new controlled relationship types with a valid reciprocal inverse rather than inventing page-local strings.
...
Continuity membership and relationship statuses come from the selected schema pack rather than a loader-local allowlist. Work-level continuity is currently executable. Segment- or claim-scoped continuity, entity incarnations, retcons, branching routes, preservation state, contributor credits, licensing records, and textual witnesses remain explicitly planned capabilities. Do not approximate those unavailable contracts with page-local fields; implement paired Python and PowerShell validation and promote the capability before storing dependent records.
