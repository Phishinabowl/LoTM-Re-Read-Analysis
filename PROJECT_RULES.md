...
`Project_Config/project.yaml` is the domain-neutral repository manifest. Tooling uses it to locate and identify the project, assign stable IDs to modeled content and resource roots, and resolve the schema-pack, taxonomy, resource, source, entity, provenance, and pinned lookup-key registries plus QA output, visualization integration, and cleanup helpers. Directory names such as `Glossary_Threads/`, `Investigations/`, and `Volumes/` are LoTM configuration values rather than reusable framework assumptions.
...
state only the local validation, serialization, or operational consequence those surfaces own.
...
Effective-schema loading must select the catalog's validated pack records and fail when project and
framework IDs, lookup-registry pins, selected IDs, or configured pack paths disagree. Project
annotations belong only in `FrameworkCatalogProjectView`, which is derived from an unchanged base
catalog plus a completed effective schema.
...
`Project_Config/sources.yaml` is the machine-readable authority for work groups, works, structural work segments, content groups, numbering and ordering schemes, continuities, authority profiles, applicability scopes, scoped continuity, per-work production/right contexts and volumes, adaptation mappings, manifestations and version mappings, release components and lineage, packages/phased runs/events, territories, platforms, provider catalog placements and offerings, localized title variants, external identifiers, evidence sources and coverage, media facets, typed relationships, position schemas, citation formats, aliases, evidence modes, comparison groups, priority, container formats, and bindings to registered resources. Work groups represent franchises, ordered series, heterogeneous adaptation programs, or collections. Works represent creative units; release forms distinguish novels, television seasons, web series, specials, films, and other packaging while medium profiles retain reader-position behavior, explicitly name their canonical-work scope field, and may opt into a pack-owned volume-catalog or segment-ordering structural validator. `applicability_scopes` target a work, segment, content group, work relationship, adaptation mapping, manifestation, release component, release package, or stable provenance claim and independently carry territory, effective time, and explicit non-negative precedence. Empty territory scope means unspecified rather than worldwide. Scopes with the same target, exact territory set, and precedence cannot overlap in time. Use the paired applicability-decision APIs to discover scopes through explicit target containment, registered territory ancestry, and effective time; do not approximate applicability with shared-work IDs or hand-built candidate lists. Omitted query territory or time conservatively excludes narrower scopes, uncertain and explicitly unknown timing remain distinct non-winning outcomes, explicit precedence selects the winner set, and tied winners remain ambiguous. `work_production_contexts` retain one owning work and use a required scope for the exact applicability of production origin, authorization, rights basis, and commerciality. Segments provide configurable chapter, issue, episode, scene, or other work-part identity. `parent_segment_id` owns structural containment only. Use `content_groups` for overlapping or curated combinations of works, segments, and reusable nested groups; every member has a globally stable ID and controlled participation role, role does not imply order, and the combined parent/member graph must remain acyclic. Numbering schemes preserve labels such as issue numbers and production codes without implying order. Named total or partial ordering schemes preserve publication, release, story, production, and recommended orders independently. Continuities model compatible canon scopes. Manifestations represent whole-work or segment-scoped editions, translations, cuts, recuts, remasters, or builds; manifestation segment mappings record structural differences between related versions. Release components may originate in a manifestation or exist only through package membership; component relationships preserve revisions, translations, dubs, and derivation. Packages group commercial material; release runs partition an exact segment order into independently paced/batched phases and exceptions; concrete events record observed launches. Platform offerings record access windows; localized titles have stable IDs and historical variants in one locale scope require windows proven disjoint, while localized provider catalog placements preserve service-specific grouping. Sources remain concrete evidence artifacts, declare every work in scope through `work_ids`, declare their permitted locator media and controlled evidence modes, use typed `observations` when they inspect multiple release objects, and state coverage through stable entries whose optional ranges must identify one canonical work, contain required fields for the entry's `medium_id`, follow configured order, and satisfy available structural data. Every coverage entry also declares one or more source-supported evidence modes. Once coverage is nonempty, a locator must match a medium/mode entry and its bounds; overlapping declarations for the same target/channel are invalid. Assertion records and claim supersession are owned by `Project_Config/provenance.yaml`; the source registry supplies their evidence, coverage, position, and authority services. An empty source `coverage` list intentionally means that no narrower position bound has been declared. Do not collapse those layers.

`Project_Config/entities.yaml` is the machine-readable authority for optional conceptual entities, continuity-bound incarnations, and continuity-specific identity phases. An entity declares one primary taxonomy category plus every additional category it legitimately occupies; the primary must appear in that category list. Use entity relationships for distinct same-continuity or conceptual subjects connected through succession, namesakes, legacy, mantle holding, cloning, faction splintering, derivation, composite construction, inspiration, or class/instance identity. Composite, derived, and inspired relationships may name controlled basis roles without inventing contribution percentages. Store one direction of an inverse pair. Every asymmetric pair declares exactly one canonical direction, while semantically hierarchical pairs share an acyclic group. Labels and aliases may be shared by genuinely distinct records: use plural lookup when ambiguity is acceptable and strict singular lookup only when the caller can handle an ambiguity error; aliases may not collide with another stable ID. Use an incarnation only when adaptation, reboot, alternate-universe, branch, or crossover incompatibility makes one conceptual subject require separate continuity-scoped realizations. Use an identity phase only when the same entity or incarnation persistently remains itself within one continuity and the phase matters independently to claims, chronology, relationships, or projection. Do not turn disguises, titles, roles, pathway states, ages, ordinary power/state changes, clones, simulations, temporal duplicates, fusions, mantle holders, portrayals, designs, translations, manifestations, releases, pages, or graph nodes into phases or incarnations without the required identity distinction. Each incarnation names one primary continuity and may add genuine crossover memberships. Incarnation and phase appearances use stable bindings to source-registry applicability scopes. Phase succession is explicit, same-subject, same-continuity, inverse-normalized, and acyclic; do not infer it from labels or publication order. Typed identity/reconciliation target APIs are narrower than provenance targets. The LoTM registry remains empty until an actual identity distinction is modeled, and canonical pages will receive stable identity references only through a later reviewed migration.

`Project_Config/reconciliation.yaml` is the machine-readable audit authority for stable IDs that redirect, merge, split, retire, or explicitly reclassify. Never delete a historical identity and reuse its ID, and never approximate reconciliation by collapsing aliases. A tombstone source must be absent from its current provider and from that provider's alias keys; a present source must still resolve in its provider and may use only redirect-style compatibility. Active retirement requires a tombstone. Redirect, merge, and split preserve target type, while `reclassify` is the sole cross-type operation. Operation/reason pairs must be pack-approved. Every split remains branch-level ambiguous even when branches converge or all retire. Keep `resolution.max_records`, `max_targets_per_record`, `max_branches`, and `max_resolution_steps` as reviewed positive safety bounds. Preserve historical labels only when appropriate through snapshot, redacted, or omitted modes, and use repository-history or strict RFC 3339 explicit audit metadata according to the record's governance needs. The reconciliation loaders resolve identity only. File/folder moves, slug changes, reference rewrites, and alias updates remain explicit migration-service work.

Treat framework registry YAML as strict executable configuration. Require valid UTF-8 without a BOM. Mapping keys must be non-empty lowercase machine strings composed from letters, digits, `_`, `-`, or `.` and must be validated before dictionary construction. Duplicate keys, non-string or case-drifted keys, noncanonical value scalars, implicit or non-lowercase nulls, tags, aliases, anchors, merge keys, document markers, unsupported or coerced schema versions, unknown keys in closed mappings, case-drifted machine IDs, runtime-specific timestamp variants, and byte/parser-budget overruns are errors. Unquoted numerics are canonical integers only; field-specific decimal semantics require a separately reviewed contract. Paired Python and PowerShell loaders must share the ingestion contract in `Framework/Contracts/strict-configuration-ingestion.md`. Do not assume page-embedded YAML already satisfies that registry contract; page normalization remains separate reviewed work.

Use the shared temporal contract for registry timing. `Tools/Runtime/Python/knowledge_framework/temporal_config.py` and `Tools/Runtime/PowerShell/KnowledgeFramework/KnowledgeFramework.psd1` own parsing, precision-aware queries, and comparison; `Framework/Packs/core/pack.yaml` owns temporal window-kind, bound-kind, precision, and certainty vocabulary. A consumer may reference the shared kernel but must not introduce a registry-local temporal shape, timestamp resolution, or value allowlist. Preserve the distinction among omitted open bounds, explicit unknown bounds, inclusivity, precision, announced/approximate/uncertain timing, and completely unknown windows. Treat coarse query values as complete calendar units rather than midnight instants. Canonical RFC 3339 values permit at most six fractional digits and must normalize inside years `0001` through `9999`. Never silently turn a partial, uncertain, or unknown result into an exact winner. Run both temporal conformance tools after changing this behavior. See `Framework/Contracts/temporal-model.md`.

Use `Project_Config/chronology.yaml` for non-civil chronology coordinate systems, ordered eras, positions, spans, explicit ordering relationships, and cross-system anchors. Core owns calendar/era-ordinal/ordinal/relative mechanics, integer domains, direction, zero policy, certainty, and comparison through `Tools/Runtime/Python/knowledge_framework/chronology_config.py` and `Tools/Runtime/PowerShell/KnowledgeFramework/KnowledgeFramework.psd1`. Narrative media owns optional story-time roles and work/continuity/branch contexts. LoTM owns its actual Epoch labels and ordering in project configuration. Do not place fictional eras in reusable packs, force unrelated axes into one total order, infer cross-axis equivalence without a mapping, admit contradictory or cyclic exact ordering, or represent causation as chronological `before`. Keep story chronology, publication/release order, reader disclosure, and causal order independently addressable. See `Framework/Contracts/chronology-registry.md`.

Use `Project_Config/occurrences.yaml` for stable occurrence templates and instances, recurrence patterns and concrete executions, aggregate realized-history cardinality, iteration identity and lifecycle, non-overlapping execution phases, typed recurrence schedules, branch topology, chronology bindings, ordered subject tracks, profiled transitions, causal edges, typed outcomes and scoped recurrence rules, subject-state transitions, and state-referencing carryover. Cardinality claims use exact, minimum, maximum, range, or unknown bounds plus complete, representative, or unmaterialized coverage; they must not invent unstored iterations or describe expected future repetitions. Multiple source-specific cardinality claims may coexist as provenance-addressable records rather than forcing the occurrence loader to choose authority. Multiple occurrences may bind the same chronology position; primary bindings on one occurrence must be concurrent or incomparable. Track iteration order is monotonic, ordered transitions require forward evidence, recurrence exits leave the named containment tree, and semantic duplicates are invalid, including repeated conditions or effects inside one rule under different IDs. Model what a subject knows, remembers, believes, or can access as a state transition with an activation occurrence; do not infer it from encounter or track membership. Carryover references an already-applicable state transition and does not duplicate the payload. Pattern defaults and execution overrides use explicit applicability, priority, selection, and resolution groups; never encode project logic as an unrestricted expression. Packs extending occurrence behavior must use schema-3 typed semantic declarations, declare recurrence-pattern target scope when applicable, provide exactly one repetition policy per effect kind, and register each incompatibility pair under exactly one global or same-target scope. Consumers may inspect proposed effects and contributor lineage for diagnostics, but may execute only `authorized_effects` when `execution_disposition` is `authorized`; conflicts and indeterminate applicability fail closed. An indeterminate applicability selector stops blocking only when independently evaluated conditions prove that the rule cannot match. Treat civil-schedule range errors, invalid repetition, and incompatible selected effects as reportable decisions, not invitations to guess. Never encode reset or causality as a chronology cycle or reuse one occurrence identity across iterations. Core owns generic recurrence cardinality, recurrence policy, and state acquisition; domain packs specialize vocabulary and incompatibility policy; chronology owns exact order; provenance owns evidence, applicability, supersession, and objective claim status. LoTM remains empty beyond `main` until source-backed occurrences are deliberately modeled. See `Framework/Contracts/occurrence-recurrence-registry.md`.

Do not use recurrence rules as a substitute for versioned game rules, format legality, editorial policy, organizational controls, clinical protocols, or legal obligations. Those normative systems require explicit version, effective scope, precedence, override, exception, and compatibility semantics that are not currently executable. Keep editorial stewardship and approval separate from contributor credit, factual authority, canon status, legal ownership, and Git identity. Keep a scheduled or release event, its diegetic occurrence, each participant session, and any recording, replay, or reconstruction as separate records even when future relationships connect them.

`Project_Config/provenance.yaml` is the sole machine-readable authority for cross-registry factual assertions and claim-supersession chains. Assertions target stable records exposed by source, entity, reconciliation, and occurrence registry providers, retain a stable claim key, asserted-value snapshot, optional semantic field path and timing, and attach mode-specific point or range locators to registered evidence sources. Source authority profiles rank evidence, but the provenance service owns claim grouping and evaluation. Load project configuration and packs first, then sources, chronology with composed source targets, entities, occurrences with their subject providers, reconciliation, and provenance; do not place assertion or supersession collections back into subject registries.

Human-facing aliases and explicitly case-insensitive semantic values must use the manifest-selected lookup-key registry through `Tools/Runtime/Python/knowledge_framework/lookup_key_config.py` or `Tools/Runtime/PowerShell/KnowledgeFramework/KnowledgeFramework.psd1`, then compare normalized keys ordinally. Do not use runtime-default lowercase/case-fold APIs or case-insensitive dictionaries for those values. Do not apply semantic lookup normalization to canonical stable IDs, schema keys, paths, or BCP-47 language tags, and do not treat it as fuzzy matching. Updating the Unicode version or normalization algorithm requires a reviewed migration and Python, PowerShell 7, and Windows PowerShell 5.1 parity testing.

Territory references use stable hierarchical IDs registered in `territories`; language values use BCP-47-style tags. Provider catalog structure is not canonical creative hierarchy. A streaming service's “Season,” “Part,” “Volume,” or collection label belongs in `catalog_placements` unless the underlying unit independently qualifies as a work. Localized title variants may belong to works, segments, content groups, manifestations, packages, or provider placements and must distinguish official, short, transliterated, literal, marketing, provider, alternate, retired, and primary-display semantics as applicable. Localized entity names remain deferred until entity-level localization has an executable contract. Release-run phases, events, and offering times use structured windows with precision and certainty instead of opaque date prose.

Use `medium` for a reader/viewer position coordinate system, `book` for a canonical work ID, and a registered `source_id` for evidence provenance. Use `release_form_id` for the creative unit or packaging level and `container_format_ids` for concrete evidence containers. Current canonical work IDs include `lotm-1`, `lotm-2`, `lotm-donghua-season-1`, and one work for each released Donghua special. New evidence entries should use canonical IDs such as `source_id: lotm-book-1-novel`, `source_id: lotm-book-2-novel`, `source_id: lotm-donghua-release`, `source_id: lotm-donghua-subtitles`, or the matching special-release source. Familiar values such as `lotm`, `coi`, `lotm-donghua`, `novel`, `donghua-visual-audit`, and `donghua-subtitle-audit` remain typed input aliases; canonical stored IDs identify the intended layer. Do not apply source-registry validation indiscriminately to every field named `source`: Relationship Seeds use `source` for an entity slug, and some type-specific data rows use it for a causal or semantic source rather than evidence provenance.

Select an authority profile, claim namespace, and actual locator evidence mode before making cross-work or cross-source judgments. Authority profiles define accepted continuity memberships, continuity order, comparable derivative-work relationships, source-priority direction, and optional claim-authority rules. Claim rules may select source IDs, roles, media, or evidence modes. A mode-specific rule matches the actual mode or one of its configured broader ancestors, not merely any mode supported somewhere by the source. Broader namespaces likewise apply to descendant claims; the highest explicit positive `precedence` wins, tied winners are invalid, and the source's numeric priority is only the fallback when no rule matches. Consumers that explain or audit a decision should preserve source ID, evidence mode, winning rule and precedence, matched namespace, inheritance state, and fallback state rather than storing only the numeric rank. Use the registry comparison API for multiple candidates: different comparison groups are `incomparable`; equal best ranks are a `tie`. Use claim evaluation for assertions sharing a `claim_key`: equally authoritative different values are a `conflict`, while equal values remain a corroborating tie. Under `lotm-adaptation-comparison`, preserve source-scoped claims: novels lead canonical-content and dialogue claims, official artwork leads visual-design claims, subtitles lead localization claims, and official Donghua releases lead their release-metadata claims. Attach adaptation deviations to the derivative Donghua work without rewriting the novel. Authority never makes unrelated works comparable and never advances another work or medium's spoiler state.

Record creative lineage as a typed work relationship, not as a source derivation. Use `sequel-to`, `spinoff-of`, `side-story-of`, `adaptation-of`, `remake-of`, `retelling-of`, `parody-of`, `crossover-with`, containment/compilation, or inspiration relationships as appropriate. A parody is its own work, even when it reuses footage or replaces dialogue; use adaptation mappings for detailed basis correspondence rather than demoting it to a dub or manifestation. When only a source-work segment or other source-side unit participates, assign an applicability scope that resolves entirely inside the relationship's source work. Parody does not imply fan production, noncommerciality, authorization, or any legal basis. Store those independently in evidence-backed work-production contexts referencing appropriate scopes. Record evidence-artifact lineage separately with source relationships such as `edition-of`, `translation-of`, `transcript-of`, `subtitle-track-of`, `dub-of`, `scan-of`, `extract-from`, or `packaged-with`. Add new controlled relationship types with a valid reciprocal inverse rather than inventing page-local strings.

Use `adaptation_mappings` when correspondence is narrower than whole-work lineage. Map one or more basis works and optional registered segments to the target work or segments, record each basis role, and classify the mapping as direct, omitted, combined, split, reordered, expanded, condensed, adaptation-original, or recontextualized. Preserve every work's source-scoped claims; a mapping describes correspondence and deviation rather than replacing a basis work.

Continuity membership and relationship statuses come from the selected schema pack rather than a loader-local allowlist. Work-level primary, adaptation, and parody continuities are executable. Use source-owned `scoped_continuity_assertions` for segment-, content-group-, or claim-specific status and provenance-owned `claim_supersessions` for acyclic, shape-compatible replacement, reinterpretation, restoration, invalidation, or soft-reboot chains. The supersession scope must target the older claim. Entity relationships, incarnations, identity phases, and stable-ID reconciliation are executable through paired registries. Historical continuity-membership transitions, manifestation- or release-scoped continuity status, occurrence-linked continuity transitions, first-class continuity systems, sliding-chronology policies, versioned normative rulesets, and editorial-governance services are not yet executable. Branching routes, crossover events, preservation state, contributor credits, textual witnesses, and detailed rights grants/restrictions remain explicitly planned capabilities. Do not approximate unavailable contracts with page-local fields, labels, inferred chronology mappings, overloaded current-state records, or recurrence policy; implement paired Python and PowerShell validation and promote the capability before storing dependent records.

Model a television season, standalone special, character special, or film as its own work when it needs independent identity, release order, evidence sources, or spoiler timing. Give it an ordered membership in the appropriate adaptation program and membership in the shared adaptation continuity, then relate it to the work or works it adapts. The adaptation program order may interleave seasons, specials, and films and may be resequenced as planned releases become concrete without changing stable work IDs. Register each observed release as an `official-release` source. Add subtitle, transcript, or dub sources only when those evidence artifacts are actually available or verified; each should relate to the matching release source rather than being treated as the creative work.
...
- `Tools`: Repeatable local maintenance, source-search, artwork, visualization, Obsidian export, formatting, and cleanup helpers. Prefer Python helpers when Python is available and repository Python requirements from `requirements-python.txt` are installed; use documented PowerShell fallbacks when Python is unavailable. Paired validation and conformance commands that print summaries must provide matching `--json` / `-Json` structured output. Use `Tools/Commands/Environment/Test-Python.ps1` and `Tools/Commands/Environment/Test-PowerShell.ps1` as read-only environment probes for unfamiliar machines or fresh agent sessions, then retain the results as session state instead of rerunning them before every command. Some PowerShell tools have module requirements tracked in `requirements-powershell.txt`. The package/module, command, conformance, compatibility, and static-tool boundaries live in `ARCHITECTURE.md`; reusable loaders belong under `Tools/Runtime/`, executable workflows under `Tools/Commands/`, suites under `Tools/Conformance/`, and policy tools under `Tools/Static/`. Do not add new flat root-level scripts.
...
Aggregate validation reporting must follow `Framework/Contracts/validation-run-reporting.md`. Use
concise structured output for routine status and a confined detailed report for retained evidence;
use complete structured standard output only for semantic consumers such as isolated extraction.
Changing report verbosity must never change selected suites, checks, runtimes, fixtures, assertions,
or failure behavior. Hosted validation writes complete reports and prints them only on failure.
...
Every tracked or nonignored untracked annotation surface must pass `python Tools/Static/lint_work_annotations.py`. A normal run must execute its permanent valid/invalid fixture corpus before repository scanning. `Tools/Static/work-annotations.json` is the executable policy registry; keep its tags, owners, eligible extensions, self-exclusions, prohibited locations, and safety bounds synchronized with `WORK_ANNOTATION_STANDARDS.md` and Todo Tree settings. The linter checks syntax and mirrored-link consistency but does not replace live GitHub verification.
...
Use `Tools/Commands/Media/search_epub.py` for repeatable novel EPUB checks when Python is available. `Tools/Commands/Media/Search-Epub.ps1` is the Windows PowerShell fallback and should remain behaviorally compatible. Use `Tools/Commands/Environment/Test-Python.ps1` as the canonical local Python availability probe when the environment is unknown, and use `Tools/Commands/Environment/Test-PowerShell.ps1` when fallback module readiness is unknown; retain those results as session state instead of probing before every command. Exact commands, flags, aliases, examples, switch maps, dependency checks, and parity notes live in `Tools/README.md` and `Tools/TOOLING_REFERENCE.md`.
