...
`Project_Config/provenance.yaml` is the sole machine-readable authority for cross-registry factual assertions and claim-supersession chains. Assertions target stable records exposed by source, entity, and reconciliation registry providers, retain a stable claim key, asserted-value snapshot, optional semantic field path and timing, and attach mode-specific point or range locators to registered evidence sources. Source authority profiles rank evidence, but the provenance service owns claim grouping and evaluation. Load project configuration and packs first, then sources, chronology with composed source targets, entities, reconciliation, and provenance; do not place assertion or supersession collections back into subject registries.
