...
Treat framework registry YAML as strict executable configuration. Duplicate keys, noncanonical scalars, tags, aliases, anchors, merge keys, document markers, unsupported or coerced schema versions, unknown keys in closed mappings, case-drifted machine IDs, runtime-specific timestamp variants, and parser-budget overruns are errors. Paired Python and PowerShell loaders must share the ingestion contract in `Framework/Contracts/strict-configuration-ingestion.md`. Do not assume page-embedded YAML already satisfies that registry contract; page normalization remains separate reviewed work.
