...
Use `Project_Config/occurrences.yaml` for stable occurrence templates and instances, recurrence/iteration identity, branch topology, chronology-position bindings, ordered perspective tracks, profiled non-chronological transitions, causal edges, and payload-bearing carryover. Multiple distinct occurrences may bind the same chronology position. Multiple primary bindings on one occurrence must be concurrent or incomparable, never known ordered positions. A transition's domain `transition_kind` does not replace its core `transition_profile`, and their pair must be registered by a selected pack; track-attached transitions move forward on that track, and every child branch requires one matching branch-fork transition. Carryover must identify the participating track and exact stable payload target. Never encode a reset or causal loop as a chronology cycle, reuse one occurrence ID across iterations, or infer a participant's subjective sequence from world-time coordinates. Core owns the generic recurrence service; narrative media owns time-loop, subjective-experience, loop-reset/escape, and memory/knowledge/awareness vocabulary. LoTM project data must remain empty beyond its `main` branch until source-backed occurrences are deliberately modeled. See `Framework/Contracts/occurrence-recurrence-registry.md`.
