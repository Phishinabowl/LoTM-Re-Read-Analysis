...
`Project_Config/reconciliation.yaml` is the machine-readable audit authority for stable IDs that redirect, merge, split, retire, or explicitly reclassify. Never delete a historical identity and reuse its ID, and never approximate reconciliation by collapsing aliases. A tombstone source must be absent from its current provider and from that provider's alias keys; a present source must still resolve in its provider. Redirect, merge, and split preserve target type, while `reclassify` is the sole cross-type operation. Operation/reason pairs must be pack-approved. Every split remains branch-level ambiguous even when branches converge or all retire. Preserve historical labels only when appropriate through snapshot, redacted, or omitted modes, and use repository-history or explicit audit metadata according to the record's governance needs. The reconciliation loaders resolve identity only. File/folder moves, slug changes, reference rewrites, and alias updates remain explicit migration-service work.
