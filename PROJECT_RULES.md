...
Treat framework registry YAML as strict executable configuration. Require valid UTF-8 without a BOM. Duplicate keys, noncanonical scalars, implicit or non-lowercase nulls, tags, aliases, anchors, merge keys, document markers, unsupported or coerced schema versions, unknown keys in closed mappings, case-drifted machine IDs, runtime-specific timestamp variants, and byte/parser-budget overruns are errors. Unquoted numerics are canonical integers only; field-specific decimal semantics require a separately reviewed contract. Paired Python and PowerShell loaders must share the ingestion contract in `Framework/Contracts/strict-configuration-ingestion.md`. Do not assume page-embedded YAML already satisfies that registry contract; page normalization remains separate reviewed work.
