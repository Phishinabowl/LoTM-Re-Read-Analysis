...
Use `Project_Config/occurrences.yaml` for stable occurrence templates and instances, recurrence patterns and concrete executions, iteration identity and lifecycle, branch topology, chronology bindings, ordered subject tracks, profiled transitions, causal edges, typed outcomes and recurrence rules, subject-state transitions, and state-referencing carryover. Multiple occurrences may bind the same chronology position; primary bindings on one occurrence must be concurrent or incomparable. Track iteration order is monotonic, ordered transitions require forward evidence, recurrence exits leave the named containment tree, and semantic duplicates are invalid. Model what a subject knows, remembers, believes, or can access as a state transition with an activation occurrence; do not infer it from encounter or track membership. Carryover references an already-applicable state transition and does not duplicate the payload. Never encode reset or causality as a chronology cycle or reuse one occurrence identity across iterations. Core owns generic recurrence rules and state acquisition; domain packs specialize vocabulary; provenance owns evidence and objective claim status. LoTM remains empty beyond `main` until source-backed occurrences are deliberately modeled. See `Framework/Contracts/occurrence-recurrence-registry.md`.
