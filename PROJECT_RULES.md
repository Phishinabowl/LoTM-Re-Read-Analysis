...
Use `Project_Config/occurrences.yaml` for stable occurrence templates and instances, recurrence patterns and concrete executions, iteration identity and lifecycle, non-overlapping execution phases, typed recurrence schedules, branch topology, chronology bindings, ordered subject tracks, profiled transitions, causal edges, typed outcomes and scoped recurrence rules, subject-state transitions, and state-referencing carryover. Multiple occurrences may bind the same chronology position; primary bindings on one occurrence must be concurrent or incomparable. Track iteration order is monotonic, ordered transitions require forward evidence, recurrence exits leave the named containment tree, and semantic duplicates are invalid. Model what a subject knows, remembers, believes, or can access as a state transition with an activation occurrence; do not infer it from encounter or track membership. Carryover references an already-applicable state transition and does not duplicate the payload. Pattern defaults and execution overrides use explicit applicability, priority, selection, and resolution groups; never encode project logic as an unrestricted expression. Treat indeterminate applicability and incompatible selected effects as reportable decisions, not invitations to guess. Never encode reset or causality as a chronology cycle or reuse one occurrence identity across iterations. Core owns generic recurrence policy and state acquisition; domain packs specialize vocabulary and outcome incompatibility; chronology owns exact order; provenance owns evidence and objective claim status. LoTM remains empty beyond `main` until source-backed occurrences are deliberately modeled. See `Framework/Contracts/occurrence-recurrence-registry.md`.
