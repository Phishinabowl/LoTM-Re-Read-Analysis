...
`Project_Config/entities.yaml` is the machine-readable authority for optional conceptual entities and continuity-bound incarnations. An entity declares one primary taxonomy category plus every additional category it legitimately occupies; the primary must appear in that category list. Use entity relationships for distinct same-continuity or conceptual subjects connected through succession, namesakes, legacy, mantle holding, cloning, faction splintering, derivation, composite construction, inspiration, or class/instance identity. Composite, derived, and inspired relationships may name controlled basis roles that identify inherited semantic dimensions without inventing contribution percentages. Store one direction of an inverse pair. Every asymmetric pair declares exactly one canonical direction, while semantically hierarchical pairs share an acyclic group so the loaders reject impossible lineage loops. Unscoped edges participate in every scoped cycle check, but edges from separate explicit scopes remain independent. Labels and aliases may be shared by genuinely distinct records: use plural lookup when ambiguity is acceptable and strict singular lookup only when the caller can handle an ambiguity error; aliases may not collide with another stable ID. Use an incarnation only when adaptation, reboot, alternate-universe, branch, or crossover incompatibility makes one conceptual subject require separate continuity-scoped realizations. Do not split disguises, titles, roles, pathway states, ages, portrayals, designs, translations, manifestations, releases, pages, or generated graph nodes into incarnations. Each incarnation names one primary continuity and may add memberships for genuine crossover movement. Material appearances use stable bindings to source-registry applicability scopes; incarnation relationships use pack-owned types with explicit inverse semantics. Same-continuity identity phases and stable-ID reconciliation remain planned and must not be approximated with speculative incarnations or destructive alias merges. The LoTM registry remains empty until an actual continuity split or entity relationship is modeled, and canonical pages will receive stable entity/incarnation references only through a later reviewed migration.
