...
`Project_Config/sources.yaml` is the machine-readable authority for work groups, works, structural work segments, mixed-member content groups, numbering and ordering schemes, continuities, authority profiles, per-work volumes, adaptation mappings, manifestations and version mappings, release components and lineage, packages/phased runs/events, territories, platforms, provider catalog placements and offerings, localized title variants, external identifiers, evidence sources and coverage, registry provenance assertions, media facets, typed relationships, position schemas, citation formats, aliases, evidence modes, comparison groups, priority, container formats, and bindings to registered resources. Work groups represent franchises, ordered series, heterogeneous adaptation programs, or collections. Works represent creative units; release forms distinguish novels, television seasons, specials, films, and other packaging while medium profiles retain reader-position behavior. Segments provide configurable chapter, issue, episode, scene, or other work-part identity. `parent_segment_id` owns structural containment only. Use `content_groups` for overlapping or curated combinations of complete works and individual segments, such as arcs, sagas, crossovers, publication units, collections, or reading paths. Numbering schemes preserve labels such as issue numbers and production codes without implying order. Named total or partial ordering schemes preserve publication, release, story, production, and recommended orders independently. Continuities model compatible canon scopes. Manifestations represent whole-work or segment-scoped editions, translations, cuts, recuts, remasters, or builds; manifestation segment mappings record structural differences between related versions. Release components may originate in a manifestation or exist only through package membership; component relationships preserve revisions, translations, dubs, and derivation. Packages group commercial material; release runs partition an exact segment order into independently paced/batched phases and exceptions; concrete events record observed launches. Platform offerings record access windows; localized provider catalog placements preserve service-specific grouping. Sources remain concrete evidence artifacts, declare every work in scope through `work_ids`, identify the release material they observe, and may state precise typed coverage. `provenance_assertions` link registry facts or fields to canonical evidence sources without duplicating citation fields throughout the schema. Do not collapse those layers.
