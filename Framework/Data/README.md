...
`Occurrence/` contains the portable V31-V33 occurrence, recurrence, rule, and subject-state corpus. Its schema-3 fixture separates recurrence patterns from executions, validates lifecycle and monotonic subject tracks, types outcomes and bounded recurrence rules, records state acquisition independently from truth provenance, and makes carryover reference an applicable state transition. It retains repeated-coordinate, nested-loop, branch, transition-profile, and cyclic-causality coverage. Twenty-six query assertions and 47 mutation cases run identically in Python, PowerShell 7, and Windows PowerShell 5.1.
