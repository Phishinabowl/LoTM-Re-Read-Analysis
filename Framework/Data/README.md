...
`Occurrence/` contains the portable V31 occurrence and recurrence corpus. Its valid schema-1 fixture keeps repeated occurrences distinct at one chronology coordinate, orders participant tracks across loop resets, nests a recurrence, carries memory and knowledge between iterations, exits into a child branch, and permits a causal cycle without weakening chronological order. Eleven mutation cases reject malformed branch/recurrence topology, iteration identity, bindings, tracks, reset direction, subjects, fork references/ownership, and carryover.
