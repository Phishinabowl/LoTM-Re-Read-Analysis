...
`Chronology/` contains the portable domain-neutral chronology corpus. Its valid schema-1 fixture covers negative and five-digit years, era-local direction, ordinary and relative integer axes, descending counters, chronology spans, direct exact equivalence, incomparable coordinate systems, and explicit same-system relations. Malformed fixtures reject forbidden zero, missing eras, duplicate era ordinals, contradictory or duplicate exact order claims, exact order cycles, same-system mappings, missing or cyclic relative anchors, reversed spans, and unknown narrative targets.
