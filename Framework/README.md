...
through Obsidian QA. Capability groups provide stable wizard/editor navigation, while the Phase
3.3.1 declarative schema-pack contract makes the existing data-only plugin boundary explicit. Phase
3.3.2 separately defines trusted executable-extension identity, compatibility, trust, permission,
...
strict project-independent roadmap registry and paired loaders. Phase 3.4.3 projects validated
delivery traceability into catalog, project-view, effective-schema, inspection, and QA diagnostics
...
Framework packs must not contain LoTM works, pages, source records, repository paths, or other project instances. A project selects packs through `Project_Config/schema-packs.yaml`. Selection makes capability declarations discoverable; capability lifecycle determines availability, and the project activation policy determines which available capabilities are enabled. Lifecycle and future delivery traceability follow `Contracts/capability-lifecycle-and-roadmap.md`; delivery metadata remains separate from portable pack declarations. Packs remain declarative data and cannot supply executable code, host permissions, credentials, commercial offerings, or entitlement state; see `Contracts/declarative-schema-pack.md`. Future code-bearing extensions remain host-local and separately governed by `Contracts/trusted-executable-extension.md`. Optional commercial acquisition remains external under `Contracts/distribution-entitlement-boundary.md`.
