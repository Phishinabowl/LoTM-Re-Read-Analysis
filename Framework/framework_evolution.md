...
The LoTM project selects the foundation and narrative extensions because its schema intentionally supports physical embodiment, while simulation and compute vocabulary remain unavailable. An unrelated project may retain an empty schema-2 hosting registry while the capability is disabled; the runtime returns an explicitly disabled service with no hosting provenance or reconciliation providers. Any nonempty hosting record still fails closed until the capability is enabled. The same runtime code therefore remains portable without making hosting mandatory or leaking domain vocabulary.
