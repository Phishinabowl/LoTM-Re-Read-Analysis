...
| `CONF-PROVENANCE` | Verify assertion shape and grouping, typed cross-registry subjects, semantic field paths, evidence roles and locators, source scope and coverage, observation/effective timing, claim applicability, acyclic supersession, winner/tie/conflict/incomparable authority decisions, malformed configuration, invalid queries, and scale without freezing project facts. | `Tools/Conformance/Suites/test_provenance.py`, `Tools/Conformance/Suites/Test-Provenance.ps1`, and `Framework/Data/Provenance/`. |
