...
| `SCENARIO-LOKI` | Source-grounded temporal-topology and branch-lifecycle stress case. Can the model separate TVA-local order, timeline branch identity and lineage, pruning, restoration, preservation, merge or replacement, repeated Loom attempts, retained learning, bootstrap causality, and final termination? Aggregate cardinality, repeated participation, chronology-context topology, and branch lifecycle should use their own services; accumulated expertise must remain an explicit unsupported boundary until its later capability exists. |
