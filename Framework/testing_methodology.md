...
| --- | --- | --- | --- | --- | --- |
| `CONF-PACK-COMPOSITION` | Canonical eight-pack project plus neutral schema-3 three-pack fixture | 56 structured invalid compositions | Dependency order/version, lifecycle, activation, typed semantic closure including dormant reusable state profiles and required kind mappings, and fail-fast orphan rejection | Duplicate providers, controlled-value ownership, hierarchy cycles, typed member collisions, profile ownership/mapping conflicts, and conflict-scope collisions | Generated 64-pack composition with 64 typed declarations plus two delimiter-colliding typed pairs |
...
| `CONF-PROJECT-COMPOSITION` | Two deterministic complete canonical loads | Ten missing, duplicate, unregistered, or capability-disabled compositions | Exact manifest paths/schemas plus available, enabled, and disabled capability boundaries | Reconciliation/provenance provider closure, chronology-context and branch-continuity typed-target closure, state-profile mapping closure, and duplicate-provider rejection | Reviewed exact oracle spanning eight packs, 119 enabled capabilities, 976 controlled values, three state profiles, nine state-kind mappings, and 60 provenance subject types |
...
| Occurrence, participation, track-entry, recurrence, state, or effect semantics | `CONF-OCCURRENCE`, `PRESSURE-RECURRENCE-STATE`, `PRESSURE-EPISTEMIC-STATE` when subject knowledge/access/belief changes, Derrick and Loki scenarios, repeated observation/review/intervention probes, cross-domain pressure, provenance targets, and project compatibility. |
