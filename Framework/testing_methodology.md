...
| `CONF-PROJECT-COMPOSITION` | Two deterministic complete canonical loads | Thirteen missing, duplicate, unregistered, or capability-disabled compositions | Exact manifest paths/schemas plus available, enabled, and disabled capability boundaries | Reconciliation/provenance provider closure, interpretation deferred-claim closure, participation-chronology, chronology-context, and branch-continuity typed-target closure, four-dimension state-profile mapping closure, and duplicate-provider rejection | Reviewed exact oracle spanning eight packs, 122 enabled capabilities, 1,014 controlled values, four state profiles, 18 state-kind mappings, and 66 provenance subject types |
