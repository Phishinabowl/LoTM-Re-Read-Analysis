...
| `CONF-PROJECT-COMPOSITION` | Two deterministic complete canonical loads | Eleven missing, duplicate, unregistered, or capability-disabled compositions | Exact manifest paths/schemas plus available, enabled, and disabled capability boundaries | Reconciliation/provenance provider closure, chronology-context and branch-continuity typed-target closure, four-dimension state-profile mapping closure, and duplicate-provider rejection | Reviewed exact oracle spanning eight packs, 120 enabled capabilities, 994 controlled values, four state profiles, 18 state-kind mappings, and 61 provenance subject types |
