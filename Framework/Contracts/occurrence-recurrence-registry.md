...
`Project_Config/occurrences.yaml` instantiates occurrence identity, recurrence structure, branch topology, observation tracks, transitions, causal relations, and carryover. `Tools/occurrence_config.py` and `Tools/Occurrence-Config.ps1` are the behaviorally paired schema-1 loaders and query service.
...
An occurrence binding has its own stable ID, chronology `position_id`, and role. The same chronology position may be reused by any number of distinct occurrences. One occurrence may carry multiple bindings when different coordinates contextualize the same happening.

## Transitions, Causality, and Carryover

Transitions connect occurrence identities without redefining chronological order. Reset transitions must advance from an earlier to a later iteration ordinal in the same recurrence. Exit transitions may leave a recurrence. Domain packs may add specialized transition kinds.
...
Carryover records state what crosses an iteration boundary for one track. Source and target iterations must belong to the same recurrence and advance in ordinal order. Core kinds cover generic state and context; domain packs may specialize memory, knowledge, physical state, awareness, or other retained properties.
...
The paired services expose deterministic queries for occurrences in an iteration, occurrences bound to a chronology position, previous and next occurrences on a track, carryover into an iteration, and the recurrence containing an occurrence. These queries answer both "what happened during iteration 7?" and "what did this participant experience immediately before iteration 7 began?" without introducing a chronological cycle.

## Layering

- Core owns occurrence/template identity, recurrence and iteration structure, branches, bindings, transitions, causal edges, tracks, carryover, validation, and queries.
...
`Framework/Data/Occurrence/` contains the portable V31 fixture corpus. Run `python Tools/test_occurrence.py` and `powershell -NoProfile -ExecutionPolicy Bypass -File Tools/Test-Occurrence.ps1` after changing occurrence vocabulary, registry shape, chronology composition, recurrence validation, or query behavior.
