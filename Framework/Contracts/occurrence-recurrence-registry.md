...
Evaluation returns `proposed_effects`, `authorized_effects`, and an explicit `execution_disposition` alongside selected rules, traces, and conflicts. Only a conflict-free `selected` evaluation is `authorized`. Any conflict is `blocked-conflict`, any indeterminate result is `blocked-indeterminate`, and no match is `not-applicable`; all three expose no authorized effects while preserving diagnostic proposals where available. Authorization is conflict-wide. Partial execution, compensation, transactions, and effect payload execution are not inferred. Missing effective time required by an applicability window or civil schedule is indeterminate; a supplied time outside the applicable window is a definite non-match.
