...
`Project_Config/occurrences.yaml` instantiates occurrence identity, occurrence participation, participation-relative chronology selection, subjective track entries, recurrence patterns and executions, aggregate recurrence cardinality, phases and schedules, branch topology and lifecycle, subject tracks, transitions, causal relations, outcomes, recurrence rules, state scales, state transitions, and iteration carryover. `Tools/Runtime/Python/knowledge_framework/occurrence_config.py` and `Tools/Runtime/PowerShell/KnowledgeFramework/KnowledgeFramework.psd1` are the behaviorally paired schema-10 loaders and query services. V40 separates one concrete happening from each subject's participation in it and from each participation's ordered placement on a track. V42 adds provenance-addressable branch-state histories without converting branch lifecycle into chronology. V43 adds pack-composed epistemic state profiles without treating subject knowledge or belief as objective truth. V44 adds local capability scales without treating credentials, assessments, or elapsed time as demonstrated competence. V46 lets participations and exact track entries select the chronology bindings that apply to that involvement and closes repeated-visit state boundaries without adding chronology edges.
...
- `occurrence_participations` give one subject's involvement in one occurrence a stable identity, role, perspective, and status.
- `participation_chronology_bindings` link either one participation or one track entry to an existing chronology binding on the same occurrence, with an optional matching chronology context.
...
Occurrence, participation, and track-entry identity are independent. One occurrence remains one concrete happening even when the same subject encounters it repeatedly. Each involvement uses a distinct participation. Role, perspective, or status may distinguish those participations directly. Semantically identical participations are also legal when every record shares a subject-matching track that orders the encounters; identical records without a common ordering track are rejected as unexplained duplication.

A participation may select several existing bindings from its occurrence through stable `participation_chronology_bindings`. A track-entry link may add a binding that applies only to that ordered placement. Participation-level links are inherited by every entry for that participation, while an entry-level duplicate of an inherited link is invalid. Every link must resolve to the same occurrence as its target participation or entry. When it names a chronology context, the selected position's coordinate system must match that context. The chronology registry owns positions, typed context topology, extratemporal oversight, mappings, comparison, and cross-context intervention; link records only select existing position/context pairs. They do not imply comparability and never add chronology edges.
...
Participation status describes involvement lifecycle, not subject knowledge or objective truth. State availability and acquisition remain `state_transitions`; evidence and authority remain provenance. `occurrence-participation`, `occurrence-track-entry`, and `occurrence-participation-chronology-binding` are stable provenance subjects.
...
Change profiles enforce broad invariants such as unavailable-to-available acquisition, equal-state preservation, improvement, degradation, removal into unavailable or inaccessible state, restoration, transfer, merge, derivation, activation, and invalidation. On one track, transitions for the same subject, payload, and state kind form a continuous chain: every later prior dimension must equal the earlier resulting value. A transition retains its canonical `activation_occurrence_id` and may name one `activation_track_entry_id` per affected track. If that occurrence appears more than once on a track, the exact entry is mandatory; if it appears once, the compact occurrence-relative boundary remains legal. State-chain ordering, carryover, and entry-relative lookup all use the resulting exact track ordinal. Encountering an occurrence does not imply awareness, memory, belief, access, or competence. Change shape describes how progression occurs; mechanism describes what caused it. Neither is inferred from elapsed time, repeated participation, successful outcomes, or assessment records.
...
Paired services query branch-state history and state at a branch-local lifecycle ordinal, iteration contents, recurrence cardinality claims, coordinate reuse, participations by occurrence or subject, chronology selections for a participation or exact track entry, track entries by occurrence, entry-relative neighbors, unambiguous occurrence-relative neighbors, iteration contents and boundaries on a track, recurrence identity and phase, expected schedule values and due status, incoming carryover, outcomes for an occurrence, rules for a recurrence pattern, state transitions for a subject, the latest applicable state at an unambiguous track occurrence or exact track entry, and deterministic recurrence-rule evaluation with a trace. These answer both what happened and which bounded policy applies without introducing chronological cycles.

## Layering

- Core owns occurrence, participation, participation-relative chronology selection, track-entry ordering, exact entry-relative state boundaries, pattern, execution, aggregate realized-history cardinality, iteration, phase, schedule, branch identity and generic lifecycle, outcome, rule evaluation, semantic declaration validation, deterministic effect resolution, civil schedule boundaries, generic subject-state profiles and continuity, carryover, validation, and queries.
