...
`Tools/provenance_config.py` and `Tools/Provenance-Config.ps1` are behaviorally paired loaders. They load only after source and entity registries and compose those registries through typed provenance-target APIs. A controlled subject type without an installed provider, or a subject ID absent from its provider, is invalid.
