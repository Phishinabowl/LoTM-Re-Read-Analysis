...
`Project_Config/sources.yaml` schema version 10 is the executable project-instance contract for selected narrative packs. `Tools/source_config.py` and `Tools/Source-Config.ps1` enforce the same fields and reference rules.
...
Continuity membership and relationship statuses come from the selected-pack `source.membership-status` namespace rather than loader-local allowlists. Authority profiles determine accepted statuses, continuity precedence, source-priority direction, comparable derivative relationships, conflict behavior, and deviation ownership. Repeatable claim-authority rules may override a source's general numeric priority by matching source IDs, source roles, medium IDs, or evidence modes. Claim namespaces inherit through the selected-pack `broader_value` hierarchy, so a `canonical-content` rule can apply to `dialogue` unless a higher-precedence dialogue rule wins. Higher positive `precedence` wins; ties among matching winners are rejected. Rule IDs are global provenance targets. Consumers fall back to ordinary source priority when no rule matches.
...
`provenance_assertions` is the reusable evidence bridge for factual registry records and globally stable nested records, including content-group members, localized titles, release-run phases, source observations, source-coverage entries and ranges, and claim-authority rules. An assertion declares a controlled `claim_namespace`, snapshots the asserted value, optional dotted/indexed field path, observation time, effective window, and verification state. A field path must resolve against the normalized target record; syntactically valid references to absent fields or array positions are rejected. Every evidence link references a canonical source ID and one or more globally stable locators. Each locator explicitly selects an allowed medium and is either a `point` with one position or a `range` with identically shaped, correctly ordered start/end positions. Positions must contain required medium fields, remain inside the source work scope, and satisfy any active structural validator. This permits one source artifact to cite a novel span and an embedded illustration without pretending those positions share one schema. Roles distinguish supporting, contradicting, and contextual evidence. Verified and inferred assertions require support, while disputed assertions require both support and contradiction. This central contract avoids adding incompatible citation fields to every record type and can be reused by non-narrative schema packs.

## Validation Boundary

The paired loaders currently validate media facets, explicit work-scope fields, and configured structural position strategies; work/release identity and hierarchy; work groups; continuities; precedence-based hierarchical claim authority; segments; recursively nested content groups with stable, role-bearing members; nonoverlapping historical localized titles; numbering schemes; total and partial ordering schemes; work and multi-input adaptation relationships; manifestations and segment mappings; package- or manifestation-scoped components and component lineage; release packages/phased runs/events; hierarchical territories; localized platform catalogs and offerings; structured time windows; external identifiers; globally addressable multi-target source observations and coverage ranges; ordered, structurally valid composite evidence coverage; point/range mixed-media assertion provenance; semantic provenance field paths; source containers; aliases; volume catalogs; citation schemas; and resource bindings.
