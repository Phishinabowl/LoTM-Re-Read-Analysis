...
Outgoing and incoming relation queries preserve registry order and may be filtered by relation type. Unknown context queries fail explicitly. Positions, contexts, relations, and bindings are independent provenance subjects. The provider exposes the canonical position objects already owned by the registry through a detached inventory map, so callers cannot remove canonical state by mutating the returned container. It does not copy the position records themselves or promote coordinate systems, eras, spans, chronology relations, or mappings into provenance subjects.
