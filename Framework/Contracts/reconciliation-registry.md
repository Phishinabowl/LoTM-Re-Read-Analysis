...
`Project_Config/reconciliation.yaml` schema version 2 is the domain-neutral project-instance audit registry for stable IDs that have been redirected, merged, split, retired, or reclassified. Target types come from selected schema packs and must have exactly one installed provider from taxonomy, resources, sources, entities, or a future registry.
...
`source_state: present` requires the source to remain in its provider. `source_state: tombstone` requires it to be absent and reserves that historical identity against reuse. A tombstoned stable ID must not also remain a provider alias. Consumers resolve stable IDs through reconciliation before ordinary alias lookup rather than allowing two mechanisms to claim the same historical key.
...
- `explicit` requires timezone-bearing RFC 3339 `recorded_at` and a non-empty `actor_ref`; `approval_ref` and stable `migration_id` are optional.
...
Portable conformance fixtures live in `Framework/Data/Reconciliation/`. Run `python Tools/test_reconciliation.py` or `powershell -NoProfile -ExecutionPolicy Bypass -File Tools/Test-Reconciliation.ps1` to verify Python, PowerShell 7, or Windows PowerShell 5.1 behavior.
