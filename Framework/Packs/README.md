...
| Pack | Purpose |
| --- | --- |
| `core` | Domain-neutral identity, stable nested-record identity, composite and source-bounded evidence scope, locator-level evidence modes, explainable hierarchical precedence-aware authority, point/range evidence locators, semantic provenance paths, structural position validation, ordered temporal windows, relationships, visibility, projection, and validation. |
...
Claim namespaces, evidence modes, and content-group member roles are pack-owned vocabulary. Claim namespaces may declare a `broader_value`; authority rules inherit down that hierarchy and use explicit precedence to resolve broad defaults against narrow exceptions. Mode-specific rules evaluate the mode selected by the locator, and explainable decisions retain the winning rule or priority fallback. Position-structure strategies are also pack-owned: narrative publishing contributes `work-volume-catalog`, while narrative media contributes `work-segment-ordering` for stable segments interpreted through an explicit total ordering. Unrelated domains may omit structural validation or supply their own strategy. Downstream projects activate only capabilities supplied by their selected packs, so an absent feature remains disabled rather than becoming an error in an unrelated industry configuration.
