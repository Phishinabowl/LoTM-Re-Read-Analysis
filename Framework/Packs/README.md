...
Core `occurrence-recurrence-modeling` keeps concrete happenings, branches, iterations, profiled transitions, causal edges, and tracks separate from chronology coordinates. `recurrence-rule-modeling` separates reusable patterns from concrete executions and adds typed outcomes, bounded conditions/effects, and coherent lifecycle. `state-availability-acquisition` records subject state independently from encounter and provenance, while carryover references an applicable state transition across an iteration boundary. Narrative media adds time-loop, subjective-experience, loop reset/escape, narrative outcomes, memory/knowledge/awareness/belief/physical-state kinds, and specialized acquisition mechanisms. Causal cycles remain valid; chronological cycles remain invalid.
